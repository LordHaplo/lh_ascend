...
void handlePostAdventure()
{
	...
	{
		if(get_property("lh_diag_round").to_int() == 0)
		{
	...
	if((get_property("lastEncounter") == "Daily Briefing") && (lh_my_path() == "License to Adventure"))
	{
		set_property("_lh_bondBriefing", "started");
	}

	if((get_property("_villainLairProgress").to_int() < 999) && ((get_property("_villainLairColor") != "") || get_property("_villainLairColorChoiceUsed").to_boolean()) && (lh_my_path() == "License to Adventure") && (my_location() == $location[Super Villain\'s Lair]))
	{
	...
	if(get_property("lh_disableAdventureHandling").to_boolean())
	{
	...
	{
		// Just so the "are we beaten up?" check in lh_koe works properly
	...
	{
		if((my_mp() >= 12) && lh_have_skill($skill[Disco Nap]))
		{
	...
	if(my_class() == $class[Avatar of Sneaky Pete])
	{
		...
		if(lh_have_skill($skill[Throw Party]) && !get_property("_petePartyThrown").to_boolean())
		{
			int threshold = 50;
		...
		if(lh_have_skill($skill[Incite Riot]) && !get_property("_peteRiotIncited").to_boolean())
		{
			int threshold = -50;
			if(!possessEquipment($item[Sneaky Pete\'s Leather Jacket]) && !possessEquipment($item[Sneaky Pete\'s Leather Jacket (Collar Popped)]))
			{
	...
	if(my_path() == "Actually Ed the Undying")
	{
		...
		if((my_level() < 13) && (my_level() > 3) && !get_property("lh_needLegs").to_boolean() && (get_property("edPoints").to_int() > 15) && !($locations[Hippy Camp, The Outskirts Of Cobb\'s Knob] contains my_location()))
		{
	...
	if(poisoned)
	{
		if((my_mp() > 12) && lh_have_skill($skill[Disco Nap]) && lh_have_skill($skill[Adventurer of Leisure]))
		{
			use_skill(1, $skill[Disco Nap]);
		}
	...
	if(my_path() == "Community Service")
	{
		...
		else if(lh_have_skill($skill[Summon Taffy]))
		{
		...
		{
			if(is_unrestricted(sk) && lh_have_skill(sk) && (my_mp() >= mp_cost(sk)))
			{
				use_skill(1, sk);
			}
		}
	...
	{
		if((my_level() >= 6) && (have_effect($effect[[1458]Blood Sugar Sauce Magic]) == 0) && lh_have_skill($skill[Blood Sugar Sauce Magic]) && !in_hardcore())
		{
	...
	if(lh_have_skill($skill[Thunderheart]) && (my_thunder() >= 90) && ((my_turncount() - get_property("lh_lastthunderturn").to_int()) >= 9))
	{
	...
	if(lh_have_skill($skill[Demand Sandwich]) && (my_mp() > 85) && (my_level() >= 9) && (get_property("_demandSandwich").to_int() < 3))
	{
		use_skill(1, $skill[Demand Sandwich]);
	}

	if(lh_have_skill($skill[Summon Smithsness]) && (my_mp() > 20))
	{
	...
	if(my_maxmp() < 50)
	{
		...
		foreach sk in toCast
		{
			if(is_unrestricted(sk) && lh_have_skill(sk) && ((my_mp() - 40) >= mp_cost(sk)))
			{
				use_skill(1, sk);
			}
		}
...
#		buffMaintain($effect[Tenacity of the Snapper], 30, 1, 10);
	...
	else if(my_maxmp() < 80)
	{
		...
		foreach sk in toCast
		{
			if(is_unrestricted(sk) && lh_have_skill(sk) && ((my_mp() - 50) >= mp_cost(sk)))
			{
				use_skill(1, sk);
			}
		}
...
#		buffMaintain($effect[Tenacity of the Snapper], 50, 1, 10);
	...
	else if(my_maxmp() < 170)
	{
		...
		foreach sk in toCast
		{
			if(is_unrestricted(sk) && lh_have_skill(sk) && ((my_mp() - 90) >= mp_cost(sk)))
			{
				use_skill(1, sk);
			}
		}
...
#		buffMaintain($effect[Rotten Memories], 100, 1, 10);
	...
	else
	{
		...
		if(((get_property("flyeredML").to_int() > 9999) || get_property("lh_hippyInstead").to_boolean() || (get_property("lh_war") == "finished") || (get_property("sidequestArenaCompleted") != "none")) && (my_level() >= 13))
		{
			doML = false;
			#lh_change_mcd(0);
		}
		...
		foreach sk in toCast
		{
			if(is_unrestricted(sk) && lh_have_skill(sk) && ((my_mp() - 85) >= mp_cost(sk)))
			{
				use_skill(1, sk);
			}
		}
...
#		buffMaintain($effect[Rotten Memories], 150, 1, 10);
		...
		if(get_property("kingLiberated").to_boolean())
		{
			if((lh_have_skill($skill[Summon Rad Libs])) && (my_mp() > 6))
			{
				use_skill(3, $skill[Summon Rad Libs]);
			}
			if((lh_have_skill($skill[Summon Geeky Gifts])) && (my_mp() > 5))
			{
				use_skill(1, $skill[Summon Geeky Gifts]);
			}
			if((lh_have_skill($skill[Summon Stickers])) && (my_mp() > 6))
			{
				use_skill(3, $skill[Summon Stickers]);
			}
			if((lh_have_skill($skill[Summon Sugar Sheets])) && (my_mp() > 6))
			{
				use_skill(3, $skill[Summon Sugar Sheets]);
			}
			if(lh_have_skill($skill[Rainbow Gravitation]))
			{
...
*/
		...
		if((my_mp() >= (1.2 * mp_cost($skill[Bind Vermincelli]))) && (cur == $thrall[none]) && lh_have_skill($skill[Bind Vermincelli]))
		{
			consider = $thrall[Vermincelli];
		}
		if((my_mp() >= (1.2 * mp_cost($skill[Bind Spice Ghost]))) && lh_have_skill($skill[Bind Spice Ghost]) && (my_daycount() > 1) && (numeric_modifier("MP Regen Min").to_int() > 9))
		{
	...
	if(get_property("lh_cubeItems").to_boolean() && (item_amount($item[Ring Of Detect Boring Doors]) == 1) && (item_amount($item[Eleven-Foot Pole]) == 1) && (item_amount($item[Pick-O-Matic Lockpicks]) == 1))
	{
	...
	if((get_property("lh_beatenUpCount").to_int() <= 10) && (have_effect($effect[Beaten Up]) > 0) && (my_mp() >= mp_cost($skill[Tongue of the Walrus])) && lh_have_skill($skill[Tongue of the Walrus]))
	{
	...
	if(get_property("kingLiberated").to_boolean() && (have_effect($effect[Beaten Up]) > 0) && (my_mp() >= mp_cost($skill[Tongue of the Walrus])) && lh_have_skill($skill[Tongue of the Walrus]))
	{
		print("Owwie, was beaten up but trying to recover", "red");
		use_skill(1, $skill[Tongue of the Walrus]);
	}
	...
	{
		set_property("lh_modernzmobiecount", "" + (get_property("lh_modernzmobiecount").to_int() + 1));
		print("Fought " + get_property("lh_modernzmobiecount") + " modern zmobies.", "blue");
	}

	if(have_effect($effect[Disavowed]) > 0)
	{
		if(get_property("_lh_bondBriefing") != "finished")
		{
			set_property("_lh_bondBriefing", "started");
		}
		if(lh_have_skill($skill[Disco Nap]) && (my_mp() > mp_cost($skill[Disco Nap])))
		{
	...
	set_property("lh_digitizeDirective", "");
