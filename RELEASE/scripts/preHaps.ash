...
{
	if((equipped_item($slot[familiar]) == $item[none]) && (my_familiar() != $familiar[none]) && (lh_my_path() == "Heavy Rains"))
	{
...
#	}
	...
	lh_debug_print("Adventuring at " + place.to_string(), "green");

	familiar famChoice = to_familiar(get_property("lh_familiarChoice"));
...
#			print("FAMILIAR DIRECTIVE ERROR: Selected " + famChoice + " but have " + my_familiar(), "red");
	...
	if(lh_have_familiar($familiar[cat burglar]))
	{
		...
		foreach mon, it in heistDesires
		{
			foreach i, mmon in get_monsters(place)
			{
				if(mmon == mon)
				{
					lh_debug_print("Using cat burglar because we want to burgle a " + it + " from " + mon);
					wannaHeist = true;
				}
			}
		}
	...
	if(my_path() == $class[Avatar of Boris])
	{
		...
		else if((place.turns_spent > 1) && (place != get_property("lh_priorLocation").to_location()))
		{
			//When do we consider Song of Cockiness?
			buffMaintain($effect[Song of Fortune], 10, 1, 1);
			if(have_effect($effect[Song of Fortune]) == 0)
	...
	{
		if((zone_combatMod(place)._int < combat_rate_modifier()) && (have_effect($effect[Shelter Of Shed]) == 0) && lh_have_skill($skill[Shelter Of Shed]))
		{
			acquireMP(25, false);
		}
		acquireMP(20, false);
	...
	if(!get_property("kingLiberated").to_boolean())
	{
		...
		{
			acquireCombatMods(zone_combatMod(place)._int, lh_beta());
		}
		if(place == $location[Sonofa Beach] && !lh_voteMonster())
		{
			acquireCombatMods(zone_combatMod(place)._int, lh_beta());
		}
	...
	{
		if(lh_wantToYellowRay(mon, place))
		{
			adjustForYellowRayIfPossible(mon);
		}

		if(lh_wantToBanish(mon, place))
		{
			adjustForBanishIfPossible(mon, place);
		}
	}

	if(lh_latteDropWanted(place))
	{
		print('We want to get the "' + lh_latteDropName(place) + '" ingredient for our latte from ' + place + ", so we're bringing it along.", "blue");
		lhEquip($item[latte lovers member's mug]);
	}
	...
	if((place == $location[The Haunted Wine Cellar]) && (my_turncount() != 0) && (get_property("lh_winebomb") == "partial"))
	{
		if(!possessEquipment($item[Unstable Fulminate]))
		{
			abort("Tried to charge a WineBomb but don't have one.");
		}
		lhEquip($slot[off-hand], $item[Unstable Fulminate]);
	}
	...
	set_property("lh_priorLocation", place);
