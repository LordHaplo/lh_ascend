...
void ed_initializeSettings()
{
	if(my_path() == "Actually Ed the Undying")
	{
		...
		set_property("lh_lashes", "");
		set_property("lh_needLegs", false);
		set_property("lh_renenutet", "");
		set_property("lh_servantChoice", "");
		set_property("lh_useCubeling", false);
		set_property("lh_wandOfNagamar", false);

		set_property("lh_edSkills", -1);
		set_property("lh_chasmBusted", false);
		set_property("lh_renenutetBought", 0);

		set_property("lh_edCombatCount", 0);
		set_property("lh_edCombatRoundCount", 0);
...
void ed_initializeSession()
{
	if(my_class() == $class[Ed])
	{
		...
		{
			set_property("lh_hpAutoRecoveryItems", get_property("hpAutoRecoveryItems"));
			set_property("lh_hpAutoRecovery", get_property("hpAutoRecovery"));
			set_property("lh_hpAutoRecoveryTarget", get_property("hpAutoRecoveryTarget"));
...
void ed_terminateSession()
{
	if(my_class() == $class[Ed])
	{
		if(get_property("hpAutoRecoveryItems") == "linen bandages")
		{
			set_property("hpAutoRecoveryItems", get_property("lh_hpAutoRecoveryItems"));
			set_property("hpAutoRecovery", get_property("lh_hpAutoRecovery"));
			set_property("hpAutoRecoveryTarget", get_property("lh_hpAutoRecoveryTarget"));
			set_property("lh_hpAutoRecoveryItems", "");
			set_property("lh_hpAutoRecovery", 0.0);
			set_property("lh_hpAutoRecoveryTarget", 0.0);
		}
	}
}
...
void ed_initializeDay(int day)
{
	...
	else if(day == 2)
	{
		...
		if(get_property("lh_day_init").to_int() < 2)
		{
			...
			if(get_property("lh_dickstab").to_boolean() && chateaumantegna_available())
			{
...
boolean L13_ed_towerHandler()
{
	...
	if(contains_text(visit_url("place.php?whichplace=nstower"), "ns_10_sorcfight"))
	{
		...
		lhAdvBypass(page, $location[Noob Cave]);
	...
	else
	{
		...
		if((my_level() < 13) && elementalPlanes_access($element[spooky]))
		{
			...
			else
			{
				...
				{
					lhEquip($slot[acc2], $item[Personal Ventilation Unit]);
				}
				lhAdv(1, $location[The Secret Government Laboratory]);
			}
			return true;
		}
		else if((my_level() < 13) && elementalPlanes_access($element[stench]))
		{
			lhAdv(1, $location[Pirates of the Garbage Barges]);
			return true;
		}
		else
		{
...
boolean L13_ed_councilWarehouse()
{
	...
	if(get_property("lh_sorceress") != "finished")
	{
		return false;
	}
...
#	if(item_amount($item[Holy MacGuffin]) == 0)
	...
	{
		lhAdv(1, $location[The Secret Council Warehouse]);
	}
	else
	{
	...
	if(get_property("lastEncounter") == "You Found It!")
	{
		...
		print("Ed Combats: " + get_property("lh_edCombatCount"), "blue");
		print("Ed Combat Rounds: " + get_property("lh_edCombatRoundCount"), "blue");
...
boolean ed_buySkills()
{
	...
	if(my_level() <= get_property("lh_edSkills").to_int())
	{
		return false;
	}
	int possEdPoints = 0;
	...
	set_property("lh_edSkills", my_level());
	return true;
}
...
boolean ed_eatStuff()
{
	...
	if(canEat > 0)
	{
		lhChew(canEat, $item[Mummified Beef Haunch]);
	}
	xiblaxian_makeStuff();
...
#	}
	...
	if(get_property("lh_dickstab").to_boolean() && !get_property("_fancyHotDogEaten").to_boolean() && (my_daycount() == 1) && ((my_fullness() + 2) <= fullness_limit()) && (item_amount($item[Astral Hot Dog]) == 0) && (item_amount($item[Clan VIP Lounge Key]) > 0) && chateaumantegna_available())
	{
	...
	{
		if((item_amount($item[Clan VIP Lounge Key]) > 0) && (my_meat() >= 500) && (inebriety_limit() == 4) && ((my_inebriety() == 0) || (my_inebriety() == 3)) && (lh_get_clan_lounge() contains $item[Clan Speakeasy]))
		{
...
boolean ed_needShop()
{
	...
	if(have_skill($skill[Upgraded Legs]) && get_property("lh_needLegs").to_boolean())
	{
		set_property("lh_needLegs", false);
	}

	if(get_property("lh_needLegs").to_boolean() && (item_amount($item[Ka Coin]) >= 10))
	{
		return true;
	}
	...
	if((canEat == 0) && have_skill(limiter) && (item_amount($item[Linen Bandages]) >= 4) && (get_property("lh_renenutetBought").to_int() >= 7) && (item_amount($item[Holy Spring Water]) >= 1) && (item_amount($item[Talisman of Horus]) >= 2))
	{
...
boolean ed_shopping()
{
	...
	else if(!have_skill($skill[Upgraded Legs]) && !get_property("lh_dickstab").to_boolean())
	{
		if(coins >= 10)
		{
			print("Buying Upgraded Legs", "green");
	...
	else if(!have_skill($skill[More Legs]) && !get_property("lh_dickstab").to_boolean())
	{
		if(coins >= 20)
		{
	...
	else if(!have_skill($skill[Replacement Liver]) && get_property("lh_dickstab").to_boolean())
	{
		if(coins >= 30)
		{
	...
	else if(!have_skill($skill[Elemental Wards]) && !get_property("lh_dickstab").to_boolean())
	{
		if(coins >= 10)
		{
	...
	else if((get_property("lh_renenutetBought").to_int() < 7) && (coins > 1))
	{
		while((get_property("lh_renenutetBought").to_int() < 7) && (coins > 1))
		{
			...
			set_property("lh_renenutetBought", 1 + get_property("lh_renenutetBought").to_int());
			coins = coins - 1;
	...
	{	//437 438?
		while((coins >= 1) && (get_property("lh_renenutetBought").to_int() < 7))
		{
			print("Buying Talisman of Renenutet", "green");
			visit_url("shop.php?pwd=&whichshop=edunder_shopshop&action=buyitem&quantity=1&whichrow=439", true);
			set_property("lh_renenutetBought", 1 + get_property("lh_renenutetBought").to_int());
...
boolean ed_handleAdventureServant(int num, location loc, string option)
{
	...
	if(reassign)
	{
		if(!have_skill($skill[Gift of the Maid]) && have_servant($servant[Maid]) && (get_property("lh_nuns") != "finished") && (get_property("lh_nuns") != "done"))
		{
...
boolean ed_lhAdv(int num, location loc, string option, boolean skipFirstLife)
{
	if((option == "") || (option == "lh_combatHandler"))
	{
		option = "lh_edCombatHandler";
	}
	...
	while(num > 0)
	{
		...
		set_property("lh_edCombatHandler", "");
		...
		if(contains_text(page, "whichchoice value=1023"))
		{
			...
			if(contains_text(page, "whichchoice value=1023"))
			{
				...
				set_property("lh_edCombatStage", 2);
...
boolean ed_lhAdv(int num, location loc, string option)
{
	return ed_lhAdv(num, loc, option, false);
}
...
#		}
	...
	if(get_property("neverendingPartyAlways").to_boolean() || get_property("_neverendingPartyToday").to_boolean())
	{
		...
		lhAdv(1, $location[The Neverending Party]);
	...
	if(elementalPlanes_access($element[hot]))
	{
		...
		lhAdv(1, $location[The SMOOCH Army HQ]);
	...
	lh_change_mcd(max(2, my_level()-1));
	if(haveLegs || item_amount($item[Ka coin]) >= 10)
	{
		...
		return lhAdv(1, $location[Hippy Camp]);
	}
	set_property("lh_needLegs", true);
	return lhAdv(1, $location[The Outskirts of Cobb\'s Knob]);
}
...
boolean L9_ed_chasmStart()
{
	if((my_class() == $class[Ed]) && !get_property("lh_chasmBusted").to_boolean())
	{
		...
		lhAdvBypass("place.php?whichplace=orc_chasm&action=bridge_done", $location[The Smut Orc Logging Camp]);
...
boolean LM_edTheUndying()
{
	...
	if(!get_property("lh_dickstab").to_boolean() || (my_daycount() >= 2))
	{
		if(L3_tavern())
		{
			return true;
