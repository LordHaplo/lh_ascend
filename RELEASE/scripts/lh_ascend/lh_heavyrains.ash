...
void hr_initializeSettings()
{
	if(my_path() == "Heavy Rains")
	{
		...
		set_property("lh_mountainmen", "");
		set_property("lh_ninjasnowmanassassin", "");
		set_property("lh_orcishfratboyspy", "");

		set_property("lh_lastthunder", "100");
		set_property("lh_lastthunderturn", "0");

		set_property("lh_wandOfNagamar", false);
		set_property("lh_writingDeskSummon", true);

		set_property("lh_day1_desk", "");
		set_property("lh_day1_skills", "");
	}
}
...
boolean hr_handleFamiliar(familiar fam)
{
	...
	{
		lhEquip($slot[familiar], $item[miniature life preserver]);
		return true;
	}
	return false;
}
...
{
	if(!have_skill($skill[Rain Man]) || (lh_my_path() != "Heavy Rains"))
	{
		return false;
	}
#	if(my_rain() > (92 - (12 * my_daycount())))
	...
	{
		if(get_property("lh_mountainmen") == "")
		{
		...
		if(get_property("lh_ninjasnowmanassassin") == "")
		{
			return rainManSummon("ninja snowman assassin", true, false);
		}

		if((have_effect($effect[Everything Looks Yellow]) == 0) && (get_property("lh_orcishfratboyspy") == ""))
		{
		...
		if(needDigitalKey())
		{
			if((get_property("lh_nuns") == "done") || (my_rain() > 92))
			{
...
void hr_initializeDay(int day)
{
	if(my_path() == "Heavy Rains")
	{
		if((day == 1) && (get_property("lh_day1_skills") != "finished"))
		{
			...
			set_property("lh_day1_skills", "finished");
			visit_url("main.php");
		}
...
boolean rainManSummon(string monsterName, boolean copy, boolean wink, string option)
{
	...
	# Some of the logic here has been lost due to lh_combat.ash
	...
	if(((get_property("lh_gunpowder") == "finished") || (item_amount($item[barrel of gunpowder]) >= 5)) && (monsterName == "lobsterfrogman"))
	{
		#already have the subgoal, don't summon
		return false;
	}
	...
	if(monsterName == "orcish frat boy spy")
	{
		set_property("lh_orcishfratboyspy", "done");
