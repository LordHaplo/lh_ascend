...
boolean groundhogSafeguard()
{
	if(lh_my_path() == "Live. Ascend. Repeat.")
	{
		...
		{
			if(get_property("_lh_groundhogSkip").to_int() == my_turncount())
			{
				set_property("_lh_groundhogSkipCounter", get_property("_lh_groundhogSkipCounter").to_int()+1);
			}
			if(get_property("_lh_groundhogSkipCounter").to_int() > 6)
			{
			...
			set_property("_lh_groundhogSkip", my_turncount());
		}
		else
		{
			set_property("_lh_groundhogSkipCounter", 0);
			set_property("_lh_groundhogSkip", -1);
		}
	}
	return false;
}
