script "lh_combat.ash"

monster ocrs_helper(string page);
void awol_helper(string page);
string ccsJunkyard(int round, string opp, string text);
string lh_edCombatHandler(int round, string opp, string text);
string lh_combatHandler(int round, string opp, string text);
...
{
	return get_property("lh_combatHandler").contains_text("(sk" + sk.to_int().to_string() + ")");
}

boolean haveUsed(item it)
{
	return get_property("lh_combatHandler").contains_text("(it" + it.to_int().to_string() + ")");
}

int usedCount(skill sk)
{
	matcher m = create_matcher("(sk" + sk.to_int().to_string() + ")", get_property("lh_combatHandler"));
	int count = 0;
	while(m.find())
	{
		++count;
...
{
	matcher m = create_matcher("(it" + it.to_int().to_string() + ")", get_property("lh_combatHandler"));
	int count = 0;
	while(m.find())
	{
		++count;
...
{
	set_property("lh_combatHandler", get_property("lh_combatHandler") + "(sk" + sk.to_int().to_string() + ")");
}
...
void markAsUsed(item it)
{
	if(it != $item[none])
	{
		set_property("lh_combatHandler", get_property("lh_combatHandler") + "(it" + it.to_int().to_string() + ")");
	}
}
...
string lh_combatHandler(int round, string opp, string text)
{
	#print("lh_combatHandler: " + round, "brown");
	#Yes, round 0, really.
	...
	if(round == 0)
	{
		...
		switch(enemy)
		{
			...
			case $monster[possessed wine rack]:
				set_property("lh_wineracksencountered", get_property("lh_wineracksencountered").to_int() + 1);
				break;
			case $monster[cabinet of Dr. Limpieza]:
				set_property("lh_cabinetsencountered", get_property("lh_cabinetsencountered").to_int() + 1);
				break;
		}

		set_property("lh_combatHandler", "");
		set_property("lh_funCombatHandler", "");
		set_property("lh_funPrefix", "");
		set_property("lh_combatHandlerThunderBird", "0");
		set_property("lh_combatHandlerFingernailClippers", "0");
		set_property("lh_combatHP", my_hp());
...
*/
	...
	{
		damageReceived = get_property("lh_combatHP").to_int() - my_hp();
		set_property("lh_combatHP", my_hp());
	}

	set_property("lh_diag_round", round);

	if(get_property("lh_diag_round").to_int() > 60)
	{
	...
	string combatState = get_property("lh_combatHandler");
	int thunderBirdsLeft = get_property("lh_combatHandlerThunderBird").to_int();
	int fingernailClippersLeft = get_property("lh_combatHandlerFingernailClippers").to_int();
...
//			}
	...
	if(get_property("lh_combatDirective") != "")
	{
		string[int] actions = split_string(get_property("lh_combatDirective"), ";");
		int idx = 0;
		if(round == 0)
		{
			if(actions[0] != "start")
		...
		if(idx >= 0)
		{
			...
			{
				set_property("lh_combatHandler", get_property("lh_combatHandler") + doThis);
				idx++;
			...
			set_property("lh_combatDirective", restore);
			if(idx < count(actions))
			{
				return doThis;
			}
	...
	{
		if(lh_have_skill($skill[Data Siphon]))
		{
			if(my_mp() < 50)
			{
				if(lh_have_skill($skill[Source Punch]) && (my_mp() >= mp_cost($skill[Source Punch])))
				{
	...
	{
		if(lh_have_skill($skill[Ambidextrous Funkslinging]))
		{
...
#		if((!contains_text(combatState, "love stinkbug")) && lh_have_skill($skill[Summon Love Stinkbug]))
#		{
#			set_property("lh_combatHandler", combatState + "(love stinkbug)");
...
#		}
	...
	if(enemy.to_string() == "the invader" && lh_have_skill($skill[Weapon of the Pastalord]))
	{
	...
	if(canUse($skill[Use the Force]) && (lh_saberChargesAvailable() > 0) && (enemy != lh_saberCurrentMonster()))
	{
		if(enemy == $monster[Blooper] && needDigitalKey())
		{
			handleTracker(enemy, $skill[Use the Force], "lh_copies");
			return lh_combatSaberCopy();
		}
	}
	...
	{
		if((enemy == $monster[Modern Zmobie]) && (get_property("lh_modernzmobiecount").to_int() < 3))
		{
	...
	if(!contains_text(combatState, "pickpocket") && ($classes[Accordion Thief, Avatar of Sneaky Pete, Disco Bandit, Gelatinous Noob] contains my_class()) && contains_text(text, "value=\"Pick") && canSurvive(2.0))
	{
		...
		if(tryIt)
		{
			set_property("lh_combatHandler", combatState + "(pickpocket)");
	...
	if(get_property("lh_useTatter").to_boolean())
	{
	...
	if((get_property("lh_usePowerPill").to_boolean()) && (get_property("_powerPillUses").to_int() < 20) && instakillable(enemy))
	{
	...
	if(get_property("lh_useCleesh").to_boolean())
	{
	...
	if(thunderBirdsLeft > 0)
	{
		...
		set_property("lh_combatHandlerThunderBird", thunderBirdsLeft.to_string());
	...
	if(canUse($skill[Thunder Bird]) && (monster_level_adjustment() <= 150))
	{
		...
		{
			if(canUse($skill[Curse Of Weaksauce]) && (my_mp() >= 60) && lh_have_skill($skill[Itchy Curse Finger]))
			{
				return useSkill($skill[Curse Of Weaksauce]);
			}

			set_property("lh_combatHandlerThunderBird", "5");
	...
	{
		if((item_amount($item[Louder Than Bomb]) > 0) && (get_property("lh_gremlins") == "finished"))
		{
			handleTracker(enemy, $item[Louder Than Bomb], "lh_banishes");
	...
	if(canUse($skill[Wink At]) && (my_familiar() == $familiar[Reanimated Reanimator]))
	{
		if($monsters[Lobsterfrogman, Modern Zmobie, Ninja Snowman Assassin] contains enemy)
		{
			...
			{
				set_property("lh_waitingArrowAlcove", get_property("cyrptAlcoveEvilness").to_int() - 20);
			}
			return useSkill($skill[Wink At]);
		}
	}

	if(canUse($item[Rain-Doh black box]) && (get_property("lh_doCombatCopy") == "yes") && (enemy != $monster[gourmet gourami]))
	{
		...
		{
			handleTracker(enemy, $item[Rain-Doh black box], "lh_copies");
	...
	if((enemy == $monster[Storm Cow]) && (lh_have_skill($skill[Unleash The Greash])))
	{
		return useSkill($skill[Unleash The Greash], false);
	}
	...
	if(fingernailClippersLeft > 0)
	{
		...
		set_property("lh_combatHandlerFingernailClippers", "" + fingernailClippersLeft);
	...
	if((item_amount($item[military-grade fingernail clippers]) > 0)  && (enemy == $monster[one of Doctor Weirdeaux\'s creations]))
	{
		if(!haveUsed($item[military-grade fingernail clippers]))
		{
			...
			set_property("lh_combatHandlerFingernailClippers", "3");
		}
	}
	...
	if(lh_wantToSniff(enemy, my_location()))
	{
		if(canUse($skill[Transcendent Olfaction]) && (have_effect($effect[On The Trail]) == 0))
		{
			handleTracker(enemy, $skill[Transcendent Olfaction], "lh_sniffs");
		...
		{
			handleTracker(enemy, $skill[Make Friends], "lh_sniffs");
		...
		{
			handleTracker(enemy, $skill[Long Con], "lh_sniffs");
			return useSkill($skill[Long Con]);
		}

		if(canUse($skill[Perceive Soul]) && enemy != get_property("lh_bat_soulmonster").to_monster())
		{
			handleTracker(enemy, $skill[Perceive Soul], "lh_sniffs");
			set_property("lh_bat_soulmonster", enemy);
		...
		{
			handleTracker(enemy, $skill[Gallapagosian Mating Call], "lh_sniffs");
		...
		{
			handleTracker(enemy, $skill[Offer Latte to Opponent], "lh_sniffs");
			return useSkill($skill[Offer Latte to Opponent]);
		}
	}

	if((canUse($item[Rock Band Flyers]) || canUse($item[Jam Band Flyers])) && (my_location() != $location[The Battlefield (Frat Uniform)]) && (my_location() != $location[The Battlefield (Hippy Uniform)]) && !get_property("lh_ignoreFlyer").to_boolean())
	{
		...
		{
			if(canUse($item[Time-Spinner]) && lh_have_skill($skill[Ambidextrous Funkslinging]))
			{
	...
	if(!contains_text(combatState, "yellowray") && lh_wantToYellowRay(enemy, my_location()))
	{
		string combatAction = yellowRayCombatString(enemy, true);
		if(combatAction != "")
		{
			set_property("lh_combatHandler", combatState + "(yellowray)");
			if(index_of(combatAction, "skill") == 0)
			{
				handleTracker(enemy, to_skill(substring(combatAction, 6)), "lh_yellowRays");
			}
			else if(index_of(combatAction, "item") == 0)
			{
				handleTracker(enemy, to_item(substring(combatAction, 5)), "lh_yellowRays");
			}
			else
			{
	...
	if(canUse($skill[Hugs and Kisses!]) && (my_familiar() == $familiar[XO Skeleton]) && (get_property("_xoHugsUsed").to_int() < 11))
	{
		...
		if(dohug)
		{
			handleTracker(enemy, $skill[Hugs and Kisses!], "lh_otherstuff");
	...
	if(!contains_text(combatState, "banishercheck"))
	{
		...
		if(banishAction != "")
		{
			set_property("lh_combatHandler", combatState + "(banisher)");
			if(index_of(banishAction, "skill") == 0)
			{
				handleTracker(enemy, to_skill(substring(banishAction, 6)), "lh_banishes");
			}
			else if(index_of(banishAction, "item") == 0)
			{
				handleTracker(enemy, to_item(substring(banishAction, 5)), "lh_banishes");
			}
			else
			{
		...
		set_property("lh_combatHandler", combatState + "(banishercheck)");
		combatState += "(banishercheck)";
	}

	// have_skill($skill[Macrometeorite]) seems to always return false, so we can't use canUse
	if(lh_have_skill($skill[Meteor Lore]) && (get_property("_macrometeoriteUses").to_int() < 10) && (my_mp() > mp_cost($skill[Macrometeorite])) && (lh_my_path() != "G-Lover"))
	{
		...
		if(dometeor)
		{
			handleTracker(enemy, $skill[Macrometeorite], "lh_otherstuff");
	...
	if(have_equipped($item[Protonic Accelerator Pack]) && isGhost(enemy))
	{
		...
		if(canUse($skill[Shoot Ghost], false) && (my_mp() > mp_cost($skill[Shoot Ghost])) && !contains_text(combatState, "shootghost3") && !contains_text(combatState, "trapghost"))
		{
			...
			if(contains_text(combatState, "shootghost2"))
			{
				...
				{
					set_property("lh_combatHandler", combatState + "(shootghost3)");
				}
			}
			else if(contains_text(combatState, "shootghost1"))
			{
				...
				{
					set_property("lh_combatHandler", combatState + "(shootghost2)");
				}
			}
			else
			{
				set_property("lh_combatHandler", combatState + "(shootghost1)");
			}
			...
			else
			{
				...
				set_property("lh_combatHandler", combatState);
			}
		}
		if(!contains_text(combatState, "trapghost") && lh_have_skill($skill[Trap Ghost]) && (my_mp() > mp_cost($skill[Trap Ghost])) && contains_text(combatState, "shootghost3"))
		{
			print("Busting makes me feel good!!", "green");
			set_property("lh_combatHandler", combatState + "(trapghost)");
			return useSkill($skill[Trap Ghost], false);
		}
	}

	# Ensorcel handler
	if(bat_shouldEnsorcel(enemy) && canUse($skill[Ensorcel]) && get_property("lh_bat_ensorcels").to_int() < 3)
	{
		set_property("lh_bat_ensorcels", get_property("lh_bat_ensorcels").to_int() + 1);
		handleTracker(enemy, $skill[Ensorcel], "lh_otherstuff");
	...
	{
		if(lh_have_skill($skill[Digitize]) && (get_property("_sourceTerminalDigitizeMonster") != enemy))
		{
			doInstaKill = false;
		}
	}
	...
	if(instakillable(enemy) && !isFreeMonster(enemy) && doInstaKill)
	{
		...
		{
			handleTracker(enemy, $skill[lightning strike], "lh_instakill");
			loopHandlerDelayAll();
		...
		if(canUse($skill[Chest X-Ray]) && equipped_amount($item[Lil\' Doctor&trade; bag]) > 0 && (get_property("_chestXRayUsed").to_int() < 3))
		{
			...
			{
				handleTracker(enemy, $skill[Chest X-Ray], "lh_instakill");
				loopHandlerDelayAll();
		...
		if(canUse($skill[shattering punch]) && (get_property("_shatteringPunchUsed").to_int() < 3))
		{
			if((my_adventures() < 20) || get_property("kingLiberated").to_boolean() || (my_daycount() >= 3))
			{
				handleTracker(enemy, $skill[shattering punch], "lh_instakill");
				loopHandlerDelayAll();
		...
		if(canUse($skill[Gingerbread Mob Hit]) && !get_property("_gingerbreadMobHitUsed").to_boolean())
		{
			if((my_adventures() < 20) || get_property("kingLiberated").to_boolean() || (my_daycount() >= 3))
			{
				handleTracker(enemy, $skill[Gingerbread Mob Hit], "lh_instakill");
				loopHandlerDelayAll();
...
//		THIS IS COPIED TO THE ED SECTION, IF IT IS FIXED, FIX IT THERE TOO!
		...
		{
			if(get_property("lh_batoomerangDay").to_int() != my_daycount())
			{
				set_property("lh_batoomerangDay", my_daycount());
				set_property("lh_batoomerangUse", 0);
			}
			if(get_property("lh_batoomerangUse").to_int() < 3)
			{
				set_property("lh_batoomerangUse", get_property("lh_batoomerangUse").to_int() + 1);
				handleTracker(enemy, $item[Replica Bat-oomerang], "lh_instakill");
				loopHandlerDelayAll();
		...
		{
			handleTracker(enemy, $skill[Fire the Jokester\'s Gun], "lh_instakill");
			loopHandlerDelayAll();
	...
	if(canUse($skill[Intimidating Bellow]) && (my_mp() >= 25) && lh_have_skill($skill[Louder Bellows]))
	{
	...
	if($monsters[Naughty Sorceress, Naughty Sorceress (2)] contains enemy && !get_property("lh_confidence").to_boolean())
	{
		mcd = 151;
	}
	...
	if((my_location() == $location[Super Villain\'s Lair]) && (lh_my_path() == "License to Adventure") && canSurvive(2.0) && (enemy == $monster[Villainous Minion]))
	{
	...
	if(canUse($skill[Duplicate]) && (get_property("_sourceTerminalDuplicateUses").to_int() == 0) && !get_property("kingLiberated").to_boolean() && (lh_my_path() != "Nuclear Autumn"))
	{
	...
	if(canUse($skill[Digitize]) && (get_property("_sourceTerminalDigitizeUses").to_int() == 0) && !get_property("kingLiberated").to_boolean())
	{
		if($monsters[Ninja Snowman Assassin, Lobsterfrogman] contains enemy)
		{
			...
			{
				handleTracker(enemy, $skill[Digitize], "lh_copies");
	...
	{
		if(get_property("lh_digitizeDirective") == enemy)
		{
			if(get_property("_sourceTerminalDigitizeMonster") != enemy)
			{
				handleTracker(enemy, $skill[Digitize], "lh_copies");
	...
	case $class[Sauceror]:
		...
		{
			set_property("lh_combatHandler", combatState + "(delaymortarshell)");
	...
	case $class[Disco Bandit]:
		...
		if(lh_have_skill($skill[Disco State of Mind]) && lh_have_skill($skill[Flashy Dancer]) && lh_have_skill($skill[Disco Greed]) && lh_have_skill($skill[Disco Bravado]) && monster_level_adjustment() < 150)
		{
	...
	case $class[Vampyre]:
		...
		if(my_hp() < my_maxhp() && (monster_hp() <= 30 || (monster_hp() <= 100 && lh_have_skill($skill[Hypnotic Eyes]))) && canUse($skill[Dark Feast]))
		{
	...
	if(round <= 25)
	{
		if(((my_hp() * 10)/3) < my_maxhp())
		{
			...
			{
				set_property("lh_combatHandler", combatState + "(stunner)");
				return stunner;
			}
			...
			if((!contains_text(combatState, "last attempt")) && (my_mp() >= costMajor))
			{
				if(canSurvive(1.4))
				{
					set_property("lh_combatHandler", combatState + "(last attempt)");
...
//	if(lh_have_skill($skill[Lunging Thrust-Smack]) && (my_mp() >= mp_cost($skill[Lunging Thrust-Smack])))
//	{
//		return "skill lunging thrust-smack";
//	}
	if(lh_have_skill($skill[Storm of the Scarab]) && (my_mp() >= mp_cost($skill[Storm of the Scarab])))
	{
		return "skill Storm of the Scarab";
	}
	return lh_combatHandler(round, opp, text);
//	if(lh_have_skill($skill[Lunge Smack]) && (my_mp() >= mp_cost($skill[Lunge Smack])) && (weapon_type(equipped_item($slot[weapon])) == $stat[Muscle]))
//	{
...
string ccsJunkyard(int round, string opp, string text)
{
	...
	if(round == 0)
	{
		...
		set_property("lh_gremlinMoly", true);
		set_property("lh_combatHandler", "");
	}
	else
	{
		print("lh_Junkyard: " + round, "brown");
	}
	string combatState = get_property("lh_combatHandler");
	string edCombatState = get_property("lh_edCombatHandler");

	if(my_class() == $class[Ed])
	{
		if(contains_text(edCombatState, "gremlinNeedBanish"))
		{
			set_property("lh_gremlinMoly", false);
		}
	}
	...
	if(!contains_text(edCombatState, "gremlinNeedBanish") && !get_property("lh_gremlinMoly").to_boolean() && (my_class() == $class[Ed]))
	{
		set_property("lh_edCombatHandler", "(gremlinNeedBanish)");
	}

	if(round >= 28)
	{
		if(lh_have_skill($skill[Lunging Thrust-Smack]) && (my_mp() >= 8))
		{
	...
	if(!contains_text(combatState, "weaksauce") && lh_have_skill($skill[Curse Of Weaksauce]) && (my_mp() >= mp_cost($skill[Curse Of Weaksauce])))
	{
		set_property("lh_combatHandler", combatState + "(weaksauce)");
		return "skill " + $skill[Curse Of Weaksauce];
	}

	if(!contains_text(combatState, "marshmallow") && lh_have_skill($skill[Curse Of The Marshmallow]) && (my_mp() > mp_cost($skill[Curse Of The Marshmallow])))
	{
		set_property("lh_combatHandler", combatState + "(marshmallow)");
		return "skill " + $skill[Curse Of The Marshmallow];
	}
	if(!contains_text(combatState, "love scarab") && lh_have_skill($skill[Summon Love Scarabs]))
	{
		set_property("lh_combatHandler", combatState + "(love scarab)");
		return "skill " + $skill[Summon Love Scarabs];
	}
	if(!contains_text(combatState, "love gnats") && lh_have_skill($skill[Summon Love Gnats]))
	{
		set_property("lh_combatHandler", combatState + "(love gnats)");
		return "skill " + $skill[Summon Love Gnats];
	}

	if(!contains_text(combatState, "badMedicine") && lh_have_skill($skill[Bad Medicine]) && (my_mp() >= mp_cost($skill[Bad Medicine])))
	{
		set_property("lh_combatHandler", combatState + "(badMedicine)");
		return "skill " + $skill[Bad Medicine];
	}

	if(!contains_text(combatState, "goodMedicine") && lh_have_skill($skill[Good Medicine]) && (my_mp() >= mp_cost($skill[Good Medicine])) && canSurvive(2.1))
	{
		set_property("lh_combatHandler", combatState + "(goodMedicine)");
		return "skill " + $skill[Good Medicine];
	}


	if(!get_property("lh_gremlinMoly").to_boolean() && (my_class() == $class[Ed]))
	{
		if((get_property("lh_edCombatStage").to_int() >= 2) || (get_property("lh_edStatus") == "dying"))
		{
			...
			else if((my_mp() >= 8) && lh_have_skill($skill[Storm Of The Scarab]))
			{
	...
	if(!contains_text(combatState, "flyers") && (my_location() != $location[The Battlefield (Frat Uniform)]) && (my_location() != $location[The Battlefield (Hippy Uniform)]) && !get_property("lh_ignoreFlyer").to_boolean())
	{
		if((item_amount($item[Rock Band Flyers]) > 0) && (get_property("flyeredML").to_int() < 10000))
		{
			set_property("lh_combatHandler", combatState + "(flyers)");
	...
	if(!get_property("lh_gremlinMoly").to_boolean())
	{
		if(my_class() == $class[Ed])
		{
			if((get_property("lh_edCombatStage").to_int() >= 2) || (get_property("lh_edStatus") == "dying"))
	...
	if(!get_property("lh_gremlinMoly").to_boolean())
	{
		...
		{
			if(lh_have_skill(sk) && (my_mp() >= mp_cost(sk)))
			{
				return "skill " + sk;
			}
		}
	...
	if(lh_have_skill($skill[Toss]) && (my_mp() >= mp_cost($skill[Toss])))
	{
...
string lh_edCombatHandler(int round, string opp, string text)
{
	...
	if(round == 0)
	{
		print("lh_combatHandler: " + round, "brown");
		set_property("lh_combatHandler", "");
		if(get_property("lh_edCombatStage").to_int() == 0)
		{
			set_property("lh_edCombatCount", 1 + get_property("lh_edCombatCount").to_int());
			set_property("lh_edCombatStage", 1);
			set_property("lh_edStatus", "UNDYING!");
		}
		else
		{
			set_property("lh_edCombatStage", 1 + get_property("lh_edCombatStage").to_int());
		}
	}
	else
	{
	...
	set_property("lh_edCombatRoundCount", 1 + get_property("lh_edCombatRoundCount").to_int());
	...
	if($locations[Hippy Camp, The Outskirts Of Cobb\'s Knob] contains my_location())
	{
		if(!ed_needShop())
		{
			set_property("lh_edStatus", "dying");
			if(my_mp() < 5)
			{
	...
	if(get_property("lh_edCombatStage").to_int() == 3)
	{
		set_property("lh_edStatus", "dying");
		set_property("lh_edCombatStage", 0);
	}
	set_property("lh_diag_round", round);
	...
	if(!contains_text(combatState, "pocket crumbs") && lh_have_skill($skill[Pocket Crumbs]))
	{
		set_property("lh_combatHandler", combatState + "(pocket crumbs)");
		return "skill " + $skill[Pocket Crumbs];
	}

	if(!contains_text(combatState, "micrometeorite") && lh_have_skill($skill[Micrometeorite]))
	{
		set_property("lh_combatHandler", combatState + "(micrometeorite)");
		return "skill " + $skill[Micrometeorite];
	}

	if(!contains_text(combatState, "air dirty laundry") && lh_have_skill($skill[Air Dirty Laundry]))
	{
		set_property("lh_combatHandler", combatState + "(air dirty laundry)");
	...
	{
		set_property("lh_combatHandler", combatState + "(time-spinner)");
		return "item " + $item[Time-Spinner];
	}

	if(((get_property("edPoints").to_int() <= 4) && (my_daycount() == 1)) || !get_property("lovebugsUnlocked").to_boolean())
	{
		if((!ed_needShop() || (get_property("lh_edCombatStage").to_int() > 1)) && (my_location() != $location[Barrrney\'s Barrr]))
		{
			set_property("lh_edStatus", "dying");
		}
	}

	if(!contains_text(combatState, "(sing along)") && lh_have_skill($skill[Sing Along]) && (my_mp() > (mp_cost($skill[Sing Along]))))
	{
		if((get_property("boomBoxSong") == "Remainin\' Alive") || ((get_property("boomBoxSong") == "Total Eclipse of Your Meat") && canSurvive(2.0)))
		{
			set_property("lh_combatHandler", combatState + "(sing along)");
	...
	if(have_equipped($item[Protonic Accelerator Pack]) && isGhost(enemy))
	{
		...
		if(lh_have_skill($skill[Shoot Ghost]) && (my_mp() > mp_cost($skill[Shoot Ghost])) && !contains_text(edCombatState, "shootghost3") && !contains_text(edCombatState, "trapghost"))
		{
			boolean shootGhost = true;
			if(contains_text(edCombatState, "shootghost2"))
			{
				...
				{
					set_property("lh_edCombatHandler", edCombatState + "(shootghost3)");
				}
			}
			else if(contains_text(edCombatState, "shootghost1"))
			{
				...
				{
					set_property("lh_edCombatHandler", edCombatState + "(shootghost2)");
				}
			}
			else
			{
				set_property("lh_edCombatHandler", edCombatState + "(shootghost1)");
			}
			...
			else
			{
				...
				set_property("lh_edCombatHandler", edCombatState);
			}
		}
		if(!contains_text(edCombatState, "trapghost") && lh_have_skill($skill[Trap Ghost]) && (my_mp() > mp_cost($skill[Trap Ghost])) && contains_text(edCombatState, "shootghost3"))
		{
			print("Busting makes me feel good!!", "green");
			set_property("lh_edCombatHandler", edCombatState + "(trapghost)");
	...
	if(instakillable(enemy) && !isFreeMonster(enemy) && doInstaKill)
	{
		if(!contains_text(combatState, "batoomerang") && (item_amount($item[Replica Bat-oomerang]) > 0))
		{
			...
			{
				set_property("lh_batoomerangUse", get_property("lh_batoomerangUse").to_int() + 1);
				set_property("lh_combatHandler", combatState + "(batoomerang)");
				handleTracker(enemy, $item[Replica Bat-oomerang], "lh_instakill");
				loopHandlerDelayAll();
		...
		if(!contains_text(combatState, "jokesterGun") && (equipped_item($slot[Weapon]) == $item[The Jokester\'s Gun]) && !get_property("_firedJokestersGun").to_boolean() && lh_have_skill($skill[Fire the Jokester\'s Gun]))
		{
			set_property("lh_combatHandler", combatState + "(jokesterGun)");
			handleTracker(enemy, $skill[Fire the Jokester\'s Gun], "lh_instakill");
			loopHandlerDelayAll();
	...
	if(get_property("lh_edStatus") == "UNDYING!")
	{
		if(!contains_text(combatState, "love gnats") && lh_have_skill($skill[Summon Love Gnats]))
		{
			set_property("lh_combatHandler", combatState + "(love gnats)");
		...
		if((item_amount($item[Ka Coin]) > 200) && lh_have_skill($skill[Curse of Fortune]))
		{
			if(!contains_text(combatState, "curse of fortune"))
			{
				set_property("lh_combatHandler", combatState + "(curse of fortune)");
				return "skill " + $skill[Curse of Fortune];
			}
		}
	}
	else if(get_property("lh_edStatus") == "dying")
	{
	...
	if(!contains_text(combatState, "sewage pistol") && lh_have_skill($skill[Fire Sewage Pistol]))
	{
		set_property("lh_combatHandler", combatState + "(sewage pistol)");
	...
	{
		if((item_amount($item[Ka Coin]) > 0) && (lh_have_skill($skill[Curse Of Fortune])))
		{
			set_property("lh_edCombatHandler", edCombatState + "(curse of fortune)");
			set_property("lh_edStatus", "dying");
			return "skill " + $skill[Curse Of Fortune];
		}
	}

	if((item_amount($item[The Big Book of Pirate Insults]) > 0) && !contains_text(combatState, "insults") && (numPirateInsults() < 8) && (get_property("lh_edCombatStage").to_int() < 3) && (get_property("lh_edStatus") != "dying"))
	{
		if(!contains_text(combatState, "beanscreen") && lh_have_skill($skill[Beanscreen]) && (my_mp() >= mp_cost($skill[Beanscreen])))
		{
			set_property("lh_combatHandler", combatState + "(beanscreen)");
			return "skill " + $skill[Beanscreen];
		}

		if(!contains_text(combatState, to_string($skill[Snap Fingers])) && lh_have_skill($skill[Snap Fingers]) && (my_mp() >= mp_cost($skill[Snap Fingers])))
		{
			set_property("lh_combatHandler", combatState + "(" + $skill[Snap Fingers] + ")");
		...
		{
			set_property("lh_combatHandler", combatState + "(insults)");
			return "item " + $item[The Big Book Of Pirate Insults];
		}
	}

	if(!contains_text(edCombatState, "curseofstench") && lh_have_skill($skill[Curse Of Stench]) && (my_mp() >= 35) && (get_property("stenchCursedMonster") != opp) && (get_property("lh_edCombatStage").to_int() < 3))
	{
		if(lh_wantToSniff(enemy, my_location()))
		{
			set_property("lh_edCombatHandler", combatState + "(curseofstench)");
			handleTracker(enemy, $skill[Curse Of Stench], "lh_sniffs");
	...
	if(contains_text(combatState, "insults") && (get_property("lh_edStatus") == "dying"))
	{
		if((enemy == $monster[shady pirate]) && lh_have_skill($skill[Curse Of Vacation]) && (my_mp() >= 30))
		{
			handleTracker(enemy, $skill[Curse Of Vacation], "lh_banishes");
			return "skill " + $skill[Curse Of Vacation];
		}
		if((enemy == $monster[shifty pirate]) && (get_property("_pantsgivingBanish").to_int() < 5) && lh_have_skill($skill[Talk About Politics]))
		{
			handleTracker(enemy, $skill[Talk About Politics], "lh_banishes");
	...
	if(lh_have_skill($skill[Curse Of Vacation]) && (my_mp() >= mp_cost($skill[Curse Of Vacation])))
	{
		if((enemy == $monster[pygmy orderlies]) && (my_location() == $location[The Hidden Bowling Alley]))
		{
			set_property("lh_combatHandler", combatState + "(curse of vacation)");
			handleTracker(enemy, $skill[Curse Of Vacation], "lh_banishes");
			return "skill " + $skill[Curse Of Vacation];
		}
		if((enemy == $monster[fallen archfiend]) && (my_location() == $location[The Dark Heart of the Woods]) && (get_property("lh_pirateoutfit") != "almost") && (get_property("lh_pirateoutfit") != "finished"))
		{
			set_property("lh_combatHandler", combatState + "(curse of vacation)");
			handleTracker(enemy, $skill[Curse Of Vacation], "lh_banishes");
			return "skill " + $skill[Curse Of Vacation];
	...
	if(item_amount($item[Disposable Instant Camera]) > 0)
	{
		if($monsters[Bob Racecar, Racecar Bob] contains enemy)
		{
			set_property("lh_combatHandler", combatState + "(disposable instant camera)");
	...
	if(!contains_text(edCombatState, "lashofthecobra") && lh_have_skill($skill[Lash of the Cobra]) && (my_mp() >= 12))
	{
		set_property("lh_edCombatHandler", edCombatState + "(lashofthecobra)");
		...
		if(((my_location() == $location[Hippy Camp]) || (my_location() == $location[Wartime Hippy Camp])) && contains_text(enemy, "hippy"))
		{
			...
			{
				if(get_property("lh_edStatus") != "dying")
				{
					doLash = true;
				}
			}
		...
		{
			if((get_property("lh_pirateoutfit") != "finished") && (get_property("lh_pirateoutfit") != "almost") && (item_amount($item[Hot Wing]) < 3))
			{
				doLash = true;
			}
		}
	...
	if((item_amount($item[Tattered Scrap of Paper]) > 0) && !contains_text(combatState, "tatters"))
	{
		...
		{
			set_property("lh_combatHandler", combatState + "(tatters)");
			return "item " + $item[Tattered Scrap Of Paper];
		}
	}
	...
	if(!contains_text(edCombatState, "talismanofrenenutet") && (item_amount($item[Talisman of Renenutet]) > 0))
	{
		...
		if(doRenenutet)
		{
			if(!contains_text(edCombatState, "curseofindecision") && lh_have_skill($skill[Curse Of Indecision]) && (my_mp() > mp_cost($skill[Curse Of Indecision])))
			{
				set_property("lh_edCombatHandler", edCombatState + "(curseofindecision)");
				return "skill " + $skill[Curse Of Indecision];
			}
			set_property("lh_edCombatHandler", edCombatState + "(talismanofrenenutet)");
			handleTracker(enemy, "lh_renenutet");
			set_property("lh_edStatus", "dying");
	...
	if(get_property("lh_edStatus") == "UNDYING!")
	{
		if(my_location() == $location[The Secret Government Laboratory])
		{
			if((item_amount($item[Rock Band Flyers]) == 0) && (item_amount($item[Jam Band Flyers]) == 0))
			{
				...
				{
					set_property("lh_combatHandler", combatState + "(love stinkbug2)");
	...
	if((my_mp() >= mp_cost($skill[Roar of the Lion])) && (my_location() == $location[The Secret Government Laboratory]) && lh_have_skill($skill[Roar of the Lion]))
	{
		if(lh_have_skill($skill[Storm Of The Scarab]) && (my_buffedstat($stat[Mysticality]) >= 60))
		{
			return "skill " + $skill[Storm Of The Scarab];
		}
		return "skill " + $skill[Roar Of The Lion];
	}
	if((my_mp() >= mp_cost($skill[Storm Of The Scarab])) && ($locations[Pirates of the Garbage Barges, The SMOOCH Army HQ, VYKEA] contains my_location()) && lh_have_skill($skill[Storm of the Scarab]))
	{
		return "skill " + $skill[Storm Of The Scarab];
	}
	if((my_mp() >= mp_cost($skill[Fist Of The Mummy])) && (my_location() == $location[Hippy Camp]) && lh_have_skill($skill[Fist Of The Mummy]))
	{
		return "skill " + $skill[Fist Of The Mummy];
	}

	int fightStat = my_buffedstat(weapon_type(equipped_item($slot[weapon]))) - 20;
	if((fightStat > monster_defense()) && (round < 20) && canSurvive(1.1) && (get_property("lh_edStatus") == "UNDYING!"))
	{
		return "attack with weapon";
	}

	if(!contains_text(combatState, "cowboy kick") && lh_have_skill($skill[Cowboy Kick]))
	{
		set_property("lh_combatHandler", combatState + "(cowboy kick)");
	...
	if(my_mp() >= mp_cost($skill[Storm Of The Scarab]) && lh_have_skill($skill[Storm Of The Scarab]) && my_buffedstat($stat[Mysticality]) > 35)
	{
		return "skill " + $skill[Storm Of The Scarab];
	}
	...
	{
		if(my_mp() >= mp_cost($skill[Fist Of The Mummy]) && lh_have_skill($skill[Fist Of The Mummy]))
		{
			return "skill " + $skill[Fist Of The Mummy];
		}
	}
	...
	if((fightStat > monster_defense()) && (round < 20) && canSurvive(1.1) && (get_property("lh_edStatus") == "dying"))
	{
...
monster ocrs_helper(string page)
{
	...
	*/
	...
	{
		if((!contains_text(combatState, "cleesh")) && lh_have_skill($skill[cleesh]) && (my_mp() > 10))
		{
			set_property("lh_useCleesh", false);
			set_property("lh_combatHandler", combatState + "(cleesh)");
		}
	}
	...
	if(last_monster().random_modifiers["unstoppable"])
	{
		...
		{
			set_property("lh_combatHandler", combatState + "(DNA)(air dirty laundry)(ply reality)(indigo cup)(love mosquito)(blue balls)(love gnats)(unstoppable)(micrometeorite)");
	...
	if(last_monster().random_modifiers["annoying"])
	{
		if(contains_text(page, "makes the most annoying noise you've ever heard, stopping you in your tracks."))
		{
			...
			set_property("lh_combatHandler", get_property("lh_funCombatHandler"));
		}
		set_property("lh_funCombatHandler", get_property("lh_combatHandler"));
	}
	...
	if(last_monster().random_modifiers["ticking"])
	{
		if((!contains_text(combatState, "cleesh")) && lh_have_skill($skill[cleesh]) && (my_mp() > 10))
		{
			set_property("lh_useCleesh", true);
		}
	}
...
void awol_helper(string page)
{
	...
	if((!contains_text(combatState, "extractSnakeOil")) && (get_property("lh_noSnakeOil").to_int() == my_daycount()))
	{
		set_property("lh_combatHandler", combatState + "(extractSnakeOil)");
	}
}
...
boolean registerCombat(string action)
{
	set_property("lh_combatHandler", get_property("lh_combatHandler") + "(" + to_lower_case(action) + ")");
	return true;
}
boolean containsCombat(string action)
{
	action = "(" + to_lower_case(action) + ")";
	return contains_text(get_property("lh_combatHandler"), action);
}
