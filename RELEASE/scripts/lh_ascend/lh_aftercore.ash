script "lh_aftercore.ash"
import <lh_ascend.ash>

#Prototypes
boolean lh_sloppySecondsDiner();
boolean lh_fingernail();
boolean lh_packOfSmokes();
boolean lh_goreBucket();
boolean lh_junglePuns();
boolean lh_jungleSandwich();
boolean lh_toxicMascot();
boolean lh_nastyBears();
boolean lh_toxicGlobules();
boolean lh_sexismReduction();
boolean lh_racismReduction();
boolean lh_lubeBarfMountain();
boolean lh_trashNet();
boolean lh_doWalford();
boolean lh_mtMolehill();
boolean lh_acquireKeycards();
boolean lh_dailyDungeon();
item lh_guildEpicWeapon();
boolean lh_guildUnlock();
boolean lh_guildClown();
boolean lh_nemesisCave();
boolean lh_nemesisIsland();
boolean lh_cheesePostCS();
boolean lh_cheesePostCS(int leave);
boolean lh_cheeseAftercore(int leave);
boolean lh_aftercore();
boolean lh_aftercore(int leave);
boolean lh_ascendIntoCS();
boolean lh_ascendIntoCS(class cl);
boolean lh_ascendIntoBond();
boolean lh_doCS();
boolean lh_customMafiaAddress();
...
boolean lh_fingernail()
{
	...
	{
		lhAdv(1, $location[The Mansion of Dr. Weirdeaux]);
	}
	return true;
}
...
boolean lh_goreBucket()
{
	...
	{
		if(!lhAdv(1, $location[The Secret Government Laboratory]))
		{
...
boolean lh_toxicGlobules()
{
	...
	{
		lhAdv(1, $location[The Toxic Teacups]);
	}
	return true;
}
...
boolean lh_lubeBarfMountain()
{
	...
	while(my_adventures() > 0)
	{
		...
		lhAdv(1, $location[Barf Mountain]);
	}
	return true;
}
...
boolean lh_doWalford()
{
	...
	if(get_property("lh_dinseyGarbageMoney").to_int() < my_daycount())
	{
		set_property("lh_dinseyGarbageMoney", my_daycount());
	...
	{
		if(!lhAdv(1, $location[The Ice Hole]))
		{
...
boolean lh_guildClown()
{
	...
	{
		lhAdv(1, $location[The \"Fun\" House]);
	}

	item toMake = lh_guildEpicWeapon();
	if(toMake == $item[none])
	{
...
boolean lh_nemesisCave()
{
	...
	if(!possessEquipment(lh_guildEpicWeapon()))
	{
		return false;
	}
	equipBaseline();
...
#		if(!lhAdv(1, $location[Nemesis Cave]))
#		{
			abort("Can not access area, aborting.");
#		}
	}
	equip(lh_guildEpicWeapon());
...
boolean lh_ascendIntoBond()
{
	...
	{
		if(!(lh_get_campground() contains $item[Asdon Martin Keyfob]))
		{
...
boolean lh_ascendIntoCS()
{
	return lh_ascendIntoCS($class[Sauceror]);
}
...
boolean lh_aftercore()
{
	return lh_cheeseAftercore(0);
}
boolean lh_aftercore(int leave)
{
	return lh_cheeseAftercore(leave);
}

boolean lh_cheeseAftercore(int leave)
{
	return lh_cheesePostCS(leave);
}

boolean lh_cheesePostCS()
{
	return lh_cheesePostCS(0);
}
...
boolean lh_cheesePostCS(int leave)
{
	...
	if(((my_daycount() == 2) && isOverdueDigitize()) || get_property("_lh_specialAftercore").to_boolean())
	{
		cli_execute("/aa none");
		set_property("_lh_specialAftercore", false);
		if(my_fullness() == 10)
		{
		...
		lh_sourceTerminalEducate($skill[Digitize], $skill[Turbo]);
		...
		if((get_property("chateauMonster") == $monster[Cheerless Mime Executive]) && !get_property("_chateauMonsterFought").to_boolean() && chateaumantegna_available())
		{
			if(get_property("_badlyRomanticArrows").to_int() == 0)
			{
				...
				set_property("lh_combatDirective", "start;skill curse of weaksauce;skill digitize; skill wink at; item rain-doh black box, spooky putty sheet;skill stuffed mortar shell;item rain-doh indigo cup, rain-doh blue balls; skill saucestorm; skill saucestorm; skill saucestorm; skill saucestorm; skill saucestorm");
			}
			else
			{
				set_property("lh_combatDirective", "start;skill curse of weaksauce;skill digitize; item rain-doh black box, spooky putty sheet;skill stuffed mortar shell;item rain-doh indigo cup, rain-doh blue balls; skill saucestorm; skill saucestorm; skill saucestorm; skill saucestorm; skill saucestorm; skill saucestorm");
			}
...
#			}
		...
		{
			set_property("lh_combatDirective", "start;skill curse of weaksauce;item rain-doh black box, time-spinner;skill stuffed mortar shell;item rain-doh indigo cup, rain-doh blue balls");
		...
		if((get_property("lh_digitizePlanet") != "") && (get_property("_spacegateCoordinates") == ""))
		{
			string temp = visit_url("place.php?whichplace=spacegate&action=sg_Terminal");
			temp = visit_url("choice.php?pwd=&whichchoice=1235&option=2&word=" + get_property("lh_digitizePlanet"));
			lhAdv($location[Through The Spacegate]);
		...
		while(get_property("_spacegateTurnsLeft").to_int() > 0)
		{
			...
			if(!lhAdv($location[Through The Spacegate]))
			{
				string temp = visit_url("place.php?whichplace=spacegate&action=sg_Terminal");
				break;
			}
		...
		if(available_amount($item[Kremlin\'s Greatest Briefcase]) > 0)
		{
			...
			if(get_property("gingerbreadCityAvailable").to_boolean() && (get_property("_gingerbreadCityTurns").to_int() < 5))
			{
				if(!get_property("_gingerbreadClockAdvanced").to_boolean())
				{
					...
					lhAdv($location[Gingerbread Civic Center]);
					set_property("choiceAdventure1215", old);
				}
				while(get_property("_gingerbreadCityTurns").to_int() < 4)
				{
					set_property("lh_combatDirective", "start;skill KGB tranquilizer dart");
					lhAdv($location[Gingerbread Upscale Retail District]);
				}
				if(get_property("_gingerbreadCityTurns").to_int() == 4)
				{
					...
					lhAdv($location[Gingerbread Train Station]);
...
//		loc = $location[An Eldritch Fissure];
	...
	if(!get_property("_didMining").to_boolean() && (my_daycount() == 2) && have_skill($skill[Unaccompanied Miner]))
	{
		...
		{
			lhMaximize("hot res, equip high-temperature mining drill, -equip snow suit", 0, 0, false);
	...
	boolean oldGarbage = get_property("lh_getDinseyGarbageMoney").to_boolean();
	set_property("lh_getDinseyGarbageMoney", true);
	dinseylandfill_garbageMoney();
	set_property("lh_getDinseyGarbageMoney", oldGarbage);
	...
	if((item_amount($item[Confusing LED Clock]) > 0) && hippy_stone_broken() && (my_adventures() >= 6) && !get_property("_confusingLEDClockUsed").to_boolean())
	{
		...
		if(lh_get_campground() contains $item[Confusing LED Clock])
		{
...
#		}
	...
	while((inebriety_left() >= 5) && get_property("lh_breakstone").to_boolean())
	{
		...
		lhDrink(1, $item[5-Hour Acrimony]);
	}


	while((inebriety_left() >= 2) && get_property("lh_breakstone").to_boolean())
	{
	...
	if(hippy_stone_broken())
	{
		...
		{
			lhOverdrink(1, $item[5-hour acrimony]);
		}
		if(get_property("lh_pvpOutfit") != "")
		{
			cli_execute("pull outfit pvp");
			cli_execute("pull kiwi beak");
			cli_execute("pull jam band");
			cli_execute("pull pail");
			cli_execute("/outfit " + get_property("lh_pvpOutfit"));
			cli_execute(get_property("lh_pvpCommands"));
		}
		int stance = 6;
		if(get_property("lh_pvpStance") != "")
		{
			stance = get_property("lh_pvpStance").to_int();
		}
