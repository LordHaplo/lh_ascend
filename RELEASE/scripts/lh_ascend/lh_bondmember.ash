...
void bond_initializeSettings()
{
	...
	if(get_property("lh_dickstab").to_boolean())
	{
		...
		if(!get_property("_pottedTeaTreeUsed").to_boolean() && (lh_get_campground() contains $item[Potted Tea Tree]))
		{
...
boolean bond_initializeDay(int day)
{
	...
	if(day == 2)
	{
		if(get_property("lh_dickstab").to_boolean())
		{
			...
			if(!get_property("_lh_kgbSetup").to_boolean())
			{
				set_property("_lh_kgbSetup", true);
				kgb_getMartini();
			}
			int have = item_amount($item[Improved Martini]) + item_amount($item[Splendid Martini]);
			...
			if(get_property("lh_dickstab").to_boolean() && !possessEquipment($item[Dented Scepter]) && is_unrestricted($item[Witchess Set]) && get_property("lovebugsUnlocked").to_boolean() && possessEquipment($item[Your Cowboy Boots]) && have_skills($skills[Curse of Weaksauce, Shell Up, Lunging Thrust-Smack, Sauceshell, Itchy Curse Finger]) && is_unrestricted($item[Source Terminal]) && (lh_get_campground() contains $item[Witchess Set]) && (lh_get_campground() contains $item[Source Terminal]) && (get_property("_witchessFights").to_int() < 5))
			{
...
boolean LM_bond()
{
	...
	if((get_property("_lh_bondBriefing") == "started") && (get_property("_villainLairProgress").to_int() >= 999))
	{
		set_property("_lh_bondBriefing", "finished");
	}

	if(get_property("_lh_bondBriefing") == "started")
	{
		if(get_property("lh_dickstab").to_boolean() && !possessEquipment($item[Dented Scepter]) && is_unrestricted($item[Witchess Set]) && get_property("lovebugsUnlocked").to_boolean() && possessEquipment($item[Your Cowboy Boots]) && have_skills($skills[Curse of Weaksauce, Shell Up, Lunging Thrust-Smack, Sauceshell, Itchy Curse Finger]) && is_unrestricted($item[Source Terminal]) && (lh_get_campground() contains $item[Witchess Set]) && (lh_get_campground() contains $item[Source Terminal]) && (get_property("_witchessFights").to_int() < 5))
		{
			lh_sourceTerminalEducate($skill[Turbo], $skill[Compress]);
		...
		boolean retval = lhAdv($location[Super Villain\'s Lair]);
		if(!retval)
		{
			set_property("_lh_bondBriefing", "finished");
			bond_buySkills();
	...
	if(get_property("_lh_bondLevel").to_int() < my_level())
	{
		set_property("_lh_bondLevel", my_level());
		bond_buySkills();
	}
	...
	if(get_property("lh_dickstab").to_boolean())
	{
		...
		if((get_property("_lh_bondBriefing") == "finished") && get_property("gingerbreadCityAvailable").to_boolean() && (get_property("_gingerbreadCityTurns").to_int() < 5))
		{
			...
			{
				return lhAdv($location[Gingerbread Upscale Retail District]);
			}
			if(get_property("_gingerbreadCityTurns").to_int() == 4)
			{
		...
		if(get_property("_lh_bondBriefing") == "finished")
		{
			...
			else if((my_mp() > 60) && (my_level() > 8) && (my_adventures() < 10))
			{
				if((inebriety_left() < 3) && (inebriety_left() >= 1))
				{
					...
					{
						lhDrink(inebriety_left(), $item[Splendid Martini]);
					}
					else
					{
						lhDrink(inebriety_left(), $item[Improved Martini]);
					}
				}
			}
			else if((my_mp() > 60) && (my_level() > 6) && (my_adventures() < 10))
			{
				if(inebriety_left() >= 10)
				{
					...
					{
						lhDrink(10, $item[Splendid Martini]);
					}
					else
					{
						lhDrink(10, $item[Improved Martini]);
					}
				}
				else if(inebriety_left() >= 3)
				{
					...
					{
						lhDrink(3, $item[Splendid Martini]);
					}
					else
					{
						lhDrink(3, $item[Improved Martini]);
					}
				}
			}
		}
		...
		if((internalQuestStatus("questL08Trapper") < 2) && (my_level() >= 8))
		{
			...
			{
				L8_trapperStart();
				set_property("lh_combatDirective", "start;(olfaction)");
		...
		if(get_property("questM20Necklace") == "finished")
		{
			if(get_property("_sourceTerminalDigitizeMonster") == $monster[Writing Desk])
			{
				if((get_property("_sourceTerminalDigitizeUses").to_int() == 1) && (item_amount($item[White Pixel]) < 30) && (item_amount($item[Richard\'s Star Key]) == 0) && !contains_text(get_property("nsTowerDoorKeysUsed"), $item[Richard\'s Star Key]))
				{
					...
					lh_sourceTerminalEducate($skill[Extract], $skill[Digitize]);
					set_property("lh_digitizeDirective", $monster[Blooper]);
		...
		if((internalQuestStatus("questL12War") >= 1) && (get_property("sidequestOrchardCompleted") == "none"))
		{
			if((item_amount($item[Filthworm Royal Guard Scent Gland]) == 0) && (item_amount($item[Heart Of The Filthworm Queen]) == 0) && canYellowRay())
			{
				...
				set_property("lh_combatDirective", "start;" + yellowRayCombatString());
				boolean retval = lhAdv(loc);
				set_property("lh_combatDirective", "");
				return retval;
			}
		}
		...
		if(my_level() >= 10)
		{
			...
			if(!get_property("lh_getStarKey").to_boolean())
			{
				if((item_amount($item[Richard\'s Star Key]) == 0) && !contains_text(get_property("nsTowerDoorKeysUsed"), $item[Richard\'s Star Key]))
				{
					...
					{
						return lhAdv($location[The Hole In The Sky]);
					}
				}
			}
		}
		...
		{
			if((get_property("lh_swordfish") != "finished") && (item_amount($item[Disposable Instant Camera]) == 0))
			{
				if(contains_text($location[The Haunted Bedroom], $monster[Animated Ornate Nightstand]))
				{
					...
					{
						lhAdv($location[The Haunted Bedroom]);
					}
