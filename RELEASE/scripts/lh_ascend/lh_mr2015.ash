...
boolean lh_barrelPrayers();
boolean lh_mayoItems();
...
boolean mayo_acquireMayo(item it)
{
	...
	if(!(lh_get_campground() contains $item[Portable Mayo Clinic]))
	{
		return false;
	}
	return true;
...
boolean chateaumantegna_usePainting(string option)
{
	...
	if(chateaumantegna_available())
	{
		return lhAdvBypass("place.php?whichplace=chateau&action=chateau_painting", $location[Noob Cave], option);
	}
	return false;
}
...
boolean deck_useScheme(string action)
{
	...
	else if(action == "turns")
	{
		...
		if((get_property("lh_trapper") == "") || (get_property("lh_trapper") == "start"))
		{
...
#	}
	...
	{
		lh_autosell(min(1, item_amount($item[1952 Mickey Mantle Card])), $item[1952 Mickey Mantle Card]);
	}
