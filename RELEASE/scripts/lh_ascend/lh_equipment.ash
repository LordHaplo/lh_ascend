script "lh_ascend/lh_equipment.ash";
...
{
	return "_lh_maximize_equip_" + s.to_string();
}
...
boolean lhEquip(slot s, item it)
{
	...
	lh_debug_print("Equipping " + it + " to slot " + s, "gold");
...
boolean lhEquip(item it)
{
	return lhEquip(it.to_slot(), it);
}
...
boolean lhForceEquip(slot s, item it)
{
	if(!possessEquipment(it) || !lh_can_equip(it))
	{
		return false;
...
boolean lhForceEquip(item it)
{
	return lhForceEquip(it.to_slot(), it);
}

boolean lhOutfit(string toWear)
{
	...
	if(useMaximizeToEquip())
	{
		...
		lh_debug_print('Adding outfit "' + toWear + '" to maximizer statement', "gold");
		boolean pass = true;
		foreach i,it in outfit_pieces(toWear)
		{
			pass = pass && lhEquip(it);
		}
		return pass;
	}
	else
...
boolean tryAddItemToMaximize(slot s, item it)
{
	...
	{
		lh_debug_print("But " + s + " is an invalid equip slot... What?", "red");
		return false;
	}
	switch(s)
	{
...
{
	return get_property("lh_maximize_baseline").to_lower_case() != "disabled";
}
...
string defaultMaximizeStatement()
{
	...
	if(lh_my_path() != "Pocket Familiars")
	{
		...
		if(lh_have_familiar($familiar[mosquito]))
		{
...
void resetMaximize()
{
	string res = get_property("lh_maximize_baseline");
	...
	set_property("lh_maximize_current", res);
	lh_debug_print("Resetting lh_maximize_current to " + res, "gold");
...
{
	lh_debug_print('Adding "' + add + '" to current maximizer statement', "gold");
	string res = get_property("lh_maximize_current");
...
{
	lh_debug_print('Removing "' + rem + '" from current maximizer statement', "gold");
	string res = get_property("lh_maximize_current");
...
{
	return get_property("lh_maximize_current").contains_text(check);
}

boolean simMaximize()
{
	string backup = get_property("lh_maximize_current");
	finalizeMaximize();
	boolean res = lhMaximize(get_property("lh_maximize_current"), true);
	set_property("lh_maximize_current", backup);
	return res;
}
...
{
	addToMaximize(add);
	lh_debug_print("Simulating: " + get_property("lh_maximize_current"), "gold");
...
void equipMaximizedGear()
{
	...
	finalizeMaximize();
	print("Maximizing: " + get_property("lh_maximize_current"), "blue");
	maximize(get_property("lh_maximize_current"), 2500, 0, false);
}
...
void equipOverrides()
{
	...
	{
		string overrides = get_property("lh_equipment_override_" + slot_str);
		if(overrides == "")
		{
			continue;
		}
		...
		foreach i,item_str in overrides_split
		{
			...
			if(it == $item[none])
			{
				print('"' + item_str + '" does not properly convert to an item (found in lh_equipment_override_' + slot_str + ')', "red");
				continue;
			}
			if(lhEquip(s, it))
			{
...
void equipBaselineGear()
{
	...
	if(!file_to_map("lh_ascend_equipment.txt", equipment_text))
		print("Could not load lh_ascend_equipment.txt. This is bad!", "red");
	...
	boolean considerGearOption(string item_str, string slot_str, string conds)
	{
		...
		if(!lh_can_equip(it, eq_slot))
			return false;
		// also we need to have it for it to be equippable, obviously
		if(equipmentAmount(it) == 0)
			return false;

		string ignore = get_property("lh_ignoreCombat");
		if(get_property("lh_beatenUpCount").to_int() >= 7)
			ignore += "(ml)";
		if(ignore != "")
		{
	...
	foreach slot_str in $strings[hat, back, shirt, weapon, off-hand, pants, acc]
	{
		string override = get_property("lh_equipment_override_" + slot_str);
		if(override == "")
			continue;
...
void makeStartingSmiths()
{
	...
	case $class[Seal Clubber]:
		...
		{
			lhCraft("smith", 1, $item[lump of Brituminous coal], $item[seal-clubbing club]);
		}
	...
	case $class[Turtle Tamer]:
		if(!possessEquipment($item[Work is a Four Letter Sword]))
		{
			...
			lhCraft("smith", 1, $item[lump of Brituminous coal], $item[sword hilt]);
		}
	...
	case $class[Sauceror]:
		...
		{
			lhCraft("smith", 1, $item[lump of Brituminous coal], $item[Saucepan]);
		}
	...
	case $class[Pastamancer]:
		...
		{
			lhCraft("smith", 1, $item[lump of Brituminous coal], $item[Pasta Spoon]);
		}
		break;
	case $class[Disco Bandit]:
		if(!possessEquipment($item[Frankly Mr. Shank]))
		{
			lhCraft("smith", 1, $item[lump of Brituminous coal], $item[Disco Ball]);
		}
		break;
	case $class[Accordion Thief]:
		if(!possessEquipment($item[Shakespeare\'s Sister\'s Accordion]))
		{
			lhCraft("smith", 1, $item[lump of Brituminous coal], $item[Stolen Accordion]);
		}
		break;
	}
	...
	{
		lhCraft("smith", 1, $item[lump of Brituminous coal], $item[maiden wig]);
	}
...
void equipBaseline()
{
	...
	if(get_property("lh_diceMode").to_boolean())
	{
		lhEquip($slot[acc1], $item[Dice Ring]);
		lhEquip($slot[acc2], $item[Dice Belt Buckle]);
		lhEquip($slot[acc3], $item[Dice Sunglasses]);
		lhEquip($slot[hat], $item[Dice-Print Do-Rag]);
		lhEquip($slot[back], $item[Dice-Shaped Backpack]);
		lhEquip($slot[pants], $item[Dice-Print Pajama Pants]);
		lhEquip($slot[familiar], $item[Kill Screen]);
	}
}
...
void ensureSealClubs()
{
	...
	{
		string ignore = get_property("lh_ignoreCombat");
		set_property("lh_ignoreCombat", ignore + "(seal)");
		equipBaseline();
		set_property("lh_ignoreCombat", ignore);
	}
}
...
void removeNonCombat()
{
	...
	{
		string ignore = get_property("lh_ignoreCombat");
		set_property("lh_ignoreCombat", ignore + "(noncombat)");
		equipBaseline();
		set_property("lh_ignoreCombat", ignore);
	}
}
...
void removeCombat()
{
	...
	{
		string ignore = get_property("lh_ignoreCombat");
		set_property("lh_ignoreCombat", ignore + "(combat)");
		equipBaseline();
		set_property("lh_ignoreCombat", ignore);
	}
}
...
void equipRollover()
{
	...
	if(lh_have_familiar($familiar[Trick-or-Treating Tot]) && !possessEquipment($item[Li'l Unicorn Costume]) && (my_meat() > 3000 + npc_price($item[Li'l Unicorn Costume])) && lh_is_valid($item[Li'l Unicorn Costume]) && lh_my_path() != "Pocket Familiars")
	{
