...
boolean januaryToteAcquire(item it)
{
	...
	if(choice == 2)
	{
		if((lh_my_path() == "Way of the Surprising Fist") || (my_class() == $class[Avatar Of Boris]))
		{
			return false;
		}
	}
...
boolean godLobsterCombat(item it, int goal, string option)
{
	...
	{
		lhAdv(1, $location[Noob Cave], option);
...
boolean fantasyRealmToken()
{
	...
	if(is100familiarRun() && lh_have_familiar($familiar[Mosquito]))
	{
		return false;
	}
	set_property("lh_familiarChoice", "none");
	use_familiar($familiar[none]);

	if(possessEquipment($item[FantasyRealm G. E. M.]))
	{
		lhEquip($slot[acc3], $item[FantasyRealm G. E. M.]);
	}

	//This does not appear to check that we no longer need to adventure there...

	lhAdv(1, $location[The Bandit Crossroads]);
	return true;
}
...
{
	if (!have_familiar($familiar[Cat Burglar]) || !lh_is_valid($familiar[Cat Burglar]))
	{
		return 0;
	}
...
boolean neverendingPartyAvailable()
{
	...
	if(get_property("lh_skipNEPOverride").to_boolean())
	{
		print("NEP access disabled. This can be turned on in the Relay by setting lh_skipNEPOverride = false", "red");
		return false;
	}
...
boolean neverendingPartyCombat(effect eff, boolean hardmode, string option, boolean powerlevelling)
{
	...
	if(get_property("choiceAdventure1324").to_int() == 5)
	{
		...
		retval = lhAdvBypass(0, pages, $location[The Neverending Party], option);
	}
	else
	{
		retval = lhAdv(1, $location[The Neverending Party], option);
	}
...
string lh_latteDropName(location l)
{
	switch(l)
	{
...
boolean lh_latteDropAvailable(location l)
{
	// obviously no latte drops are available if you don't HAVE a latte
	if(available_amount($item[latte lovers member's mug]) == 0)
		return false;
	string latteDrop = lh_latteDropName(l);
	if(latteDrop == "")
		return false;
	return !get_property("latteUnlocks").contains_text(latteDrop);
}

boolean lh_latteDropWanted(location l)
{
	return lh_latteDropAvailable(l) && !($locations[Noob Cave, The Haunted Boiler Room, The Arid\, Extra-Dry Desert] contains l);
}

string lh_latteTranslate(string ingredient)
{
...
boolean lh_latteRefill(string want1, string want2, string want3, boolean force)
{
	if(available_amount($item[latte lovers member's mug]) == 0)
		return false;
	...
	want1 = lh_latteTranslate(want1);
	want2 = lh_latteTranslate(want2);
	want3 = lh_latteTranslate(want3);
...
boolean lh_latteRefill(string want1, string want2, string want3)
{
	return lh_latteRefill(want1, want2, want3, false);
}

boolean lh_latteRefill(string want1, string want2, boolean force)
{
	return lh_latteRefill(want1, want2, "", force);
}

boolean lh_latteRefill(string want1, string want2)
{
	return lh_latteRefill(want1, want2, false);
}

boolean lh_latteRefill(string want1, boolean force)
{
	return lh_latteRefill(want1, "", force);
}

boolean lh_latteRefill(string want1)
{
	return lh_latteRefill(want1, false);
}

boolean lh_latteRefill()
{
	return lh_latteRefill("");
}

boolean lh_voteSetup()
{
	return lh_voteSetup(0,0,0);
}

boolean lh_voteSetup(int candidate)
{
	return lh_voteSetup(candidate,0,0);
}

boolean lh_voteSetup(int candidate, int first, int second)
{
...
boolean lh_voteMonster()
{
	return lh_voteMonster(false);
}

boolean lh_voteMonster(boolean freeMon)
{
	return lh_voteMonster(freeMon, $location[none], "");
}

boolean lh_voteMonster(boolean freeMon, location loc)
{
	return lh_voteMonster(freeMon, loc, "");
}

boolean lh_voteMonster(boolean freeMon, location loc, string option)
{
	if(!get_property("_voteToday").to_boolean() && !get_property("voteAlways").to_boolean())
	{
		return false;
