...
boolean lhMaximize(string req, boolean simulate);
boolean lhMaximize(string req, int maxPrice, int priceLevel, boolean simulate);
aggregate lhMaximize(string req, int maxPrice, int priceLevel, boolean simulate, boolean includeEquip);
...
int solveCookie();
boolean use_barrels();
int lhCraft(string mode, int count, item item1, item item2);
int[item] lh_get_campground();
int towerKeyCount();
...
boolean in_ronin();
boolean lh_autosell(int quantity, item toSell);
...
boolean lh_is_valid(item it);
boolean lh_is_valid(familiar fam);
boolean lh_is_valid(skill sk);
boolean lh_debug_print(string s, string color);
boolean lh_debug_print(string s);
boolean lh_can_equip(item it);
boolean lh_can_equip(item it, slot s);
boolean lh_badassBelt();
...
#				}
	...
	if(get_property("lh_shareMaximizer").to_boolean() && get_property("lh_allowSharingData").to_boolean())
	{
...
boolean backupSetting(string setting, string newValue)
{
	...
	if(found == 1)
	{
		...
		if(get_property("lh_backup_" + setting) == "")
		{
			set_property("lh_backup_" + setting, oldValue);
		}
...
boolean restoreSetting(string setting)
{
	if(get_property("lh_backup_" + setting) != "")
	{
		if(get_property("lh_backup_" + setting) == "__BLANK__")
		{
			set_property(setting, "");
		}
		else
		{
			set_property(setting, get_property("lh_backup_" + setting));
		}
		remove_property("lh_backup_" + setting);
		return true;
	}
...
{
	boolean boo = loopHandlerDelay("_lh_lastABooCycleFix");
	boolean desk = loopHandlerDelay("_lh_digitizeDeskCounter");
	boolean digitize = loopHandlerDelay("_lh_digitizeAssassinCounter");
...
boolean is100FamiliarRun()
{
	...
	if(get_property("lh_100familiar") == $familiar[Egg Benedict])
	{
	...
	if(get_property("lh_100familiar") == $familiar[none])
	{
		return false;
	}
	if(get_property("lh_100familiar") == "")
	{
		return false;
	}
	return true;
...
boolean is100FamiliarRun(familiar thisOne)
{
	if(is100FamiliarRun())
	{
		if(get_property("lh_100familiar") == thisOne)
		{
			return false;
		}
		return true;
...
int solveCookie()
{
	...
	while(i < 250)
	{
		...
		{
			set_property("lh_cookie", my_turncount() + i);
			break;
		}
		i = i + 1;
	}

	return get_property("lh_cookie").to_int();
}


boolean needOre()
{
	if((get_property("lh_trapper") == "yeti") || (get_property("lh_trapper") == "finished"))
	{
		return false;
	}
...
boolean canYellowRay(monster target)
{
	...
	if((my_familiar() == $familiar[Crimbo Shrub]) || (!is100FamiliarRun($familiar[Crimbo Shrub]) && lh_have_familiar($familiar[Crimbo Shrub])))
	{
	...
	if(!get_property("_internetViralVideoBought").to_boolean() && (item_amount($item[BACON]) >= 20) && lh_is_valid($item[Viral Video]))
	{
...
// private
boolean[string] lh_reallyBanishesUsedAt(location loc)
{
...
boolean[string] lh_banishesUsedAt(location loc)
{
	if($locations[Next To That Barrel With Something Burning In It, Out By That Rusted-Out Car, Over Where The Old Tires Are, Near an Abandoned Refrigerator] contains loc)
	{
		...
		{
			boolean[string] used = lh_reallyBanishesUsedAt(l);
			foreach s in used
			{
	...
	return lh_reallyBanishesUsedAt(loc);
}

boolean lh_wantToBanish(monster enemy, location loc)
{
	location locCache = my_location();
	set_location(loc);
	boolean [monster] monstersToBanish = lh_getMonsters("banish");
...
string banisherCombatString(monster enemy, location loc, boolean inCombat)
{
	...
	boolean[string] used = lh_banishesUsedAt(loc);
	...
	*/
	...
	if((inCombat ? lh_have_skill($skill[Throw Latte on Opponent]) : possessEquipment($item[latte lovers member's mug])) && !get_property("_latteBanishUsed").to_boolean() && !(used contains "Throw Latte on Opponent") && get_property("_lh_maximize_equip_off-hand") != "")
	{
		return "skill " + $skill[Throw Latte on Opponent];
	}

	if((inCombat ? lh_have_skill($skill[Give Your Opponent The Stinkeye]) : possessEquipment($item[stinky cheese eye])) && !get_property("_stinkyCheeseBanisherUsed").to_boolean() && (my_mp() >= mp_cost($skill[Give Your Opponent The Stinkeye])))
	{
		return "skill " + $skill[Give Your Opponent The Stinkeye];
	}

	if((inCombat ? lh_have_skill($skill[Creepy Grin]) : possessEquipment($item[V for Vivala mask])) && !get_property("_vmaskBanisherUsed").to_boolean() && (my_mp() >= mp_cost($skill[Creepy Grin])))
	{
		return "skill " + $skill[Creepy Grin];
	}

	if(lh_have_skill($skill[Baleful Howl]) && my_hp() > hp_cost($skill[Baleful Howl]) && get_property("_balefulHowlUses").to_int() < 10 && !(used contains "baleful howl"))
	{
		loopHandlerDelayAll();
		return "skill " + $skill[Baleful Howl];
	}

	if(lh_have_skill($skill[Thunder Clap]) && (my_thunder() >= thunder_cost($skill[Thunder Clap])) && (!(used contains "thunder clap")))
	{
		return "skill " + $skill[Thunder Clap];
	}
	if(lh_have_skill($skill[Asdon Martin: Spring-Loaded Front Bumper]) && (get_fuel() >= fuel_cost($skill[Asdon Martin: Spring-Loaded Front Bumper])) && (!(used contains "Spring-Loaded Front Bumper")))
	{
	...
	if(lh_have_skill($skill[Curse Of Vacation]) && (my_mp() > mp_cost($skill[Curse Of Vacation])) && (!(used contains "curse of vacation")))
	{
		return "skill " + $skill[Curse Of Vacation];
	}

	if((inCombat ? lh_have_skill($skill[Show Them Your Ring]) : possessEquipment($item[Mafia middle finger ring])) && !get_property("_mafiaMiddleFingerRingUsed").to_boolean() && (my_mp() >= mp_cost($skill[Show Them Your Ring])))
	{
		return "skill " + $skill[Show Them Your Ring];
	}
	if(lh_have_skill($skill[Breathe Out]) && (my_mp() >= mp_cost($skill[Breathe Out])) && (!(used contains "breathe out")))
	{
		return "skill " + $skill[Breathe Out];
	}
	if(lh_have_skill($skill[Batter Up!]) && (my_fury() >= 5) && (inCombat ? (item_type(equipped_item($slot[weapon])) == "club") : true) && (!(used contains "batter up!")))
	{
		return "skill " + $skill[Batter Up!];
	}

	if(lh_have_skill($skill[Banishing Shout]) && (my_mp() > mp_cost($skill[Banishing Shout])) && (!(used contains "banishing shout")))
	{
		return "skill " + $skill[Banishing Shout];
	}
	if(lh_have_skill($skill[Walk Away From Explosion]) && (my_mp() > mp_cost($skill[Walk Away From Explosion])) && (have_effect($effect[Bored With Explosions]) == 0) && (!(used contains "walk away from explosion")))
	{
		return "skill " + $skill[Walk Away From Explosion];
	}

	if((inCombat ? lh_have_skill($skill[Talk About Politics]) : possessEquipment($item[Pantsgiving])) && (get_property("_pantsgivingBanish").to_int() < 5) && have_equipped($item[Pantsgiving]) && (!(used contains "pantsgiving")))
	{
		return "skill " + $skill[Talk About Politics];
	}
	if((inCombat ? lh_have_skill($skill[Reflex Hammer]) : possessEquipment($item[Lil' Doctor&trade; bag])) && get_property("_reflexHammerUsed").to_int() < 3 && !(used contains "Reflex Hammer"))
	{
		return "skill " + $skill[Reflex Hammer];
	}
	if((inCombat ? lh_have_skill($skill[KGB Tranquilizer Dart]) : possessEquipment($item[Kremlin's Greatest Briefcase])) && (get_property("_kgbTranquilizerDartUses").to_int() < 3) && (my_mp() >= mp_cost($skill[KGB Tranquilizer Dart])) && (!(used contains "KGB tranquilizer dart")))
	{
		boolean useIt = true;
		if((get_property("lh_gremlins") == "finished") && (my_daycount() >= 2) && (get_property("_kgbTranquilizerDartUses").to_int() >= 2))
		{
			useIt = false;
		}
	...
	if(lh_have_skill($skill[Snokebomb]) && (get_property("_snokebombUsed").to_int() < 3) && ((my_mp() - 20) >= mp_cost($skill[Snokebomb])) && (!(used contains "snokebomb")))
	{
		return "skill " + $skill[Snokebomb];
	}
	if(lh_have_skill($skill[Beancannon]) && (get_property("_beancannonUses").to_int() < 5) && ((my_mp() - 20) >= mp_cost($skill[Beancannon])) && (!(used contains "beancannon")))
	{
	...
	if(lh_have_skill($skill[Breathe Out]) && (!(used contains "breathe out")))
	{
		return "skill " + $skill[Breathe Out];
	}
...
boolean adjustForBanish(string combat_string)
{
	...
	{
		return lhEquip($item[latte lovers member's mug]);
	}
	if(combat_string == "skill " + $skill[Give Your Opponent The Stinkeye])
	{
		return lhEquip($item[stinky cheese eye]);
	}
	if(combat_string == "skill " + $skill[Creepy Grin])
	{
		return lhEquip($item[V for Vivala mask]);
	}
	if(combat_string == "skill " + $skill[Show Them Your Ring])
	{
		return lhEquip($item[Mafia middle finger ring]);
	}
	...
	{
		return lhEquip($item[Pantsgiving]);
	}
	if(combat_string == "skill " + $skill[Reflex Hammer])
	{
		return lhEquip($item[Lil' Doctor&trade; bag]);
	}
	if(combat_string == "skill " + $skill[KGB Tranquilizer Dart])
	{
		return lhEquip($item[Kremlin's Greatest Briefcase]);
	}
...
string yellowRayCombatString(monster target, boolean inCombat)
{
	...
	{
		if((item_amount($item[Yellowcake Bomb]) > 0) && lh_is_valid($item[Yellowcake Bomb]))
		{
			return "item " + $item[Yellowcake Bomb];
		}
		if(lh_have_skill($skill[Disintegrate]) && (my_mp() >= mp_cost($skill[Disintegrate])))
		{
			return "skill " + $skill[Disintegrate];
		}
		if(lh_have_skill($skill[Ball Lightning]) && (my_lightning() >= lightning_cost($skill[Ball Lightning])))
		{
			return "skill " + $skill[Ball Lightning];
		}
		if(lh_have_skill($skill[Wrath of Ra]) && (my_mp() >= mp_cost($skill[Wrath of Ra])))
		{
			return "skill " + $skill[Wrath of Ra];
		}
		if((item_amount($item[Mayo Lance]) > 0) && (get_property("mayoLevel").to_int() > 0) && lh_is_valid($item[Mayo Lance]))
		{
			return "item " + $item[Mayo Lance];
		}
		if((get_property("peteMotorbikeHeadlight") == "Ultrabright Yellow Bulb") && lh_have_skill($skill[Flash Headlight]) && (my_mp() >= mp_cost($skill[Flash Headlight])))
		{
			return "skill " + $skill[Flash Headlight];
		}
		foreach it in $items[Golden Light, Pumpkin Bomb, Unbearable Light, Viral Video]
		{
			if((item_amount(it) > 0) && lh_is_valid(it))
			{
				return "item " + it;
			}
		}
		if(lh_have_skill($skill[Unleash Cowrruption]) && (have_effect($effect[Cowrruption]) >= 30))
		{
			return "skill " + $skill[Unleash Cowrruption];
		}
		if((inCombat ? my_familiar() == $familiar[Crimbo Shrub] : lh_have_familiar($familiar[Crimbo Shrub])) && (get_property("shrubGifts") == "yellow"))
		{
	...
	if((inCombat ? have_equipped($item[Fourth of May cosplay saber]) : possessEquipment($item[Fourth of May cosplay saber])) && (lh_saberChargesAvailable() > 0))
	{
		// can't use the force on uncopyable monsters
		if(target == $monster[none] || target.copyable)
		{
			return lh_combatSaberYR();
		}
	}
...
boolean adjustForYellowRay(string combat_string)
{
	...
	{
		return lhEquip($slot[weapon], $item[Fourth of May cosplay saber]);
	}
	return true;
}
...
boolean uneffect(effect toRemove)
{
	...
	if(($effects[Driving Intimidatingly, Driving Obnoxiously, Driving Observantly, Driving Quickly, Driving Recklessly, Driving Safely, Driving Stealthily, Driving Wastefully, Driving Waterproofly] contains toRemove) && (lh_get_campground() contains $item[Asdon Martin Keyfob]))
	{
...
skill preferredLibram()
{
	if(lh_have_skill($skill[Summon Brickos]) && (get_property("_brickoEyeSummons").to_int() < 3))
	{
		return $skill[Summon Brickos];
	}
	if(lh_have_skill($skill[Summon Party Favor]) && (get_property("_favorRareSummons").to_int() < 3))
	{
		return $skill[Summon Party Favor];
	}
	if(lh_have_skill($skill[Summon Resolutions]))
	{
		return $skill[Summon Resolutions];
	}
	if(lh_have_skill($skill[Summon Taffy]))
	{
...
void tootGetMeat()
{
	lh_autosell(min(5, item_amount($item[hamethyst])), $item[hamethyst]);
	lh_autosell(min(5, item_amount($item[baconstone])), $item[baconstone]);
	lh_autosell(min(5, item_amount($item[porquoise])), $item[porquoise]);
}


boolean ovenHandle()
{
	if((lh_get_campground() contains $item[Dramatic&trade; range]) && !get_property("lh_haveoven").to_boolean())
	{
		if((lh_get_campground() contains $item[Certificate of Participation]) && (my_class() == $class[Ed]))
		{
	...
	if(!get_property("lh_haveoven").to_boolean() && (my_meat() > 4000) && isGeneralStoreAvailable())
	{
		buyUpTo(1, $item[Dramatic&trade; range]);
		use(1, $item[Dramatic&trade; range]);
		set_property("lh_haveoven", true);
	}
	return get_property("lh_haveoven").to_boolean();
}
...
boolean acquireMP(int goal, boolean buyIt)
{
	...
	{
		boolean gLoverBlock = (lh_my_path() == "G-Lover");
		item goal = $item[none];
...
boolean isHermitAvailable()
{
	...
	if(lh_my_path() == "Zombie Master")
	{
		return false;
	}
	if(lh_my_path() == "Kingdom of Exploathing")
	{
		return false;
	}
	return true;
...
questRecord[int] questDatabase()
{
	...
	retval[0] = questRecord("lh_mosquito", "questL02Larva", 0, "L2_mosquito");
	retval[1] = questRecord("lh_tavern", "questL03Rat", 0, "L3_tavern");
	retval[2] = questRecord("lh_bat", "questL04Bat", 0, "L4_batCave");
	return retval;
}
...
boolean lh_deleteMail(kmailObject msg)
{
	...
	if (get_property("lh_consultChoice") != ""){
		int id = get_player_id(get_property("lh_consultChoice")).to_int();
		if( msg.fromid == id && (contains_text(msg.message, "completed your relationship fortune test")) && get_property("lh_hideAdultery").to_boolean())
		{
			return true;
		}
	}
	if((msg.fromid == 3038166) && (contains_text(msg.message, "completed your relationship fortune test")) && get_property("lh_hideAdultery").to_boolean())
	{
		return true;
	}
...
boolean handleCopiedMonster(item itm, string option)
{
	...
	if(id != 0)
	{
		return lhAdvBypass("inv_use.php?pwd&which=3&whichitem=" + id, $location[Noob Cave], option);
	}
	return false;
}
...
//	foreach eff in $effects[Driving Stealthily, The Sonata of Sneakiness, Patent Invisibility, Shelter of Shed]
	...
	if(lh_have_familiar($familiar[Jumpsuited Hound Dog]) && my_familiar() == $familiar[Jumpsuited Hound Dog])
	{
	...
	if(doEquips)
	{
		if(!useMaximizeToEquip())
		{
			...
			if(have_familiar($familiar[Grim Brother]) && possessEquipment($item[Buddy Bjorn]))
			{
				...
				{
					lhEquip($slot[Back], $item[Buddy Bjorn]);
				}
		...
		if(lh_have_familiar($familiar[Jumpsuited Hound Dog]))
		{
			handleFamiliar($familiar[Jumpsuited Hound Dog]);
		}
	}
...
boolean lh_have_familiar(familiar fam)
{
...
boolean basicAdjustML()
{
	if((monster_level_adjustment() > 150) && (monster_level_adjustment() <= 160))
	{
		...
		if(base <= 150)
		{
			...
			lh_change_mcd(canhave);
		}
	}
	else
	{
		if(((get_property("flyeredML").to_int() >= 10000) || get_property("lh_ignoreFlyer").to_boolean()) && (my_level() >= 13))
		{
			lh_change_mcd(0);
		}
...
boolean lh_change_mcd(int mcd)
{
	...
	int handicap = best - get_property("lh_beatenUpCount").to_int();
	if(my_level() >= 13)
	{
		if((get_property("questL12War") == "finished") || (get_property("sidequestArenaCompleted") != "none") || (get_property("flyeredML").to_int() >= 10000) || get_property("lh_ignoreFlyer").to_boolean())
		{
			mcd = 0;
		}
	}
...
boolean evokeEldritchHorror(string option)
{
	...
	return lhAdvBypass(0, pages, $location[Noob Cave], option);
}
...
boolean handleSealNormal(item it, string option)
{
	...
	{
		ensureSealClubs();
		return lhAdvBypass("inv_use.php?pwd=&whichitem=" + to_int(it) + "&checked=1", $location[Noob Cave], option);
	}
	else
	{
...
boolean handleSealAncient(string option)
{
	...
	{
		return lhAdvBypass("inv_use.php?pwd=&whichitem=3905&checked=1", $location[Noob Cave], option);
	}
	else
	{
...
boolean handleSealElement(element flavor, string option)
{
	...
	return lhAdvBypass(page, $location[Noob Cave], option);
}
...
int towerKeyCount(boolean effective)
{
	...
	if(effective && (item_amount($item[Daily Dungeon Malware]) > 0) && !get_property("_dailyDungeonMalwareUsed").to_boolean() && !get_property("dailyDungeonDone").to_boolean() && (get_property("_lastDailyDungeonRoom").to_int() < 14) && (lh_my_path() != "Pocket Familiars"))
	{
		tokens = tokens + 1;
	}
	return tokens;
...
int doNumberology(string goal, boolean doIt, string option)
{
	if(!lh_have_skill($skill[Calculate the Universe]))
	{
		return -1;
	}
	...
	while(i < 100)
	{
		...
		if(numberwang[current] == goal)
		{
			...
			if(goal == "battlefield")
			{
				...
				lhAdvBypass(0, pages, $location[Noob Cave], option);
				handleTracker($monster[War Frat 151st Infantryman], $skill[Calculate the Universe], "lh_copies");
			}
			else
			{
...
boolean lh_have_skill(skill sk)
{
	return lh_is_valid(sk) && have_skill(sk);
}
...
boolean buy_item(item it, int quantity, int maxprice)
{
	...
	while((item_amount(it) < quantity) && (lh_mall_price(it) < maxprice))
	{
		if(lh_mall_price(it) > my_meat())
		{
...
boolean useCocoon()
{
	...
	if(have_skill($skill[Cannelloni Cocoon]))
	{
		...
		if(lh_have_skill($skill[Blood Bubble]) && lh_have_skill($skill[Blood Bond]))
		{
		...
		else if(lh_have_skill($skill[Blood Bubble]))
		{
			blood_skill = $skill[Blood Bubble];
		}
		else if(lh_have_skill($skill[Blood Bond]) && canUseFamiliars)
		{
			blood_skill = $skill[Blood Bond];
		}
		...
		if(lh_beta() && blood_skill != $skill[none])
		{
...
void shrugAT(effect anticipated)
{
	...
	if(lh_have_skill($skill[Mariachi Memory]))
	{
		maxSongs += 1;
	}
...
int [item] lh_get_campground()
{
	...
	if((campItems contains $item[Source Terminal]) && !get_property("lh_haveSourceTerminal").to_boolean())
	{
		set_property("lh_haveSourceTerminal", true);
	}

	static boolean didCheck = false;
	if((lh_my_path() == "Nuclear Autumn") && !didCheck)
	{
		didCheck = true;
	...
	if(!(campItems contains $item[Dramatic&trade; range]) && get_property("lh_haveoven").to_boolean())
	{
...
boolean buffMaintain(effect buff, int mp_min, int casts, int turns)
{
	...
	if((useSkill != $skill[none]) && lh_have_skill(useSkill))
	{
...
boolean lh_debug_print(string s, string color)
{
	if(get_property("lh_debug").to_boolean())
	{
		print(s, color);
		return true;
	}
...
boolean lh_can_equip(item it)
{
	return lh_can_equip(it, it.to_slot());
}
...
boolean lh_can_equip(item it, slot s)
{
	...
	if(s == $slot[off-hand] && it.to_slot() == $slot[weapon] && !lh_have_skill($skill[Double-Fisted Skull Smashing]))
		return false;

	if(it.item_type() == "chefstaff" && (!(lh_have_skill($skill[Spirit of Rigatoni]) || (my_class() == $class[Sauceror] && equipped_amount($item[special sauce glove]) > 0) || my_class() == $class[Avatar of Jarlsberg]) || s != $slot[weapon]))
		return false;

	return lh_is_valid(it) && can_equip(it);
}
...
boolean lh_check_conditions(string conds)
{
	if(conds == "")
		return true;
	...
	boolean check_condition(string cond)
	{
		...
		switch(condition_type)
		{
			...
			// As a precaution, lh_ascend aborts if to_class returns $class[none]
			case "class":
			...
			// As a precaution, lh_ascend aborts if to_stat returns $stat[none]
			case "mainstat":
			...
			// As a precaution, lh_ascend aborts if to_item returns $item[none]
			case "item":
			...
			// As a precaution, lh_ascend aborts if to_familiar returns $familiar[none]
			...
			case "havefamiliar":
				...
				if(havefamiliar == $familiar[none])
					abort('"' + condition_data + '" does not properly convert to a familiar!');
				return lh_have_familiar(havefamiliar);
			...
			// As a precaution, lh_ascend aborts if to_location returns $location[none]
			case "loc":
			...
			// As a precaution, lh_ascend will abort if to_monster returns $monster[none]
			case "sniffed":
				...
				if($classes[Cow Puncher, Beanslinger, Snake Oiler] contains my_class() && get_property("longConMonster").to_monster() == check_sniffed)
					return true;
				if(my_class() == $class[Vampyre] && get_property("lh_bat_soulmonster").to_monster() == check_sniffed)
					return true;
			...
			case "latte":
				return lh_latteDropAvailable(my_location());
			// data: Doesn't matter, but put something so I don't have to support dataless conditions
...
boolean [monster] lh_getMonsters(string category)
{
	boolean [monster] res;
	string [string,int,string] monsters_text;
	if(!file_to_map("lh_ascend_monsters.txt", monsters_text))
		print("Could not load lh_ascend_monsters.txt. This is bad!", "red");
...
boolean lh_wantToSniff(monster enemy, location loc)
{
	location locCache = my_location();
	set_location(loc);
	boolean [monster] toSniff = lh_getMonsters("sniff");
	set_location(locCache);
	return toSniff[enemy];
}

boolean lh_wantToYellowRay(monster enemy, location loc)
{
	location locCache = my_location();
	set_location(loc);
	boolean [monster] toSniff = lh_getMonsters("yellowray");
	set_location(locCache);
	return toSniff[enemy];
}
...
boolean lh_beta()
{
	return get_property("lh_beta_test").to_boolean();
}

void lh_interruptCheck()
{
	if(get_property("lh_interrupt").to_boolean())
	{
		set_property("lh_interrupt", false);
		restoreAllSettings();
		abort("lh_interrupt detected and aborting, lh_interrupt disabled.");
	}
}
...
void resetFlavour()
{
	set_property("_lh_tunedElement", "");
}

boolean setFlavour(element ele)
{
	if(!lh_have_skill($skill[Flavour of Magic]))
	{
		return false;
	}
	set_property("_lh_tunedElement", ele);
	return true;
}
...
boolean executeFlavour()
{
	...
	if(get_property("_lh_tunedElement") == "" && lh_beta())
	{
		autoFlavour(my_location());
	}
	if(get_property("_lh_tunedElement") == "")
	{
		return false;
	}
	element ele = get_property("_lh_tunedElement").to_element();
...
boolean canSimultaneouslyAcquire(int[item] needed)
{
	...
	void addToAlreadyUsed(int amount, item toAdd)
	{
		...
		if(needToCraft > 0)
		{
			...
			{
				// not craftable
				lh_debug_print("canSimultaneouslyAcquire failing on " + toAdd, "red");
				failed = true;
			}
...
int lh_reserveAmount(item it)
{
	string [string,int,string] itemdata;
	if(!file_to_map("lh_ascend_items.txt", itemdata))
		print("Could not load lh_ascend_items.txt! This is bad!", "red");
...
int lh_reserveCraftAmount(item orig_it)
{
	...
	int inner(item it)
	{
		...
		{
			int ingReserve = lh_reserveAmount(ing);
			if(ingReserve == -1)
			{
				return 0;
			}
