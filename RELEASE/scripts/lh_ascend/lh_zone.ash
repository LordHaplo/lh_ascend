...
generic_t zone_needItem(location loc)
{
	...
	case $location[The Haunted Laundry Room]:
		value = 5.0 * (1.0 + get_property("lh_cabinetsencountered").to_float());
		break;
	case $location[The Haunted Wine Cellar]:
		value = 5.0 * (1.0 + get_property("lh_wineracksencountered").to_float());
		break;
	...
	case $location[The Haunted Pantry]:
		if((lh_my_path() == "Community Service") && (item_amount($item[Tomato]) < 2) && have_skill($skill[Advanced Saucecrafting]))
		{
			retval._float = 59.4;
		}
		break;
	case $location[The Skeleton Store]:
		if((lh_my_path() == "Community Service") && have_skill($skill[Advanced Saucecrafting]) && ((item_amount($item[Cherry]) < 1) || (item_amount($item[Grapefruit]) < 1) || (item_amount($item[Lemon]) < 1)))
		{	//No idea, should spade this for great justice.
			retval._float = 33.0;
		}
		break;
	case $location[The Secret Government Laboratory]:
		if((lh_my_path() == "Community Service") && (item_amount($item[Experimental Serum G-9]) < 2))
...
generic_t zone_available(location loc)
{
	...
	case $location[Super Villain\'s Lair]:
		if((lh_my_path() == "License to Adventure") && (get_property("_villainLairProgress").to_int() < 999) && (get_property("_lh_bondBriefing") == "started"))
		{
			retval._boolean = true;
		}
		break;
	...
	case $location[The F\'c\'le]:
		...
		break;
	case $location[The Poop Deck]:
		if((have_outfit("swashbuckling getup") || possessEquipment($item[Pirate Fledges])) && (get_property("lastIslandUnlock").to_int() == my_ascensions()) && (internalQuestStatus("questM12Pirate") >= 6) && (get_property("lh_mcmuffin") != ""))
		{
			if((get_property("questL12War") == "unstarted") || (get_property("questL12War") == "finished"))
			{
				retval._boolean = true;
	...
	case $location[Belowdecks]:
		if((have_outfit("swashbuckling getup") || possessEquipment($item[Pirate Fledges])) && (get_property("lastIslandUnlock").to_int() == my_ascensions()) && (get_property("questM12Pirate") == "finished") && (get_property("lh_mcmuffin") != ""))
		{
			if((get_property("questL12War") == "unstarted") || (get_property("questL12War") == "finished"))
			{
				retval._boolean = true;
