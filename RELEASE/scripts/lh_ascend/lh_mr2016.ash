...
boolean lh_advWitchess(string target);
boolean lh_advWitchess(string target, string option);

boolean lh_haveWitchess();
boolean lh_haveSourceTerminal();
boolean lh_sourceTerminalRequest(string request);
boolean lh_sourceTerminalEnhance(string request);
int lh_sourceTerminalEnhanceLeft();
boolean lh_sourceTerminalExtrude(string request);
int lh_sourceTerminalExtrudeLeft();
int[string] lh_sourceTerminalStatus();
boolean lh_doPrecinct();
item lh_bestBadge();
boolean lh_sourceTerminalEducate(skill first);
boolean lh_sourceTerminalEducate(skill first, skill second);
...
//Supplemental
int lh_advWitchessTargets(string target);
...
boolean lh_haveSourceTerminal()
{
	...
	return (lh_get_campground() contains $item[Source Terminal]);
}
...
boolean lh_sourceTerminalExtrude(string request)
{
	...
	return lh_sourceTerminalRequest("extrude -f " + actual + ".ext");
}
...
boolean lh_sourceTerminalEnhance(string request)
{
	...
	{
		return lh_sourceTerminalRequest("enhance " + actual + ".enh");
	}
	return false;
}
int lh_sourceTerminalEnhanceLeft()
...
int[string] lh_sourceTerminalMissing()
{
	int[string] status;
	...
	status["tram.ext"] = 1;
	status["turbo.edu"] = 1;
	int[string] have = lh_sourceTerminalStatus();
...
boolean lh_sourceTerminalEducate(skill first, skill second)
{
	...
	lh_sourceTerminalRequest("educate " + firstSkill);
	if(secondSkill != "none.edu")
	{
		lh_sourceTerminalRequest("educate " + secondSkill);
	}
	return true;
}
...
boolean lh_sourceTerminalEducate(skill first)
{
	return lh_sourceTerminalEducate(first, $skill[none]);
}
...
boolean lh_haveWitchess()
{
	...
	return (lh_get_campground() contains $item[Witchess Set]);
}

boolean lh_advWitchess(string target)
{
	return lh_advWitchess(target, "");
}
...
boolean lh_advWitchess(string target, string option)
{
	...
	int goal = lh_advWitchessTargets(target);
	if(goal == 0)
	{
		return false;
	}
...
#	if(get_property("_witchessFights").to_int() > get_property("_lh_witchessBattles").to_int())
#	{
...
#		set_property("_lh_witchessBattles", get_property("_witchessFights"));
#	}

	set_property("_lh_witchessBattles", get_property("_lh_witchessBattles").to_int() + 1);

	string temp = visit_url("campground.php?action=witchess");
	if(!contains_text(temp, "Examine the shrink ray"))
	{
		set_property("_lh_witchessBattles", 5);
		return false;
	}
	...
	if(witchessMatcher.find())
	{
		...
		if(consider > get_property("_lh_witchessBattles").to_int())
		{
			set_property("_lh_witchessBattles", consider);
		}
	}
	else
	{
	...
	return lhAdvBypass(4, pages, $location[Noob Cave], option);
}
...
int lh_advWitchessTargets(string target)
{
	...
	if((target == 1942) && (lh_my_path() == "Teetotaler"))
	{
		return 1936;
	}
...
boolean lh_doPrecinct()
{
	...
	if(get_property("lh_eggDetective") != "")
	{
		set_property("lh_eggDetective", "");
	}
	...
	while(!contains_text(get_property("lh_eggDetective"), "solved"))
	{
		string[int] eggData = split_string(get_property("lh_eggDetective"), ",");
		int i=1;
		while(i<=9)
		{
			...
			if(!visited)
			{
				...
				if(personMatcher.find())
				{
					...
					set_property("lh_eggDetective", generated + "," + get_property("lh_eggDetective"));
				}
			}
			i += 1;
		}

		eggData = split_string(get_property("lh_eggDetective"), ",");
		...
		foreach index in eggData
		{
			...
			if(subEgg[4] != "liar")
			{
				...
				string temp = get_property("lh_eggDetective");
				temp = replace_string(temp, oldValue, replaceString);
				set_property("lh_eggDetective", temp);
				eggData = split_string(get_property("lh_eggDetective"), ",");
		...
		set_property("lh_eggDetective", get_property("lh_eggDetective") + "solved");
		return false;
	}
...
boolean LX_ghostBusting()
{
	...
	{
		if((lh_my_path() == "Community Service") && (my_daycount() == 1) && (goal == $location[The Spooky Forest]))
		{
			return false;
		}
		...
		if((goal == $location[Lair of the Ninja Snowmen]) && ((get_property("lh_trapper") != "yeti") && (get_property("lh_trapper") != "finished")))
		{
			return false;
		}
		if((goal == $location[The VERY Unquiet Garves]) && (get_property("lh_crypt") != "finished"))
		{
			return false;
		}
		...
		if((goal == $location[The Hidden Park]) && (get_property("lh_hiddenunlock") == "finished"))
		{
			return false;
		}
		...
		{
			lhEquip($slot[Back], $item[Protonic Accelerator Pack]);
		}

		print("Time to bust some ghosts!!!", "green");
		boolean advVal = lhAdv(goal);
		if(replaceAcc3 != $item[none])
		{
			lhEquip($slot[acc3], replaceAcc3);
		}
...
boolean timeSpinnerAdventure(string option)
{
	if(is_unrestricted($item[Time-Spinner]) && (item_amount($item[Time-Spinner]) > 0) && (get_property("_timeSpinnerMinutesUsed").to_int() <= 9))
	{
		...
		if(get_property("_timeSpinnerMinutesUsed").to_int() <= 9)
		{
			...
			if(lhAdvBypass(0, pages, $location[Noob Cave], option)) {}
			return true;
		}
	}
	return false;
...
boolean timeSpinnerCombat(monster goal, string option)
{
	if(is_unrestricted($item[Time-Spinner]) && (item_amount($item[Time-Spinner]) > 0) && (get_property("_timeSpinnerMinutesUsed").to_int() <= 7))
	{
		...
		if(get_property("_timeSpinnerMinutesUsed").to_int() <= 7)
		{
			...
			if(lhAdvBypass(0, pages, $location[Noob Cave], option))
			{
				handleTracker(goal, $item[Time-Spinner], "lh_copies");
			}
			else
			{
...
boolean rethinkingCandy(effect acquire, boolean simulate)
{
	...
	int maxprice = 2500;
	if(get_property("lh_maxCandyPrice").to_int() != 0)
	{
		maxprice = get_property("lh_maxCandyPrice").to_int();
	}
	...
	foreach it in $items[]
	{
		if(it.candy && (item_amount(it) > 0) && (lh_mall_price(it) <= maxprice) && it.tradeable)
		{
	...
	sort simpleList by lh_mall_price(value);
	sort complexList by lh_mall_price(value);
	item[int] simple = List(simpleList);
	item[int] complex = List(complexList);

#	foreach idx, it in simple
#	{
#		print(it + ": " + item_amount(it) + " (" + to_int(it) + "): " + it.candy_type + " Cost: " + lh_mall_price(it), "blue");
#	}
#
		...
		for(int i=0; i<count(simple); i++)
		{
			...
			for(int j=startNextIndex; j<count(simple); j++)
			{
				...
				if(sum == goal)
				{
					...
					if((lh_mall_price(simple[i]) + lh_mall_price(simple[j])) < bestCost)
					{
						bestCost = lh_mall_price(simple[i]) + lh_mall_price(simple[j]);
						bestFirst = simple[i];
						bestSecond = simple[j];
					}
				}
		...
		for(int i=0; i<count(simple); i++)
		{
			...
			for(int j=0; j<count(complex); j++)
			{
				...
				if(sum == goal)
				{
					...
					if((lh_mall_price(simple[i]) + lh_mall_price(complex[j])) < bestCost)
					{
						bestCost = lh_mall_price(simple[i]) + lh_mall_price(complex[j]);
						bestFirst = simple[i];
		...
		for(int i=0; i<count(complex); i++)
		{
			...
			for(int j=startNextIndex; j<count(complex); j++)
			{
				...
				if(sum == goal)
				{
					...
					if((lh_mall_price(complex[i]) + lh_mall_price(complex[j])) < bestCost)
					{
						bestCost = lh_mall_price(complex[i]) + lh_mall_price(complex[j]);
						bestFirst = complex[i];
						bestSecond = complex[j];
					}
				}
