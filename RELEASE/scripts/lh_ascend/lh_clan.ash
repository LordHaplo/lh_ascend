...
boolean lh_floundryAction(item it);
boolean lh_floundryAction();
...
boolean handleFaxMonster(monster enemy, boolean fightIt, string option)
{
	...
	if(!(lh_get_clan_lounge() contains $item[Deluxe Fax Machine]))
	{
		return false;
	}
	...
	if(item_amount($item[Photocopied Monster]) != 0)
	{
		if(get_property("photocopyMonster") == enemy)
		{
			...
			handleTracker(enemy, $item[Deluxe Fax Machine], "lh_copies");
			return lhAdvBypass("inv_use.php?pwd&which=3&whichitem=4873", $location[Noob Cave], option);
		}
		else
		{
...
boolean [location] get_floundry_locations()
{
	...
	if(!(lh_get_clan_lounge() contains $item[Clan Floundry]))
	{
		return floundryLocations;
	}
...
boolean canDrinkSpeakeasyDrink(item drink)
{
	...
	if(!(lh_get_clan_lounge() contains $item[Clan Speakeasy]))
	{
		return false;
	}

	if(!(lh_get_clan_lounge() contains drink))
	{
		return false;
	}
...
boolean zataraSeaside(string who)
{
	...
	if(!(lh_get_clan_lounge() contains $item[Clan Carnival Game]))
	{
		return false;
	}
...
#	set_property("_clanFortuneConsultUses", get_property("_clanFortuneConsultUses").to_int() + 1);
	...
	int oldClan = get_clan_id();
	string clanName = get_property("lh_consultClan");
	if (clanName != "")
	{
		changeClan(clanName);	
	}
	...
	int attempts = 0;
	int player = 3038166;
	string consultOverrideName = get_property("lh_consultChoice");
	...
	while(attempts < 5)
	{
		...
		if (get_property("lh_optimizeConsultsInRun").to_boolean() && my_path() != "None")
		{
...
boolean eatFancyDog(string dog)
{
	...
	if(!(lh_get_clan_lounge() contains $item[Clan Hot Dog Stand]))
	{
		return false;
	}
...
boolean lh_floundryAction()
{
	...
	if(!get_property("_floundryItemGot").to_boolean() && (lh_get_clan_lounge() contains $item[Clan Floundry]) && !get_property("kingLiberated").to_boolean())
	{
		if(get_property("lh_floundryChoice") != "")
		{
			string[int] floundryChoice = split_string(get_property("lh_floundryChoice"), ";");
			item myFloundry = trim(floundryChoice[min(count(floundryChoice), my_daycount()) - 1]).to_item();
			if(lh_floundryAction(myFloundry))
			{
...
boolean lh_floundryAction(item it)
{
	...
	int[item] fish = lh_get_clan_lounge();
	if(fish[it] > 0)
	{
