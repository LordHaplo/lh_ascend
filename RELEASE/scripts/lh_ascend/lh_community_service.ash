script "lh_community_service.ash"
...
static int[int] lh_cs_fastQuestList;
...
boolean LA_cs_communityService()
{
	...
	if(lh_voteMonster(true, $location[Barf Mountain], "cs_combatNormal"))
	{
		return true;
	}
	...
	familiar toFam = get_property("lh_familiarChoice").to_familiar();
	...
	if(isOverdueDigitize() && (my_daycount() == 1) && (get_property("_sourceTerminalDigitizeMonsterCount").to_int() == 1))
	{
		if((get_property("_sourceTerminalDigitizeUses").to_int() > 0) && (get_property("_sourceTerminalDigitizeUses").to_int() < 3))
		{
			...
			lh_sourceTerminalEducate($skill[Digitize], $skill[Extract]);
			set_property("lh_combatDirective", "start;skill digitize");
			lhAdv(1, $location[Barf Mountain], "cs_combatNormal");
	...
	if((my_daycount() == 2) && lh_haveWitchess() && have_skills($skills[Conspiratorial Whispers, Curse of Weaksauce, Sauceshell, Shell Up, Silent Slam]) && (have_skill($skill[Tattle]) || have_skill($skill[Meteor Lore])) && !possessEquipment($item[Dented Scepter]) && (get_property("_lh_witchessBattles").to_int() < 5) && have_familiar($familiar[Galloping Grill]) && (my_ascensions() >= 100))
	{
		...
		lh_sourceTerminalEducate($skill[Turbo], $skill[Compress]);
		boolean result = lh_advWitchess("king", "cs_combatKing");
		lh_sourceTerminalEducate($skill[Compress], $skill[Extract]);
		return result;
	}
	if((my_daycount() == 2) && lh_haveWitchess() && have_skills($skills[Conspiratorial Whispers, Curse of Weaksauce, Sauceshell, Shell Up, Silent Slam]) && (have_skill($skill[Tattle]) || have_skill($skill[Meteor Lore]))  && possessEquipment($item[Dented Scepter]) && !possessEquipment($item[Battle Broom]) && (get_property("_lh_witchessBattles").to_int() < 5) && have_familiar($familiar[Galloping Grill]) && (my_ascensions() >= 100))
	{
		while((my_mp() < 120) && (get_property("timesRested").to_int() < total_free_rests()) && chateaumantegna_available())
		{
			doRest();
		...
		lh_sourceTerminalEducate($skill[Compress], $skill[Extract]);
		boolean result = lh_advWitchess("witch", "cs_combatWitch");
		lh_sourceTerminalEducate($skill[Extract], $skill[Turbo]);
		return result;
	}
...
//		cheeseWarMachine(0, 1, 3, 0);
	...
	if((curQuest == 11) || (curQuest == 6) || (curQuest == 9) || (curQuest == 7))
	{
		...
		if(curQuest != 7)
		{
			if((my_inebriety() == 0) && (lh_get_clan_lounge() contains $item[Clan Speakeasy]) && (item_amount($item[Clan VIP Lounge Key]) > 0) && (my_meat() >= 500))
			{
				lhDrink(1, $item[Lucky Lindy]);
				if(my_inebriety() != 1)
				{
					if(lh_get_clan_lounge() contains $item[Lucky Lindy])
					{
			...
			if((my_inebriety() == 0) && !(lh_get_clan_lounge() contains $item[Clan Speakeasy]))
			{
			...
			if((inebriety_left() > 9) && (my_inebriety() != 0))
			{
				if((item_amount($item[Shot of Kardashian Gin]) > 0) && hippy_stone_broken())
				{
					...
					{
						lhDrink(min(item_amount($item[Shot of Kardashian Gin]), (inebriety_left() - 9)), $item[Shot of Kardashian Gin]);
					}
					else
					{
						lhOverdrink(min(item_amount($item[Shot of Kardashian Gin]), (inebriety_left() - 9)), $item[Shot of Kardashian Gin]);
					}
				}
				if(item_amount($item[Sacramento Wine]) > 0)
				{
					lhDrink(min(item_amount($item[Sacramento Wine]), (inebriety_left() - 9)), $item[Sacramento Wine]);
				}
				else if(item_amount($item[Ice Island Long Tea]) > 0)
				{
					lhDrink(1, $item[Ice Island Long Tea]);
				}
				else if((item_amount($item[Splendid Martini]) > 0) && canDrink($item[Splendid Martini]))
				{
					lhDrink(min(item_amount($item[Splendid Martini]), (inebriety_left() - 9)), $item[Splendid Martini]);
				}
				else if(item_amount($item[Meadeorite]) > 0)
				{
					lhDrink(min(item_amount($item[Meadeorite]), (inebriety_left() - 9)), $item[Meadeorite]);
				}

			}

			if(get_property("lh_csDoWheel").to_boolean() && do_chateauGoat())
			{
				return true;
			}
			...
			if((curQuest == 11) && ((my_turncount() + 60) < get_property("lh_cookie").to_int()) && (my_adventures() > 65))
			{
				if(do_cs_quest(11))
				{
					use(1, $item[A Ten-Percent Bonus]);
			...
			if((curQuest != 11) && (have_effect($effect[substats.enh]) == 0) && (lh_sourceTerminalEnhanceLeft() >= 1))
			{
				lh_sourceTerminalEnhance("substats");
			}

			if(get_property("_lh_witchessBattles").to_int() <= 2)
			{
				if(cs_witchess())
				{
					return true;
			...
			if(get_property("_lh_witchessBattles").to_int() == 3)
			{
				if((my_mp() >= 120) && lh_haveWitchess() && have_skills($skills[Conspiratorial Whispers, Curse Of Weaksauce, Sauceshell, Shell Up, Silent Slam]) && (have_skill($skill[Tattle]) || have_skill($skill[Meteor Lore])) && !possessEquipment($item[Dented Scepter]) && have_familiar($familiar[Galloping Grill]) && (my_ascensions() >= 100))
				{
					handleFamiliar($familiar[Galloping Grill]);
					lh_sourceTerminalEducate($skill[Turbo], $skill[Compress]);
					boolean retval = lh_advWitchess("king", "cs_combatKing");
					lh_sourceTerminalEducate($skill[Compress], $skill[Extract]);
					return retval;
				}
			}
...
//			if((my_turncount() >= 60) && cs_witchess())
			...
			if(my_ascensions() > get_property("lastGuildStoreOpen").to_int())
			{
				...
				lhAdv(1, loc, "cs_combatNormal");
				return true;
			}

			int tomatoGoal = 2;
			if(get_property("lh_hccsNoConcludeDay").to_boolean())
			{
				tomatoGoal = 1;
			}
			...
			if((item_amount($item[Tomato]) < tomatoGoal) && have_skill($skill[Advanced Saucecrafting]))
			{
				...
				{
					lhEquip($slot[Off-Hand], $item[Latte Lovers Member\'s Mug]);
				}
				lhAdv(1, $location[The Haunted Pantry], "cs_combatNormal");
				return true;
			}
			...
			if(have_skill($skill[Advanced Saucecrafting]) && ((item_amount($item[Cherry]) < 1) || (item_amount($item[Grapefruit]) < 1) || (item_amount($item[Lemon]) < 1)))
			{
				...
				lhAdv(1, $location[The Skeleton Store], "cs_combatNormal");
				return true;
			}
			...
			{
				lhAdv(1, $location[Pirates of the Garbage Barges], "cs_combatNormal");
				return true;
			}
			...
			if((((item_amount($item[Gene Tonic: Elemental]) == 0) && (get_property("_dnaPotionsMade").to_int() < 3)) || !get_property("_dnaHybrid").to_boolean()) && (item_amount($item[DNA Extraction Syringe]) > 0) && elementalPlanes_access($element[hot]))
			{
				...
				lhAdv(1, $location[The Bubblin\' Caldera], "cs_combatNormal");
				if((have_effect($effect[Beaten Up]) > 0) && have_skill($skill[Tongue of the Walrus]) && (my_mp() > (3 * mp_cost($skill[Tongue of the Walrus]))))
				{
					if(contains_text(get_property("lh_combatHandler"), "(DNA)"))
					{
						use_skill(2, $skill[Tongue of the Walrus]);
					}
					else
					{
						set_property("lh_beatenUpCount", get_property("lh_beatenUpCount").to_int() + 1);
					}
				}
				return true;
			}
...
#			if(((get_property("_g9Effect").to_int() > 125) || (item_amount($item[Airborne Mutagen]) == 0)) && (((curQuest == 9) || (my_turncount() < get_property("lh_cookie").to_int())) && elementalPlanes_access($element[spooky])))
			if(((curQuest == 9) || (my_turncount() < get_property("lh_cookie").to_int())) && elementalPlanes_access($element[spooky]))
			{
				...
				{
					lhEquip($slot[acc1], $item[Personal Ventilation Unit]);
				}
				int turns_max = 10;
				...
				if($location[The Secret Government Laboratory].turns_spent <= turns_max)
				{
					...
					lhAdv(1, $location[The Secret Government Laboratory], "cs_combatNormal");
					return true;
				}
			}
		}
		else if((curQuest == 7) && (item_amount($item[Emergency Margarita]) > 0))
		{
			...
			if((have_effect($effect[substats.enh]) == 0) && (lh_sourceTerminalEnhanceLeft() >= 1))
			{
				lh_sourceTerminalEnhance("substats");
			}
			...
			if((inebriety_left() > 0) && (my_adventures() <= 2))
			{
				...
				{
					lhDrink(min(inebriety_left(), item_amount($item[Astral Pilsner])), $item[Astral Pilsner]);
				}
				else if(item_amount($item[Sacramento Wine]) > 0)
				{
					lhDrink(1, $item[Sacramento Wine]);
				}
				else if(item_amount($item[Iced Plum Wine]) > 0)
				{
					lhDrink(1, $item[Iced Plum Wine]);
				}
				else if(item_amount($item[Meadeorite]) > 0)
				{
					lhDrink(1, $item[Meadeorite]);
				}
				else if((lh_get_clan_lounge() contains $item[Clan Speakeasy]) && (item_amount($item[Clan VIP Lounge Key]) > 0))
				{
					lhDrink(1, $item[cup of &quot;tea&quot;]);
				}
			}

			if((my_spleen_use() == 12) && (item_amount($item[Abstraction: Category]) > 0))
			{
				lhChew(1, $item[Abstraction: Category]);
			}
			...
			if((isOverdueDigitize() || isOverdueArrow()) && elementalPlanes_access($element[stench]) && (get_property("_lh_margaritaWanderer") != my_turncount()))
			{
				set_property("_lh_margaritaWanderer", my_turncount());
			...
			if(canYellowRay() && elementalPlanes_access($element[hot]))
			{
				...
				if((!possessEquipment($item[Heat-Resistant Necktie]) || !possessEquipment($item[Heat-Resistant Gloves]) || !possessEquipment($item[Lava-Proof Pants])) && $location[LavaCo&trade; Lamp Factory].turns_spent < 10)
				{
					...
					lhAdv(1, $location[LavaCo&trade; Lamp Factory], "cs_combatYR");
					return true;
				}
			}

			boolean turnSave = get_property("lh_hccsTurnSave").to_boolean() || !in_hardcore();
			...
			{
				lhEquip($slot[acc1], $item[Continuum Transfunctioner]);

				if(get_property("lh_tryPowerLevel").to_boolean())
				{
				...
				lhAdv(1, $location[8-bit Realm], "cs_combatNormal");
				return true;
			}
			...
			if(have_skill($skill[Advanced Saucecrafting]) && (lh_get_campground() contains $item[Dramatic&trade; Range]))
			{
				int tomatoCheck = 4;
				int tomatoMake = 6;
			...
			if(!have_familiar($familiar[Puck Man]) && !have_familiar($familiar[Ms. Puck Man]) && (get_property("lh_csPuckCounter").to_int() > 0))
			{
				set_property("lh_csPuckCounter", get_property("lh_csPuckCounter").to_int() - 1);
				doFarm = true;
			}
			...
			if(doFarm)
			{
				if(get_property("lh_tryPowerLevel").to_boolean())
				{
					print("Trying to powerlevel, since lh_tryPowerLevel=true", "blue");
...
#						buffMaintain($effect[Physicali Tea], 0, 1, 1);
						...
						{
							lhAdv(1, $location[Uncle Gator\'s Country Fun-Time Liquid Waste Sluice], "cs_combatNormal");
							return true;
						}
					}
				}
...
#				{
#					lhAdv(1, $location[The Velvet / Gold Mine], "cs_combatNormal");
#				}
				...
				else if(elementalPlanes_access($element[cold]))
				{
					...
					lhAdv(1, $location[VYKEA], "cs_combatNormal");
				}
				...
				{
					lhAdv(1, $location[The Thinknerd Warehouse], "cs_combatNormal");
				}
				return true;
			}
			...
			if((missing > (item_amount($item[Miniature Power Pill]) + item_amount($item[Power Pill]))) && (have_familiar($familiar[Puck Man]) || have_familiar($familiar[Ms. Puck Man])) && (!is100familiarRun() || !is100familiarRun($familiar[Puck Man]) || !is100familiarRun($familiar[Ms. Puck Man])))
			{
				...
				{
					lhAdv(1, $location[The Deep Dark Jungle], "cs_combatNormal");
				}
				else if(elementalPlanes_access($element[sleaze]))
				{
					lhAdv(1, $location[Sloppy Seconds Diner], "cs_combatNormal");
				}
				else
				{
					lhAdv(1, $location[The Haunted Kitchen], "cs_combatNormal");
				}
			...
			if((canYellowRay() || (numeric_modifier("item drop") >= 150.0)) && !get_property("_photocopyUsed").to_boolean() && ($classes[Pastamancer, Sauceror] contains my_class()))
			{
				...
				lh_sourceTerminalEducate($skill[Duplicate], $skill[Extract]);
				useCocoon();
			...
			if(elementalPlanes_access($element[hot]) && have_skills($skills[Meteor Lore, Snokebomb]) && have_familiar($familiar[XO Skeleton]) && (my_mp() > mp_cost($skill[Snokebomb])) && (get_property("_snokebombUsed").to_int() < 3) && (get_property("_macrometeoriteUses").to_int() < 10) && (get_property("_xoHugsUsed").to_int() < 11) && !is100FamiliarRun($familiar[XO Skeleton]))
			{
				if((!possessEquipment($item[Fireproof Megaphone]) && !possessEquipment($item[Meteorite Guard])) || !possessEquipment($item[High-Temperature Mining Mask]))
				{
					...
					lhAdv(1, $location[The Velvet / Gold Mine], "cs_combatXO");
					return true;
				}
				...
				{
					handleFamiliar($familiar[XO Skeleton]);
					lhAdv(1, $location[LavaCo&trade; Lamp Factory], "cs_combatXO");
					return true;
				}
			}
...
//			}
			...
			if(!get_property("lh_hccsNoConcludeDay").to_boolean())
			{
				...
				{
					lhChew(1, $item[Handful of Smithereens]);
				}
				...
				{
					lhChew(1, $item[cuppa Activi tea]);
				}
				while((spleen_left() >= 2) && (item_amount($item[Nasty Snuff]) > 0))
				{
					lhChew(1, $item[Nasty Snuff]);
				}
			...
			if(!get_property("lh_saveMargarita").to_boolean() && (inebriety_left() == 0))
			{
				if((my_adventures() > 160) || (my_daycount() > 1))
				{
					abort("We have an emergency margarita but it seems really dumb to drink it right now (consider closeting the margarita and set lh_saveMargarita = true)...");
				}
				...
				lhOverdrink(1, $item[Emergency Margarita]);
			}
			else
			{
				...
				{
					lh_sourceTerminalExtrude($item[Hacked Gibson]);
				}
				...
				set_property("lh_familiarChoice", "");
...
#			}
			...
			{
				lhChew(1, $item[Ancient Medicinal Herbs]);
			}
			...
			{
				lhChew(1, $item[Abstraction: Action]);
			}

			if((inebriety_left() >= 12) && (item_amount($item[Clan VIP Lounge Key]) > 0) && (lh_get_clan_lounge() contains $item[Clan Speakeasy]) && (have_effect($effect[On The Trolley]) == 0) && (get_cs_questCost(curQuest) < 20))
			{
				lhDrink(1, $item[Bee's Knees]);
			}
	...
	case 2:		#Muscle Quest
		{
			...
			if((inebriety_left() >= 12) && (item_amount($item[Clan VIP Lounge Key]) > 0) && (lh_get_clan_lounge() contains $item[Clan Speakeasy]) && (have_effect($effect[On The Trolley]) == 0) && (estimate_cs_questCost(curQuest) > 1))
			{
				lhDrink(1, $item[Bee's Knees]);
			}
	...
	case 3:		#Myst Quest
		{
			...
			{
				lhChew(1, $item[Abstraction: Thought]);
			}
	...
	case 4:		#Moxie Quest
		{
			...
			{
				lhChew(1, $item[Abstraction: Sensation]);
			}
	...
	case 5:		#Familiar Weight
		{
			...
			if((have_effect($effect[Puzzle Champ]) == 0) && get_property("lh_pauseForWitchess").to_boolean())
			{
			...
			if(inebriety_left() >= 10)
			{
				if(my_adventures() < needCost)
				{
					...
					if((my_adventures() + extraAdv) > needCost)
					{
						...
						{
							lhDrink(4, $item[Sacramento Wine]);
						}
						...
						if((my_meat() > 5000) && (item_amount($item[Clan VIP Lounge Key]) > 0) && (inebriety_left() >= 3) && (lh_get_clan_lounge() contains $item[Clan Speakeasy]))
						{
							lhDrink(1, $item[Hot Socks]);
						}
						if((item_amount($item[Asbestos Thermos]) > 0) && (inebriety_left() >= 4))
						{
							lhDrink(1, $item[Asbestos Thermos]);
						}
						if((item_amount($item[Cold One]) > 0) && (inebriety_left() >= 1))
						{
							lhDrink(1, $item[Cold One]);
						}

					}
					else if ((item_amount($item[Vintage Smart Drink]) > 0) && !get_property("lh_saveVintage").to_boolean())
					{
						lhDrink(1, $item[Vintage Smart Drink]);
					}
				}
			}
...
//			januaryToteAcquire($item[Broken Champagne Bottle]);
			...
			{
				if(get_property("lh_noSleepingDog").to_boolean() || have_skill($skill[Dog Tired]))
				{
	...
	case 8:			#Non-Combat
		{
			...
			if((lh_mall_price($item[Shady Shades]) < 20000) && (questCost > 12))
			{
				buffMaintain($effect[Throwing Some Shade], 0, 1, 1);
			}
			if((lh_mall_price($item[Squeaky Toy Rose]) < 20000) && (questCost > 12))
			{
				buffMaintain($effect[A Rose by Any Other Material], 0, 1, 1);
			}
			getHorse("non-combat");
			...
			if(my_adventures() < questCost)
			{
				...
				lhChew(item_amount($item[Handful of Smithereens]), $item[Handful of Smithereens]);
			}

			if((my_adventures() < questCost) || (lh_mall_price($item[Pocket Wish]) < 35000))
			{
	...
	case 9:		#item/booze drops
		{
			...
			if(have_familiar($familiar[Trick-or-Treating Tot]) && !possessEquipment($item[Li\'l Ninja Costume]))
			{
				if(have_familiar($familiar[Pair of Stomping Boots]))
				{
					...
					while(runs > 0)
					{
						...
						lhAdv(1, $location[The Haiku Dungeon], "cs_combatNormal");
				...
				{
					backupSetting("choiceAdventure297", "3");
					boolean result = lhAdv(1, $location[The Haiku Dungeon], "cs_combatNormal");
					restoreSetting("choiceAdventure297");
					return result;
				}
			}
			...
			{
				lhChew(1, $item[Abstraction: Certainty]);
			}
...
#				makeGenieWish($effect[Frosty]);
			...
			if((get_property("lh_csDoWheel").to_boolean()) && (get_cs_questCost(curQuest) > 7))
			{
				deck_cheat("items");
			}
...
//				}
				if(elementalPlanes_access($element[hot]))
				{
					if((!possessEquipment($item[Fireproof Megaphone]) && !possessEquipment($item[Meteorite Guard])) || !possessEquipment($item[High-Temperature Mining Mask]))
					{
						...
						lhAdv(1, $location[The Velvet / Gold Mine], "cs_combatYR");
						return true;
					}
				}
			}
...
boolean cs_witchess()
{
	...
	if(get_property("_lh_witchessBattles").to_int() >= 5)
	{
		return false;
	}
...
#	}
	...
	else if(have_familiar($familiar[Galloping Grill]) && lh_haveSourceTerminal())
	{
		handleFamiliar($familiar[Galloping Grill]);
	}
	...
	{
		result = lh_advWitchess("rook", "cs_combatNormal");
	}
	else
	{
		string goal = "booze";
...
*/
		...
		result = lh_advWitchess(goal, "cs_combatNormal");
	}
...
void cs_initializeDay(int day)
{
	...
	if(day == 1)
	{
		if(get_property("lh_day_init").to_int() < 1)
		{
			...
			if(get_property("lh_csDoWheel").to_boolean())
			{
			...
			if(!(lh_get_campground() contains $item[Packet Of Tall Grass Seeds]))
			{
...
boolean do_chateauGoat()
{
	if(!get_property("_chateauMonsterFought").to_boolean() && chateaumantegna_available() && (get_property("chateauMonster") == $monster[dairy goat]))
	{
		...
		lh_sourceTerminalEducate($skill[Compress], $skill[Turbo]);
...
void cs_make_stuff(int curQuest)
{
	...
	if(my_daycount() == 1)
	{
		if(!possessEquipment($item[Hairpiece on Fire]) && (item_amount($item[Lump of Brituminous Coal]) > 0) && !get_property("lh_hccsNoConcludeDay").to_boolean())
		{
...
boolean cs_eat_stuff(int quest)
{
	...
	if((quest == 0) && (my_fullness() == 0))
	{
		...
		if(get_property("lh_noSleepingDog").to_boolean() || have_skill($skill[Dog Tired]))
		{
			if(lh_sourceTerminalExtrudeLeft() > 0)
			{
				if(item_amount($item[Browser Cookie]) == 0)
				{
					lh_sourceTerminalExtrude($item[Browser Cookie]);
				}
			}

			if(item_amount($item[Browser Cookie]) > 0)
			{
				lhEat(1, $item[Browser Cookie]);
			}
			else
			{
				eatFancyDog("savage macho dog");
	...
	if(quest == 9)
	{
		if(item_amount($item[Weird Gazelle Steak]) > 0)
		{
			...
			lhEat(1, $item[Weird Gazelle Steak]);

			if(get_property("lh_noSleepingDog").to_boolean())
			{
				if(lh_sourceTerminalExtrudeLeft() > 0)
				{
	...
	{
		if((item_amount($item[Sausage Without A Cause]) > 0) && !get_property("lh_saveSausage").to_boolean())
		{
			if(item_amount($item[Milk of Magnesium]) > 0)
			{
				use(1, $item[Milk of Magnesium]);
...
string cs_combatNormal(int round, string opp, string text)
{
	...
	{
		set_property("lh_combatHandler", combatState + "(lattegulp)");
	...
	if(!contains_text(combatState, "snokebomb") && have_skill($skill[Snokebomb]) && (get_property("_snokebombUsed").to_int() < 3) && ((my_mp() - 20) >= mp_cost($skill[Snokebomb])))
	{
		...
		{
			set_property("lh_combatHandler", combatState + "(Snokebomb)");
			handleTracker(enemy, $skill[Snokebomb], "lh_banishes");
	...
	if(!contains_text(combatState, "throwlatte") && have_skill($skill[Throw Latte On Opponent]) && !get_property("_latteBanishUsed").to_boolean() && (my_mp() >= mp_cost($skill[Throw Latte On Opponent])))
	{
		...
		{
			set_property("lh_combatHandler", combatState + "(throwlatte)");
			handleTracker(enemy, $skill[Throw Latte On Opponent], "lh_banishes");
	...
	if(!contains_text(combatState, $skill[KGB Tranquilizer Dart]) && have_skill($skill[KGB Tranquilizer Dart]) && (get_property("_kgbTranquilizerDartUses").to_int() < 3))
	{
		...
		{
			set_property("lh_combatHandler", combatState + "(" + $skill[KGB Tranquilizer Dart] + ")");
			handleTracker(enemy, $skill[KGB Tranquilizer Dart], "lh_banishes");
...
#		}
		...
		{
			set_property("lh_combatHandler", combatState + "(candyblast)");
	...
	if((have_effect($effect[On The Trail]) == 0) && have_skill($skill[Transcendent Olfaction]) && (my_mp() >= mp_cost($skill[Transcendent Olfaction])))
	{
		...
		if(doOlfaction)
		{
			...
			{
				set_property("lh_combatHandler", combatState + "(soulbubble)");
				return "skill " + $skill[Soul Bubble];
			}

			set_property("lh_combatHandler", combatState + "(olfaction)");
	...
	if(!contains_text(combatState, "(lattesniff)") && have_skill($skill[Offer Latte To Opponent]) && (my_mp() >= mp_cost($skill[Offer Latte To Opponent])) && !get_property("_latteCopyUsed").to_boolean())
	{
		if(($monster[Government Scientist] == enemy) && (get_property("_latteMonster") != enemy))
		{
			...
			set_property("lh_combatHandler", combatState + "(lattesniff)");
	...
	if(((have_effect($effect[On The Trail]) < 40) || (contains_text(combatState, "(lattesniff)"))) && have_skill($skill[Gallapagosian Mating Call]) && (my_mp() >= mp_cost($skill[Gallapagosian Mating Call])) && !contains_text(combatState, "(matingcall)"))
	{
		if(($monster[Government Scientist] == enemy) && (get_property("_gallapagosMonster") != enemy))
		{
			...
			set_property("lh_combatHandler", combatState + "(matingcall)");
	...
	if(!contains_text(combatState, "DNA") && (item_amount($item[DNA Extraction Syringe]) > 0))
	{
		if(type != current)
		{
			set_property("lh_combatHandler", combatState + "(DNA)");
	...
	if(!contains_text(combatState, "winkat") && (my_familiar() == $familiar[Reanimated Reanimator]))
	{
		...
		{
			set_property("lh_combatHandler", combatState + "(winkat)");
	...
	if(!contains_text(combatState, "badlyromanticarrow") && (my_familiar() == $familiar[Obtuse Angel]))
	{
		if($monsters[Witchess Bishop, Witchess Knight] contains enemy)
		{
			set_property("lh_combatHandler", combatState + "(badlyromanticarrow)");
			if((get_property("_badlyRomanticArrows").to_int() == 1) && (get_property("romanticTarget") != enemy))
			{
	...
	if((my_location() == $location[The Secret Government Laboratory]) && get_property("controlPanel9").to_boolean())
	{
		...
		{
			set_property("lh_combatHandler", combatState + "(conspiratorialwhispers)");
	...
	{
		set_property("lh_combatHandler", combatState + "(digitize)");
	...
	{
		set_property("lh_combatHandler", combatState + "(extract)");
	...
	{
		set_property("lh_combatHandler", combatState + "(shattering punch)");
	...
	{
		set_property("lh_combatHandler", combatState + "(gingerbread mob hit)");
	...
	{
		set_property("lh_combatHandler", combatState + "(love mosquito)");
	...
	{
		set_property("lh_combatHandler", combatState + "(entangling noodles)");
...
string cs_combatXO(int round, string opp, string text)
{
	...
	if(my_location() == $location[LavaCo&trade; Lamp Factory])
	{
		if(!possessEquipment($item[Heat-Resistant Necktie]) || !possessEquipment($item[Heat-Resistant Gloves]) || !possessEquipment($item[Lava-Proof Pants]))
		{
			...
			{
				set_property("lh_combatHandler", combatState + "(hugpocket)");
...
string cs_combatYR(int round, string opp, string text)
{
	...
	if(have_skill($skill[Duplicate]) && (enemy == $monster[Sk8 Gnome]))
	{
		foreach action in $skills[Curse of Weaksauce, Conspiratorial Whispers, Summon Love Mosquito, Shell Up, Silent Slam, Summon Love Stinkbug, Extract, Duplicate]
		{
			...
			{
				set_property("lh_combatHandler", combatState + "(" + action + ")");
				return "skill " + action;
			}
		}
	}
	...
	{
		set_property("lh_combatHandler", combatState + "(summon love gnats)");
	...
	if(have_skill($skill[Disintegrate]) && (my_mp() < 150) && (have_effect($effect[Everything Looks Yellow]) == 0))
	{
		...
		{
			set_property("lh_combatHandler", combatState + "(turbo)");
	...
	if((have_effect($effect[Everything Looks Yellow]) == 0) && (lookFor contains enemy))
	{
		if(!contains_text(combatState, "yellowray"))
		{
			if(have_skill($skill[Disintegrate]) && (my_mp() > mp_cost($skill[Disintegrate])) && (have_effect($effect[Everything Looks Yellow]) == 0))
			{
				...
				handleTracker(enemy, $skill[Disintegrate], "lh_yellowRays");
	...
	{
		set_property("lh_combatHandler", combatState + "(summon love stinkbug)");
	...
	{
		set_property("lh_combatHandler", combatState + "(summon love mosquito)");
...
#	}
	if(!contains_text(combatState, "giant growth") && have_skill($skill[Giant Growth]))
	{
		...
		set_property("lh_combatHandler", combatState + "(giant growth)");
	...
	{
		set_property("lh_combatHandler", combatState + "(louder than bomb)");
	...
	{
		set_property("lh_combatHandler", combatState + "(tennis ball)");
	...
	{
		set_property("lh_combatHandler", combatState + "(power pill)");
...
boolean cs_giant_growth()
{
	...
	if((isOverdueDigitize() || isOverdueArrow()) && elementalPlanes_access($element[stench]))
	{
		...
		lhAdv(1, $location[Barf Mountain], "cs_combatLTB");
		if(have_effect($effect[Giant Growth]) > 0)
		{
			return true;
		}
...
#	print("Starting LTBs: " + item_amount($item[Louder Than Bomb]), "blue");
	...
	if(my_familiar() == $familiar[Machine Elf])
	{
		backupSetting("choiceAdventure1119", 1);
		handleFamiliar($familiar[Machine Elf]);
		lhAdv(1, $location[The Deep Machine Tunnels], "cs_combatLTB");
		restoreSetting("choiceAdventure1119");
		set_property("choiceAdventure1119", "");
	}
	else if(!godLobsterCombat($item[none], 3, "cs_combatLTB"))
	{
#		lhAdv(1, $location[8-bit Realm], "cs_combatLTB");
		lhAdv(1, $location[The Thinknerd Warehouse], "cs_combatLTB");
	}
...
boolean lh_csHandleGrapes()
{
	...
	if((item_amount($item[Gr8ps]) > 0) && (item_amount($item[Potion of Temporary Gr8ness]) == 0) && (have_effect($effect[Gr8tness]) == 0) && (npc_price($item[Delectable Catalyst]) < my_meat()) && (freeCrafts() > 0) && (item_amount($item[Scrumptious Reagent]) > 0) && have_skill($skill[Advanced Saucecrafting]) && ($classes[Pastamancer, Sauceror] contains my_class()) && have_skill($skill[The Way of Sauce]) && (lh_get_campground() contains $item[Dramatic&trade; Range]))
	{
		if(!have_skills($skills[Expert Corner-Cutter, Rapid Prototyping]) && have_skill($skill[Inigo\'s Incantation of Inspiration]))
		{
			shrugAT($effect[Inigo\'s Incantation of Inspiration]);
...
boolean do_cs_quest(int quest)
{
	...
	switch(quest)
	{
	case 1:		lhMaximize("hp, -equip snow suit", 1500, 0, false);					break;
	case 2:		lhMaximize("muscle, -equip snow suit", 1500, 0, false);				break;
	case 3:		lhMaximize("myst, -equip snow suit", 1500, 0, false);				break;
	case 4:		lhMaximize("moxie, -equip snow suit", 1500, 0, false);				break;
	case 5:		lhMaximize("familiar weight, -equip snow suit", 1500, 0, false);	break;
	case 6:		lhMaximize("weapon damage, -equip snow suit", 1500, 0, false);		break;
	case 7:		lhMaximize("spell damage, -equip snow suit", 1500, 0, false);		break;
	case 8:		lhMaximize("-combat, -equip snow suit", 1500, 0, false);			break;
	case 9:		lhMaximize("item, -equip snow suit, -equip broken champagne bottle", 1500, 0, false);				break;
	case 10:	lhMaximize("hot res, -equip snow suit", 1500, 0, false);			break;
	}
...
{
	int[int] empty;
	lh_cs_fastQuestList = empty;
	foreach idx, val in fast
	{
		lh_cs_fastQuestList[val] = val;
	}
}
...
boolean cs_preTurnStuff(int curQuest)
{
	...
	int[int] questsLeft = lh_cs_fastQuestList;
	...
	if(is100FamiliarRun())
	{
		...
		{
			print("100% familiar is dangerous, to disable: set lh_100familiar=none", "red");
	...
	{
		if((item_amount($item[Cashew]) >= 1) && !possessEquipment($item[Glass Casserole Dish]) && ((item_amount($item[January\'s Garbage Tote]) == 0) || (get_property("lh_beatenUpCount").to_int() >= 3)))
		{
