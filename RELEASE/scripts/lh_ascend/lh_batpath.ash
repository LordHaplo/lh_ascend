...
boolean bat_wantHowl(location loc)
{
	...
	if(lh_banishesUsedAt(loc) contains "baleful howl")
	{
		return false;
	}
	...
	{
		if (!(banished contains monsters[i]) && (lh_wantToBanish(monsters[i], loc))) {
			return true;
		}
	}
	return false;
...
void bat_formNone()
{
	...
	if(get_property("lh_bat_desiredForm") != "")
	{
		set_property("lh_bat_desiredForm", "");
	}
}
...
{
	if(my_class() != $class[Vampyre]) return;
	set_property("lh_bat_desiredForm", "wolf");
...
{
	if(my_class() != $class[Vampyre]) return;
	set_property("lh_bat_desiredForm", "mist");
...
{
	if(my_class() != $class[Vampyre]) return;
	set_property("lh_bat_desiredForm", "bats");
...
boolean bat_formPreAdventure()
{
	...
	string desiredForm = get_property("lh_bat_desiredForm");
	effect form;
	switch(desiredForm)
	{
	case "wolf":
	...
	case "":
		...
		return true;
	default:
		print("lh_bat_desiredForm was set to bad value: '" + desiredForm + "'. Should be '', 'wolf', 'mist', or 'bats'.", "red");
		set_property("lh_bat_desiredForm", "");
		return false;
	}
}
...
void bat_initializeSession()
{
	...
	{
		set_property("lh_mpAutoRecovery", get_property("mpAutoRecovery"));
		set_property("lh_mpAutoRecoveryTarget", get_property("mpAutoRecoveryTarget"));
...
void bat_terminateSession()
{
	if(my_class() == $class[Vampyre])
	{
		set_property("mpAutoRecovery", get_property("lh_mpAutoRecovery"));
		set_property("lh_mpAutoRecovery", 0.0);
		set_property("mpAutoRecoveryTarget", get_property("lh_mpAutoRecoveryTarget"));
		set_property("lh_mpAutoRecoveryTarget", 0.0);
	}
}
...
void bat_initializeDay(int day)
{
	...
	if(get_property("lh_day_init").to_int() < day)
	{
		set_property("_lh_bat_bloodBank", 0); // 0: no blood yet, 1: base blood, 2: intimidating blood
		set_property("lh_bat_ensorcels", 0);
		set_property("lh_bat_soulmonster", "");
		bat_tryBloodBank();
...
int bat_remainingBaseHP()
{
	...
	{
		// important that this uses have_skill and not lh_have_skill, as lh_have_skill would
...
boolean[skill] bat_desiredSkills(int hpLeft, boolean[skill] forcedPicks)
{
	...
	if(get_property("_lh_bat_bloodBank") != "2")
	{
...
{
	if(my_class() != $class[Vampyre] || !lh_have_skill($skill[Ensorcel]))
		return false;
...
boolean bat_consumption()
{
		 ...
		 (bat_creatable_amount($item[blood-soaked sponge cake]) > 0)))
	{
		...
		if (item_amount($item[blood-soaked sponge cake]) > 0)
		{
			lhChew(1, $item[dieting pill]);
			lhEat(1, $item[blood-soaked sponge cake]);
			return true;
		}
	}
	if (item_amount($item[blood bag]) > 0)
...
{
	int bloodBank = get_property("_lh_bat_bloodBank").to_int();
	if(bloodBank == 0 || (bloodBank == 1 && have_skill($skill[Intimidating Aura])))
	{
		...
		set_property("_lh_bat_bloodBank", (have_skill($skill[Intimidating Aura]) ? 2 : 1));
		return true;
	}
