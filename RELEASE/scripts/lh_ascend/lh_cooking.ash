...
boolean lhEat(int howMany, item toEat);
boolean lhEat(int howMany, item toEat, boolean silent);
boolean lhDrink(int howMany, item toDrink);
boolean lhOverdrink(int howMany, item toOverdrink);
boolean lhChew(int howMany, item toChew);
boolean tryPantsEat();
boolean tryCookies();
boolean canDrink(item toDrink);
boolean canEat(item toEat);
boolean lh_maximizedConsumeStuff();

boolean keepOnTruckin()
{
	if(get_property("lh_limitConsume").to_boolean())
	{
		return false;
	}
...
float expectedAdventuresFrom(item it)
{
	...
	if(lh_have_skill($skill[Saucemaven]) && saucemavenApplies(it))
	{
...
boolean tryCookies()
{
	...
	if((lh_my_path() == "Heavy Rains") && (get_property("lh_orchard") == "finished"))
	{
		return false;
	}
	while((fullness_limit() - my_fullness()) > 0)
	{
		...
		if((item_amount($item[Mayoflex]) > 0) && (lh_get_campground() contains $item[Portable Mayo Clinic]))
		{
			use(1, $item[Mayoflex]);
		}
		buffMaintain($effect[Song of the Glorious Lunch], 10, 1, 1);
		lhEat(1, $item[Fortune Cookie]);
	...
	if(fullness_left() > 0)
	{
		foreach it in $items[Tasty Tart, Deviled Egg, Actual Tapas, Cold Mashed Potatoes, Dinner Roll, Whole Turkey Leg, Can of Sardines, High-Calorie Sugar Substitute, Pat of Butter]
		{
			...
			if(item_amount(it) > 0)
			{
				...
				if((get_property("mayoInMouth") == "") && (lh_get_campground() contains $item[Portable Mayo Clinic]))
				{
...
boolean lhDrink(int howMany, item toDrink)
{
	...
	if(canOde(toDrink) && lh_have_skill($skill[The Ode to Booze]))
	{
	...
	while(howMany > 0)
	{
		...
		if(retval)
		{
			handleTracker(toDrink, "lh_drunken");
		}
		howMany = howMany - 1;
	}
...
string cafeFoodName(int id)
{
	...
	default: abort("lhDrinkCafe does not recognize item id: " + id);
	}
	return "";
}
...
boolean lhDrinkCafe(int howmany, int id)
{
	...
	for (int i=0; i<howmany; i++)
	{
		...
		handleTracker(name, "lh_drunken");
	}
	return true;
}

boolean lhEatCafe(int howmany, int id)
{
...
boolean lhChew(int howMany, item toChew)
{
	...
	if(retval)
	{
		for(int i = 0; i < howMany; ++i)
		{
			handleTracker(toChew, "lh_chewed");
		}
	}
...
boolean lhEat(int howMany, item toEat)
{
	return lhEat(howMany, toEat, true);
}
...
boolean lhEat(int howMany, item toEat, boolean silent)
{
	...
	while(howMany > 0)
	{
		...
		if((lh_get_campground() contains $item[Portable Mayo Clinic]) && (my_meat() > 11000) && (get_property("mayoInMouth") == "") && is_unrestricted($item[Portable Mayo Clinic]))
		{
...
boolean canDrink(item toDrink)
{
	...
	if(!lh_is_valid(toDrink))
	{
		return false;
	}
	if((lh_my_path() == "Nuclear Autumn") && (toDrink.inebriety != 1))
	{
		return false;
	}
	if((lh_my_path() == "Dark Gyffte") != ($items[vampagne, dusty bottle of blood, Red Russian, mulled blood, bottle of Sanguiovese] contains toDrink))
	{
		return false;
	}
	if(lh_my_path() == "KOLHS")
...
boolean canEat(item toEat)
{
	...
	if(!lh_is_valid(toEat))
	{
		return false;
	}
	if((lh_my_path() == "Nuclear Autumn") && (toEat.fullness != 1))
	{
		return false;
	}
	if((lh_my_path() == "Dark Gyffte") && (toEat == $item[magical sausage]))
	{
		// the one thing you can eat as Vampyre AND other classes
		return true;
	}
	if((lh_my_path() == "Dark Gyffte") != ($items[blood-soaked sponge cake, blood roll-up, blood snowcone, actual blood sausage, bloodstick] contains toEat))
	{
		return false;
	}
	if(lh_my_path() == "Zombie Slayer")
	{
...
void consumeStuff()
{
	...
	lh_sausageGrind(23 - get_property("_sausagesMade").to_int());
	lh_sausageEatEmUp();
	...
	if (lh_beta() && !get_property("lh_legacyConsumeStuff").to_boolean())
	{
		lh_maximizedConsumeStuff();
		return;
	}
	else
	{
		// print("Using old hard-coded consumption strategies. 'set lh_legacyConsumeStuff=false' to use the knapsack-solver consumption strategy.", "red");
	}

	int mpForOde = mp_cost($skill[The Ode to Booze]);
	if(!lh_have_skill($skill[The Ode to Booze]))
	{
		mpForOde = 0;
	}
	...
	if((my_inebriety() <= 8) || (my_adventures() < 20) || hasSpookyravenLibraryKey() || (get_property("questM20Necklace") == "finished"))
	{
		...
		if(((my_inebriety() + item_amount($item[Sacramento Wine])) <= inebriety_limit()) && (my_mp() >= mpForOde) && is_unrestricted($item[Witchess Set]) && canDrink($item[Sacramento Wine]))
		{
			if(item_amount($item[Sacramento Wine]) > 0)
			{
				lhDrink(min(item_amount($item[Sacramento Wine]), have_effect($effect[Ode to Booze])), $item[Sacramento Wine]);
			}
		}
	...
	if((fullness_left() >= 4) && (my_level() >= 4) && is_unrestricted($item[Browser Cookie]) && canEat($item[Browser Cookie]))
	{
		...
		lhEat(browserCookies, $item[Browser Cookie]);
	}
	...
	if(my_daycount() == 1)
	{
		...
		{
			lhChew(1, $item[Grim Fairy Tale]);
		}

		if(!contains_text(get_counters("Fortune Cookie", 0, 200), "Fortune Cookie") && (my_turncount() < 70) && (fullness_left() > 0) && (my_meat() >= npc_price($item[Fortune Cookie])) && (item_amount($item[Deck of Every Card]) == 0) && (item_amount($item[Stone Wool]) < 2) && !(lh_get_clan_lounge() contains $item[Clan Speakeasy]))
		{
		...
		if((my_meat() > 400) && (item_amount($item[Handful of Smithereens]) == 3) && (get_property("lh_mosquito") == "finished") && (internalQuestStatus("questL03Rat") >= 0) && canDrink($item[Paint A Vulgar Pitcher]))
		{
		...
		{
			lhDrink(2, $item[Agitated Turkey]);
		}
		...
		{
			lhDrink(1, $item[Paint A Vulgar Pitcher]);
		...
		if((get_property("lh_ballroomsong") == "finished") && (inebriety_left() >= 2) && (get_property("_speakeasyDrinksDrunk").to_int() == 1) && (my_mp() >= (mpForOde+30)) && ((my_inebriety() + 2) <= inebriety_limit()) && !($classes[Avatar of Boris, Ed] contains my_class()))
		{
		...
		if((my_adventures() < 4) && (my_fullness() == 0) && (my_level() >= 7) && !in_hardcore() && can_eat())
		{
			...
			if((item_amount($item[Spaghetti Breakfast]) > 0) && (fullness_left() >= 1) && canEat($item[Spaghetti Breakfast]))
			{
				...
				buffMaintain($effect[Song of the Glorious Lunch], 10, 1, 1);
				lhEat(1, $item[Spaghetti Breakfast]);
			}
			...
			if((item_amount($item[Digital Key Lime Pie]) > 0) && (fullness_left() >= 4) && canEat($item[Digital Key Lime Pie]))
			{
				buffMaintain($effect[Song of the Glorious Lunch], 10, 1, 1);
				buffMaintain($effect[Got Milk], 0, 1, 1);
				lhEat(1, $item[Digital Key Lime Pie]);
				tryPantsEat();
			}
			else
			{
		...
		if((my_adventures() < 4) && (fullness_left() >= 12) && (my_level() >= 6) && (item_amount($item[Boris\'s Key Lime Pie]) > 0) && (item_amount($item[Jarlsberg\'s Key Lime Pie]) > 0) && (item_amount($item[Sneaky Pete\'s Key Lime Pie]) > 0) && !in_hardcore() && canEat($item[Boris\'s Key Lime Pie]))
		{
			...
			buffMaintain($effect[Got Milk], 0, 1, 1);
			buffMaintain($effect[Song of the Glorious Lunch], 10, 1, 1);
			lhEat(1, $item[Boris\'s Key Lime Pie]);
			lhEat(1, $item[Jarlsberg\'s Key Lime Pie]);
			lhEat(1, $item[Sneaky Pete\'s Key Lime Pie]);
			tryPantsEat();
			tryPantsEat();
			tryPantsEat();
		}
	...
	else if(my_daycount() == 2)
	{
		if((my_level() >= 7) && (my_fullness() == 0) && ((my_adventures() < 10) || (get_counters("Fortune Cookie", 0, 5) == "Fortune Cookie") || (get_counters("Fortune Cookie", 0, 200) != "Fortune Cookie") || (get_property("middleChamberUnlock").to_boolean())) && !in_hardcore())
		{
			...
			else if(!get_property("lh_useCubeling").to_boolean())
			{
		...
		if((my_inebriety() == 0) && (my_mp() >= mpForOde) && (my_meat() > 300) && (item_amount($item[Handful of Smithereens]) >= 2) && (internalQuestStatus("questL03Rat") >= 0) && canDrink($item[Paint A Vulgar Pitcher]))
		{
			...
			lhDrink(2, $item[Paint A Vulgar Pitcher]);
		}
		...
		if((inebriety_left() >= 5) && (my_adventures() < 10) && (my_meat() > 150) && (my_mp() >= mpForOde) && (lh_my_path() != "KOLHS") && (lh_my_path() != "Nuclear Autumn"))
		{
		...
		{
			lhDrink(1, $item[Dinsey Whinskey]);
		}

		if((my_level() >= 11) && (my_mp() > mpForOde) && (item_amount($item[Cold One]) > 1) && (inebriety_left() >= 2) && canDrink($item[Cold One]))
		{
			lhDrink(2, $item[Cold One]);
		}

		if((my_inebriety() >= 6) && (my_inebriety() <= 11) && (get_property("lh_orchard") == "finished") && (my_mp() >= mpForOde) && is_unrestricted($item[Fist Turkey Outline]))
		{
			if((get_property("lh_nuns") != "finished") && (get_property("lh_nuns") != "done") && (get_property("currentNunneryMeat").to_int() == 0))
			{
		...
		if((lh_my_path() == "Picky") && (my_mp() > mpForOde) && (my_meat() > 150) && (item_amount($item[Paint A Vulgar Pitcher]) > 0) && ((my_inebriety() + 2) <= inebriety_limit()) && canDrink($item[Paint A Vulgar Pitcher]))
		{
			lhDrink(1, $item[Paint A Vulgar Pitcher]);
		}


		if((lh_my_path() == "Picky") && (my_mp() > mpForOde) && (my_meat() > 150) && (item_amount($item[Ambitious Turkey]) > 0) && ((my_inebriety() + 1) <= inebriety_limit()) && canDrink($item[Ambitious Turkey]))
		{
			lhDrink(1, $item[Ambitious Turkey]);
		}

		if((lh_my_path() == "Picky") && (my_mp() > mpForOde) && (inebriety_left() > 0) && (my_meat() > 500) && (get_property("_speakeasyDrinksDrunk").to_int() < 3) && (item_amount($item[Clan VIP Lounge Key]) > 0))
		{
...
/*****	This section needs to merge into a "Standard equivalent"		*****/
		if((lh_my_path() == "Standard") && (my_mp() >= mpForOde) && (my_meat() > 150) && (item_amount($item[Paint A Vulgar Pitcher]) > 0) && ((my_inebriety() + 2) <= inebriety_limit()) && is_unrestricted($item[The Smith\'s Tome]) && canDrink($item[Paint A Vulgar Pitcher]))
		{
			lhDrink(1, $item[Paint A Vulgar Pitcher]);
		}


		if((lh_my_path() == "Standard") && (my_mp() >= mpForOde) && (item_amount($item[Ambitious Turkey]) > 0) && ((my_inebriety() + 1) <= inebriety_limit()) && is_unrestricted($item[Fist Turkey Outline]) && canDrink($item[Ambitious Turkey]))
		{
			lhDrink(1, $item[Ambitious Turkey]);
		}

		if((lh_my_path() == "Standard") && (my_mp() >= mpForOde) && (my_inebriety() <= inebriety_limit()) && (my_meat() > 500) && (get_property("_speakeasyDrinksDrunk").to_int() < 3))
		{
			# We could check for good drinks here but I don't know what would be good checks
	...
	else if(my_daycount() == 3)
	{
		if((my_level() >= 7) && (my_fullness() == 0) && (my_adventures() < 10) && canEat($item[Star Key Lime Pie]))
		{
			...
			if((item_amount($item[Star Key Lime Pie]) >= 3) && (fullness_left() >= 12) && canEat($item[Star Key Lime Pie]))
			{
				buffMaintain($effect[Song of the Glorious Lunch], 10, 1, 1);
				buffMaintain($effect[Got Milk], 0, 1, 1);
				lhEat(3, $item[Star Key Lime Pie]);
				tryPantsEat();
				tryPantsEat();
				tryPantsEat();
			}
		...
		if((lh_my_path() == "Picky") && (my_mp() > mpForOde) && (item_amount($item[Paint A Vulgar Pitcher]) > 0) && ((my_inebriety() + 2) <= inebriety_limit()) && canDrink($item[Paint A Vulgar Pitcher]))
		{
			lhDrink(1, $item[Paint A Vulgar Pitcher]);
		}

		if((lh_my_path() == "Picky") && (my_mp() > mpForOde) && (item_amount($item[Ambitious Turkey]) > 0) && ((my_inebriety() + 1) <= inebriety_limit()) && canDrink($item[Ambitious Turkey]))
		{
			lhDrink(1, $item[Ambitious Turkey]);
		}
	}
...
int lh_ORGAN_STOMACH = 1;
int lh_ORGAN_LIVER   = 2;

int lh_OBTAIN_NULL  = 100;
int lh_OBTAIN_CRAFT = 101;
int lh_OBTAIN_PULL  = 102;
int lh_OBTAIN_BUY   = 103;
...
string consumable_name(ConsumeAction action)
{
	...
	else if (action.organ == lh_ORGAN_LIVER) name = cafeDrinkName(action.cafeId);
	else if (action.organ == lh_ORGAN_STOMACH) name = cafeFoodName(action.cafeId);
	return name;
}

string to_pretty_string(ConsumeAction action)
{
	string organ_name = action.organ == lh_ORGAN_STOMACH ? "fullness" : "inebriety";
	string logline = consumable_name(action) + " for " + action.adventures + " base adv (" + action.size + " " + organ_name + ")";
	if (action.howToGet == lh_OBTAIN_PULL)
	{
		logline += " [PULL]";
	}
	if (action.howToGet == lh_OBTAIN_CRAFT)
	{
		logline += " [CRAFT]";
	}
	if (action.howToGet == lh_OBTAIN_BUY)
	{
		logline += " [BUY]";
	}
	return logline;
...
{
	int organ = it.inebriety > 0 ? lh_ORGAN_LIVER : lh_ORGAN_STOMACH;
	int size = max(it.inebriety, it.fullness);
	float adv = expectedAdventuresFrom(it);
	return new ConsumeAction(it, 0, size, adv, adv, organ, lh_OBTAIN_NULL);
}
...
{
	if (action.howToGet != lh_OBTAIN_NULL)
	{
		abort("ConsumeAction not prepped: " + to_debug_string(action));
	}

	if (action.organ == lh_ORGAN_LIVER)
	{
	...
	if(action.cafeId != 0)
	{
		if (action.organ == lh_ORGAN_LIVER)
		{
			return lhDrinkCafe(1, action.cafeId);
		}
		else if (action.organ == lh_ORGAN_STOMACH)
		{
			return lhEatCafe(1, action.cafeId);
		}
	}
	else if(action.it != $item[none])
	{
		if (action.organ == lh_ORGAN_LIVER)
		{
			return lhDrink(1, action.it);
		}
		else if (action.organ == lh_ORGAN_STOMACH)
		{
			return lhEat(1, action.it);
		}
		else
		{
...
boolean loadConsumables(string _type, ConsumeAction[int] actions)
{
	...
	int type  = 0;
	if (_type == "eat")   type = lh_ORGAN_STOMACH;
	else if (_type == "drink") type = lh_ORGAN_LIVER;
	else return false;

	boolean canConsume(item it)
	{
		return type == lh_ORGAN_STOMACH ? canEat(it) : canDrink(it);
	}

	int organLeft()
	{
		return type == lh_ORGAN_STOMACH ? fullness_left() : inebriety_left();
	}

	int organCost(item it)
	{
		return type == lh_ORGAN_STOMACH ? it.fullness : it.inebriety;
	}
	...
	foreach it in $items[]
	{
			...
			(historical_price(it) <= 20000 || (KEY_LIME_PIES contains it && historical_price(it) < 40000)))
		{
			...
			{
				small_owned[it] = min(max(item_amount(it) - lh_reserveAmount(it), 0), howmany);
			}
			if (npc_price(it) > 0)
			{
			...
			if (item_amount(it) > 0 && organCost(it) > 5)
			{
				large_owned[it] = min(max(item_amount(it) - lh_reserveAmount(it), 0), howmany);
			}
			...
			{
				craftables[it] = min(howmany, max(0, creatable_amount(it) - lh_reserveCraftAmount(it)));
			}
	...
	void add(item it, int obtain_mode, int howmany)
	{
		for (int i = 0; i < howmany; i++)
		{
			...
			if (obtain_mode == lh_OBTAIN_CRAFT)
			{
	...
	{
		add(it, lh_OBTAIN_PULL, howmany);
	}
	foreach it, howmany in small_owned
	{
		add(it, lh_OBTAIN_NULL, howmany);
	}
	foreach it, howmany in buyables
	{
		add(it, lh_OBTAIN_BUY, howmany);
	}
	...
	{
		add(it, lh_OBTAIN_CRAFT, howmany);
	}

	// Now, to load cafe consumables. This has some TCRS-specific code.

	if(type == lh_ORGAN_LIVER && !gnomads_available()) return false;
	if(type == lh_ORGAN_STOMACH && !canadia_available()) return false;
	...
	if(!in_tcrs()) 
	{
		...
		if(type == lh_ORGAN_LIVER)
		{
			...
			for (int i=0; i < limit; i++)
			{
				int size = 3;
				float adv = 11.0/3.0;
				actions[count(actions)] = new ConsumeAction($item[none], -3, size, adv, adv, lh_ORGAN_LIVER, lh_OBTAIN_NULL);
			}
		}
		if(type == lh_ORGAN_STOMACH)
		{
			...
			for (int i=0; i < limit; i++)
			{
				int size = 3;
				float adv = 3.5;
				actions[count(actions)] = new ConsumeAction($item[none], -1, size, adv, adv, lh_ORGAN_LIVER, lh_OBTAIN_NULL);
			}
			...
			for (int i=0; i < limit; i++)
			{
				int size = 4;
				float adv = 5.0;
				actions[count(actions)] = new ConsumeAction($item[none], -2, size, adv, adv, lh_ORGAN_LIVER, lh_OBTAIN_NULL);
			}
...
void lh_autoDrinkNightcap(boolean simulate)
{
	ConsumeAction[int] actions;
	loadConsumables("drink", actions);

	boolean have_ode = lh_have_skill($skill[The Ode to Booze]);
...
boolean lh_autoDrinkOne(boolean simulate)
{
	...
	print("lh_autoDrinkOne: Planning to execute " + to_pretty_string(actions[best]), "blue");
...
boolean lh_knapsackAutoConsume(string type, boolean simulate)
{
	...
	foreach i in result
	{
		...
		if (actions[i].it != $item[none] && actions[i].howToGet != lh_OBTAIN_PULL)
		{
	...
	if (type == "drink" && lh_have_skill($skill[The Ode to Booze]))
	{
	...
	if(type == "eat")
	{
		if (in_tcrs() && get_property("lh_useWishes").to_boolean() && (0 == have_effect($effect[Got Milk])))
		{
...
boolean lh_maximizedConsumeStuff()
{
	if(my_adventures() < 10)
	{
		if(my_inebriety() < 8 && inebriety_left() > 0)
		{
			...
			return lh_autoDrinkOne(false);
		}
		if(inebriety_left() > 0)
		{
			use_familiar($familiar[none]);
			return lh_knapsackAutoConsume("drink", false);
		}
		if(fullness_left() > 0)
		{
			return lh_knapsackAutoConsume("eat", false);
		}
	}
	return false;
}
