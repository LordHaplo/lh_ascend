...
boolean volcano_lavaDogs()
{
	...
	if(!(lh_get_campground() contains $item[Haunted Doghouse]))
	{
		return false;
	}
	...
	if(!get_property("calderaVolcoino").to_boolean())
	{
		...
		lhAdv($location[The Bubblin\' Caldera]);
...
boolean volcano_bunkerJob()
{
	...
	int ticketValue = lh_mall_price($item[One-Day Ticket To That 70s Volcano]) / 3;
	int option = 0;
	...
	foreach goal in $strings[_volcanoItem1, _volcanoItem2, _volcanoItem3]
	{
		...
		{
			currentCost = lh_mall_price(it) * get_property("_volcanoItemCount" + index).to_int();
		}
		else if(it == $item[Smooth Velvet Bra])
		{
			currentCost = 3 * lh_mall_price($item[Unsmoothed Velvet]) * get_property("_volcanoItemCount" + index).to_int();
		}
		else if(it == $item[SMOOCH Bracers])
		{
			currentCost = 5 * lh_mall_price($item[Superheated Metal]) * get_property("_volcanoItemCount" + index).to_int();
		}
	...
	if((option != 0) && ((optionCost * 1.5) < ticketValue))
	{
		...
		if($items[Gooey Lava Globs, New Age Healing Crystal, SMOOCH Bottlecap, Superduperheated Metal] contains it)
		{
			buy(get_property("_volcanoItemCount" + option).to_int(), it, lh_mall_price(it)*2);
		}
		else if(it == $item[Smooth Velvet Bra])
		{
			buy(get_property("_volcanoItemCount" + option).to_int() * 3, $item[Unsmoothed Velvet], lh_mall_price($item[Unsmoothed Velvet])*2);
			cli_execute("make 3 " + it);
		}
		else if(it == $item[SMOOCH Bracers])
		{
			buy(get_property("_volcanoItemCount" + option).to_int() * 5, $item[Superheated Metal], lh_mall_price($item[Superheated Metal])*2);
			cli_execute("make 3 " + it);
		}
