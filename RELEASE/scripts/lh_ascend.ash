script "lh_ascend.ash";
notify LordHaplo;
since r19516; // haunted paddle-ball and Jacob's rug help in the exploaded battlefield
/***
	Killing is wrong, and bad. There should be a new, stronger word for killing like badwrong or badong. YES, killing is badong. From this moment, I will stand for the opposite of killing, gnodab.

	lh_ascend_header.ash must be first import
	All non-accessory scripts must be imported here

	Accessory scripts can import lh_ascend.ash

***/


import <lh_ascend/lh_ascend_header.ash>
import <lh_ascend/lh_util.ash>
import <lh_ascend/lh_deprecation.ash>
import <lh_ascend/lh_combat.ash>
import <lh_ascend/lh_floristfriar.ash>
import <lh_ascend/lh_equipment.ash>
import <lh_ascend/lh_eudora.ash>
import <lh_ascend/lh_elementalPlanes.ash>
import <lh_ascend/lh_clan.ash>
import <lh_ascend/lh_cooking.ash>
import <lh_ascend/lh_adventure.ash>
import <lh_ascend/lh_mr2012.ash>
import <lh_ascend/lh_mr2014.ash>
import <lh_ascend/lh_mr2015.ash>
import <lh_ascend/lh_mr2016.ash>
import <lh_ascend/lh_mr2017.ash>
import <lh_ascend/lh_mr2018.ash>
import <lh_ascend/lh_mr2019.ash>

import <lh_ascend/lh_boris.ash>
import <lh_ascend/lh_jellonewbie.ash>
import <lh_ascend/lh_fallout.ash>
import <lh_ascend/lh_community_service.ash>
import <lh_ascend/lh_sneakypete.ash>
import <lh_ascend/lh_heavyrains.ash>
import <lh_ascend/lh_picky.ash>
import <lh_ascend/lh_standard.ash>
import <lh_ascend/lh_edTheUndying.ash>
import <lh_ascend/lh_summerfun.ash>
import <lh_ascend/lh_awol.ash>
import <lh_ascend/lh_bondmember.ash>
import <lh_ascend/lh_groundhog.ash>
import <lh_ascend/lh_digimon.ash>
import <lh_ascend/lh_majora.ash>
import <lh_ascend/lh_glover.ash>
import <lh_ascend/lh_batpath.ash>
import <lh_ascend/lh_tcrs.ash>
import <lh_ascend/lh_koe.ash>
import <lh_ascend/lh_monsterparts.ash>
import <lh_ascend/lh_theSource.ash>
import <lh_ascend/lh_optionals.ash>
import <lh_ascend/lh_list.ash>
import <lh_ascend/lh_zlib.ash>
import <lh_ascend/lh_zone.ash>


void initializeSettings()
{
	if(my_ascensions() <= get_property("lh_doneInitialize").to_int())
	{
		return;
	}
	...
	if(my_familiar() != $familiar[none])
	{
		...
		if(userAnswer)
		{
			set_property("lh_100familiar", my_familiar());
		}
		if(is100FamiliarRun())
		{
	...
	if(!is100FamiliarRun() && lh_have_familiar($familiar[Crimbo Shrub]))
	{
	...
	set_property("lh_aboocount", "0");
	set_property("lh_aboopending", 0);
	set_property("lh_aftercore", false);
	set_property("lh_airship", "");
	set_property("lh_ballroom", "");
	set_property("lh_ballroomflat", "");
	set_property("lh_ballroomopen", "");
	set_property("lh_ballroomsong", "");
	set_property("lh_banishes", "");
	set_property("lh_bat", "");
	set_property("lh_batoomerangDay", 0);
	set_property("lh_bean", false);
	set_property("lh_beatenUpCount", 0);
	set_property("lh_getBeehive", false);
	set_property("lh_blackfam", true);
	set_property("lh_blackmap", "");
	set_property("lh_boopeak", "");
	set_property("lh_breakstone", get_property("lh_pvpEnable").to_boolean());
	set_property("lh_bruteForcePalindome", false);
	set_property("lh_cabinetsencountered", 0);
	set_property("lh_castlebasement", "");
	set_property("lh_castleground", "");
	set_property("lh_castletop", "");
	set_property("lh_chasmBusted", true);
	set_property("lh_chewed", "");
	set_property("lh_clanstuff", "0");
	set_property("lh_combatHandler", "");
	set_property("lh_consumption", "");
	set_property("lh_cookie", -1);
	set_property("lh_copies", "");
	set_property("lh_copperhead", 0);
	set_property("lh_crackpotjar", "");
	set_property("lh_crypt", "");
	set_property("lh_cubeItems", true);
	set_property("lh_dakotaFanning", false);
	set_property("lh_day_init", 0);
	set_property("lh_day1_cobb", "");
	set_property("lh_day1_dna", "");
	set_property("lh_disableAdventureHandling", false);
	set_property("lh_doCombatCopy", "no");
	set_property("lh_drunken", "");
	set_property("lh_eaten", "");
	set_property("lh_familiarChoice", $familiar[none]);
	set_property("lh_fcle", "");
	set_property("lh_friars", "");
	set_property("lh_funTracker", "");
	set_property("lh_getBoningKnife", false);
	set_property("lh_getStarKey", false);
	set_property("lh_getSteelOrgan", get_property("lh_alwaysGetSteelOrgan").to_boolean());
	set_property("lh_gnasirUnlocked", false);
	set_property("lh_goblinking", "");
	set_property("lh_gremlins", "");
	set_property("lh_gremlinBanishes", "");
	set_property("lh_grimstoneFancyOilPainting", true);
	set_property("lh_grimstoneOrnateDowsingRod", true);
	set_property("lh_gunpowder", "");
	set_property("lh_haveoven", false);
	set_property("lh_haveSourceTerminal", false);
	set_property("lh_hedge", "fast");
	set_property("lh_hiddenapartment", "0");
	set_property("lh_hiddenbowling", "");
	set_property("lh_hiddencity", "");
	set_property("lh_hiddenhospital", "");
	set_property("lh_hiddenoffice", "");
	set_property("lh_hiddenunlock", "");
	set_property("lh_hiddenzones", "");
	set_property("lh_highlandlord", "");
	set_property("lh_hippyInstead", false);
	set_property("lh_holeinthesky", true);
	set_property("lh_ignoreCombat", "");
	set_property("lh_ignoreFlyer", false);
	set_property("lh_instakill", "");
	set_property("lh_masonryWall", false);
	set_property("lh_mcmuffin", "");
	set_property("lh_mistypeak", "");
	set_property("lh_modernzmobiecount", "");
	set_property("lh_mosquito", "");
	set_property("lh_nuns", "");
	set_property("lh_oilpeak", "");
	set_property("lh_orchard", "");
	set_property("lh_otherstuff", "");
	set_property("lh_palindome", "");
	set_property("lh_paranoia", -1);
	set_property("lh_paranoia_counter", 0);
	set_property("lh_phatloot", "");
	set_property("lh_prewar", "");
	set_property("lh_prehippy", "");
	set_property("lh_pirateoutfit", "");
	set_property("lh_priorCharpaneMode", "0");
	set_property("lh_powerLevelLastLevel", "0");
	set_property("lh_powerLevelAdvCount", "0");
	set_property("lh_powerLevelLastAttempted", "0");
	set_property("lh_pulls", "");
	set_property("lh_shenCopperhead", true);
	set_property("lh_skipDesert", 0);
	set_property("lh_snapshot", "");
	set_property("lh_sniffs", "");
	set_property("lh_spookyfertilizer", "");
	set_property("lh_spookymap", "");
	set_property("lh_spookyravensecond", "");
	set_property("lh_spookysapling", "");
	set_property("lh_sonofa", "");
	set_property("lh_sorceress", "");
	set_property("lh_swordfish", "");
	set_property("lh_tavern", "");
	set_property("lh_trytower", "");
	set_property("lh_trapper", "");
	set_property("lh_treecoin", "");
	set_property("lh_twinpeak", "");
	set_property("lh_twinpeakprogress", "");
	set_property("lh_waitingArrowAlcove", "50");
	set_property("lh_wandOfNagamar", true);
	set_property("lh_war", "");
	set_property("lh_winebomb", "");
	set_property("lh_wineracksencountered", 0);
	set_property("lh_wishes", "");
	set_property("lh_writingDeskSummon", false);
	set_property("lh_yellowRays", "");

	set_property("choiceAdventure1003", 0);
	beehiveConsider();

	lh_sourceTerminalEducate($skill[Extract], $skill[Digitize]);
	if(contains_text(get_property("sourceTerminalEnquiryKnown"), "monsters.enq") && (lh_my_path() == "Pocket Familiars"))
	{
		lh_sourceTerminalRequest("enquiry monsters.enq");
	}
	else if(contains_text(get_property("sourceTerminalEnquiryKnown"), "familiar.enq") && lh_have_familiar($familiar[Mosquito]))
	{
		lh_sourceTerminalRequest("enquiry familiar.enq");
	}
	else if(contains_text(get_property("sourceTerminalEnquiryKnown"), "stats.enq"))
	{
		lh_sourceTerminalRequest("enquiry stats.enq");
	}
	else if(contains_text(get_property("sourceTerminalEnquiryKnown"), "protect.enq"))
	{
		lh_sourceTerminalRequest("enquiry protect.enq");
	}
	...
	set_property("lh_doneInitialize", my_ascensions());
}
...
boolean handleFamiliar(string type)
{
	...
	int ascensionThreshold = get_property("lh_ascensionThreshold").to_int();
	if(ascensionThreshold == 0)
	{
		ascensionThreshold = 100;
	}

	if(get_property("lh_blacklistFamiliar") != "")
	{
		string[int] noFams = split_string(get_property("lh_blacklistFamiliar"), ";");
	...
	if(lh_beta())
	{
		...
		if(!file_to_map("lh_ascend_familiars.txt", familiars_text))
			print("Could not load lh_ascend_familiars.txt. This is bad!", "red");
		foreach i,name,conds in familiars_text[type]
		{
			...
			if(!lh_check_conditions(conds))
				continue;
			if(!lh_have_familiar(thisFamiliar))
				continue;
...
/*
	if((type == "item") && (get_property("lh_beatenUpCount").to_int() > 5))
	{
...
#		}
		...
		{
			if(lh_have_familiar(fams[index]) && !(blacklist contains fams[index]))
			{
	...
	else if(type == "item")
	{
		...
		if((my_daycount() == 1) && ($familiar[Angry Jung Man].drops_today == 0) && (get_property("lh_crackpotjar") == ""))
		{
		...
		if((my_ascensions() > ascensionThreshold) && ((!get_property("lh_grimstoneFancyOilPainting").to_boolean() && !get_property("lh_grimstoneOrnateDowsingRod").to_boolean()) || possessEquipment($item[Buddy Bjorn]) || ($familiar[Grimstone Golem].drops_today == 1)))
		{
		...
		if((my_ascensions() > ascensionThreshold) && ((get_property("lh_crackpotjar") != "") || ($familiar[Angry Jung Man].drops_today == 1)))
		{
			fams = ListRemove(fams, $familiar[Angry Jung Man]);
		...
		if(lh_have_familiar($familiar[Pair of Stomping Boots]) && get_property("_bootStomps").to_int() < 7)
		{
		...
		if(($familiar[Fist Turkey].drops_today >= $familiar[Fist Turkey].drops_limit) && (lh_my_path() != "Teetotaler"))
		{
		...
		if((item_amount($item[Yellow Pixel]) < 30) && lh_have_familiar($familiar[Ms. Puck Man]))
		{
			fams = ListInsertAt(fams, $familiar[Ms. Puck Man], 0);
		}
		else if((item_amount($item[Yellow Pixel]) < 30) && lh_have_familiar($familiar[Puck Man]))
		{
...
#		foreach fam in $familiars[Rockin\' Robin, Grimstone Golem, Angry Jung Man, Intergnat, Bloovian Groose, Fist Turkey, Slimeling, Jumpsuited Hound Dog, Adventurous Spelunker, Gelatinous Cubeling, Baby Gravy Fairy, Obtuse Angel, Pair of Stomping Boots, Jack-in-the-Box, Syncopated Turtle]
		{
			#if(lh_have_familiar(fam) && !(blacklist contains fam))
...
boolean handleFamiliar(familiar fam)
{
	...
	if((fam == $familiar[Ms. Puck Man]) && !lh_have_familiar($familiar[Ms. Puck Man]) && lh_have_familiar($familiar[Puck Man]))
	{
		fam = $familiar[Puck Man];
	}
	if((fam == $familiar[Puck Man]) && !lh_have_familiar($familiar[Puck Man]) && lh_have_familiar($familiar[Ms. Puck Man]))
	{
	...
	else
	{
		...
		if(in_hardcore() && ((my_spleen_use() + spleen_hold) <= spleen_limit()))
		{
			...
			{
				if((fam.drops_today < 1) && lh_have_familiar(fam))
				{
					toEquip = fam;
				}
			}
		}
		else if(in_hardcore() && (item_amount($item[Yellow Pixel]) < 30) && lh_have_familiar($familiar[Ms. Puck Man]))
		{
			toEquip = $familiar[Ms. Puck Man];
		}
		else if(in_hardcore() && (item_amount($item[Yellow Pixel]) < 30) && lh_have_familiar($familiar[Puck Man]))
		{
		...
		{
			if((lh_have_familiar(thing)) && (my_bjorned_familiar() != thing))
			{
				toEquip = thing;
			}
		}
...
{
	if(($familiars[Adventurous Spelunker, Rockin\' Robin] contains my_familiar()) && lh_have_familiar($familiar[Grimstone Golem]) && (in_hardcore() || !possessEquipment($item[Buddy Bjorn])))
	{
	...
	if((spleen_left() >= (4 + spleen_hold)) && haveSpleenFamiliar())
	{
		...
		if(spleenHave > 0)
		{
			...
			foreach fam in $familiars[Baby Sandworm, Bloovian Groose, Golden Monkey, Grim Brother, Unconscious Collective]
			{
				if((fam.drops_today < bound) && lh_have_familiar(fam))
				{
					handleFamiliar(fam);
					break;
				}
	...
	else if((item_amount($item[Yellow Pixel]) < 20) && (lh_have_familiar($familiar[Ms. Puck Man]) || lh_have_familiar($familiar[Puck Man])))
	{
...
{
#	if((get_property("chateauMonster") == $monster[Writing Desk]) && (lh_get_campground() contains $item[Source Terminal]))
#	{
...
boolean LX_witchess()
{
	...
	{
		return lh_advWitchess("ml");
	}

	if(lh_my_path() == "License to Adventure")
	{
		return lh_advWitchess("ml");
	}
...
{
	/* Note that this is called from preHaps.ash - WE WILL OVERRIDE FAMILIAR IN
	 * PREADVENTURE IF WE NEED THE BURGLE.
	 */
	item[monster] wannaHeists;

	item oreGoal = to_item(get_property("trapperOre"));
	if((oreGoal != $item[none]) && (item_amount(oreGoal) < 3) && get_property("lh_trapper") == "start" && in_hardcore())
	...
	{
		if(!possessEquipment($item[amulet of extreme plot significance]) && get_property("lh_castlebasement") != "finished")
			wannaHeists[$monster[Quiet Healer]] = $item[amulet of extreme plot significance];
		if(!possessEquipment($item[Mohawk wig]) && get_property("lh_castletop") != "finished")
			wannaHeists[$monster[Burly Sidekick]] = $item[Mohawk wig];
	}

	// foreach mon, it in wannaHeists
	// {
	//	lh_debug_print("catBurglarHeistDesires(): Want to heist a " + it + " from a " + mon);
	// }
	return wannaHeists;
}
...
void maximize_hedge()
{
	...
	if((first == $element[none]) || (second == $element[none]) || (third == $element[none]))
	{
		...
		{
			lhMaximize("all res -equip snow suit", 2500, 0, false);
		}
	}
	else
	{
		...
		{
			lhMaximize(to_string(first) + " res, " + to_string(second) + " res, " + to_string(third) + " res -equip snow suit", 2500, 0, false);
		}
	}
...
int pullsNeeded(string data)
{
	...
	if((my_class() == $class[Ed]) || (lh_my_path() == "Community Service"))
	{
		return 0;
	}
	...
	int progress = 0;
	if(get_property("lh_sorceress") == "hedge")
	{
		progress = 1;
	}
	if(get_property("lh_sorceress") == "door")
	{
		progress = 2;
	}
	if(get_property("lh_sorceress") == "tower")
	{
		progress = 3;
	}
	if(get_property("lh_sorceress") == "top")
	{
		progress = 4;
	}
...
//		Note: Maximizer gives concert White-boy angst, instead of concert 3 (consequently, it doesn\'t work).
	...
	if(progress < 3)
	{
		if((item_amount($item[Richard\'s Star Key]) == 0) && (item_amount($item[Star Chart]) == 0))
		{
			...
			if((lh_my_path() == "Heavy Rains") && (my_rain() >= 50))
			{
	...
	{
		adv = adv + 6;
		if(get_property("lh_wandOfNagamar").to_boolean() && (item_amount($item[Wand Of Nagamar]) == 0) && (cloversAvailable() == 0))
		{
...
boolean tophatMaker()
{
	...
	if(possessEquipment($item[Mark IV Steam-Hat]))
	{
		...
		lhCraft("combine", 1, $item[Brass Gear], $item[Mark IV Steam-Hat]);
	}
	else if(possessEquipment($item[Mark III Steam-Hat]))
	{
		...
		lhCraft("combine", 1, $item[Brass Gear], $item[Mark III Steam-Hat]);
	}
	else if(possessEquipment($item[Mark II Steam-Hat]))
	{
		...
		lhCraft("combine", 1, $item[Brass Gear], $item[Mark II Steam-Hat]);
	}
	else if(possessEquipment($item[Mark I Steam-Hat]))
	{
		...
		lhCraft("combine", 1, $item[Brass Gear], $item[Mark I Steam-Hat]);
	}
	else if(possessEquipment($item[Brown Felt Tophat]))
	{
		...
		lhCraft("combine", 1, $item[Brass Gear], $item[Brown Felt Tophat]);
	}
	else
	{
		return false;
...
boolean warOutfit(boolean immediate)
{
	boolean reallyWarOutfit(string toWear)
	{
		...
		{
			return lhOutfit(toWear);
		}
	}

	if(!get_property("lh_hippyInstead").to_boolean())
	{
...
{
	if(lh_have_familiar($familiar[Space Jellyfish]) && (get_property("_spaceJellyfishDrops").to_int() < 3))
	{
	...
	if(!get_property("lh_hippyInstead").to_boolean())
	{
		if(!lhAdv(1, $location[The Battlefield (Frat Uniform)]))
		{
	...
	{
		if(!lhAdv(1, $location[The Battlefield (Hippy Uniform)]))
		{
...
{
	if(lh_my_path() == "Two Crazy Random Summer" || in_koe())
	{
		set_property("lh_nuns", "finished"); // if only :(
		return false;
	}
	if(get_property("lh_nuns") == "done")
	{
		return false;
	}
	if((get_property("currentNunneryMeat").to_int() >= 100000) || (get_property("sidequestNunsCompleted") != "none") || (lh_my_path() == "Way of the Surprising Fist"))
	{
		handleBjornify($familiar[el vibrato megadrone]);
		set_property("lh_nuns", "done");
		return false;
	}
	...
	if(get_property("lh_hippyInstead").to_boolean() || (get_property("hippiesDefeated").to_int() >= 192))
	{
	...
	if((equipped_item($slot[off-hand]) != $item[Half a Purse]) && !possessEquipment($item[Half a Purse]) && (item_amount($item[Lump of Brituminous Coal]) > 0))
	{
		...
		lhCraft("smith", 1, $item[Lump of Brituminous Coal], $item[Loose purse strings]);
	}

	lhEquip($item[Half a Purse]);
	if(lh_my_path() == "Heavy Rains")
	{
		lhEquip($item[Thor\'s Pliers]);
	}
	lhEquip($item[Miracle Whip]);
	...
	if(is100FamiliarRun())
	{
		...
		{
			lhMaximize("meat drop, -equip snow suit", 1500, 0, false);
		}
	}
	else
	{
		...
		{
			lhMaximize("meat drop, -equip snow suit, switch Hobo Monkey, switch rockin' robin, switch adventurous spelunker, switch Grimstone Golem, switch Fist Turkey, switch Unconscious Collective, switch Golden Monkey, switch Angry Jung Man, switch Leprechaun,switch cat burglar", 1500, 0, false);
		}
	...
	if(get_property("lh_useWishes").to_boolean())
	{
	...
	{
		lhChew(1, $item[body spradium]);
	}
	if(have_effect($effect[meat.enh]) == 0)
	{
		if(lh_sourceTerminalEnhanceLeft() > 0)
		{
			lh_sourceTerminalEnhance("meat");
		}
	}
	...
	handleFamiliar("meat");
	if(lh_have_familiar($familiar[Trick-or-Treating Tot]) && (available_amount($item[Li\'l Pirate Costume]) > 0) && !is100FamiliarRun($familiar[Trick-or-Treating Tot]) && (lh_my_path() != "Heavy Rains"))
	{
		use_familiar($familiar[Trick-or-Treating Tot]);
		lhEquip($item[Li\'l Pirate Costume]);
...
#	}
#	if(get_property("lh_dickstab").to_boolean())
#	{
...
#	}
...

	{
		...
		if(!lhAdv(1, $location[The Themthar Hills]))
		{
			//Maybe we passed it!
...
int handlePulls(int day)
{
	...
	if(day == 1)
	{
		...
		if(((lh_my_path() == "Picky") || is100FamiliarRun()) && (item_amount($item[Deck of Every Card]) == 0) && (fullness_left() >= 4))
		{
		...
		if((my_primestat() == $stat[Muscle]) && (lh_my_path() != "Heavy Rains"))
		{
...
#		pullXWhenHaveY($item[mojo filter], 1, 0);
		...
		{
			if((item_amount($item[Deck of Every Card]) == 0) && !lh_have_skill($skill[Summon Smithsness]))
			{
				pullXWhenHaveY($item[Thor\'s Pliers], 1, 0);
			}
		...
		if(lh_have_skill($skill[Summon Smithsness]))
		{
		...
		if((lh_my_path() != "Heavy Rains") && (lh_my_path() != "License to Adventure") && !($classes[Avatar of Boris, Avatar of Jarlsberg, Avatar of Sneaky Pete, Ed] contains my_class()))
		{
		...
		if((lh_my_path() != "Pocket Familiars") && (lh_my_path() != "G-Lover"))
		{
			pullXWhenHaveY($item[Replica Bat-oomerang], 1, 0);
		}

		if((!lh_have_familiar($familiar[Grim Brother])) && (my_class() != $class[Ed]) && glover_usable($item[Unconscious Collective Dream Jar]))
		{
			pullXWhenHaveY($item[Unconscious Collective Dream Jar], 1, 0);
			if(item_amount($item[Unconscious Collective Dream Jar]) > 0)
			{
				lhChew(1, $item[Unconscious Collective Dream Jar]);
			}
		}
...
boolean doVacation()
{
	...
	return lhAdv(1, $location[The Shore\, Inc. Travel Agency]);
}
...
boolean fortuneCookieEvent()
{
	if(get_counters("Fortune Cookie", 0, 0) == "Fortune Cookie")
	{
		...
		if((goal == $location[The Hidden Temple]) && ((item_amount($item[stone wool]) >= 2) || (get_property("lh_hiddenunlock") == "nose") || (get_property("lh_hiddenunlock") == "finished") || (internalQuestStatus("questL11Worship") >= 3) || (get_property("semirareLocation") == goal) || (get_property("lastTempleUnlock").to_int() < my_ascensions()) || (get_property("lh_spookysapling") != "finished")))
		{
			goal = $location[The Castle in the Clouds in the Sky (Top Floor)];
		}

		if((goal == $location[The Castle in the Clouds in the Sky (Top Floor)]) && ((item_amount($item[Mick\'s IcyVapoHotness Inhaler]) > 0) || (get_property("lh_nuns") == "done") || (get_property("lh_nuns") == "finished") || (internalQuestStatus("questL10Garbage") < 9) || (get_property("semirareLocation") == goal) || (get_property("lastCastleTopUnlock").to_int() < my_ascensions()) || (get_property("lh_castleground") != "finished") || (get_property("sidequestNunsCompleted") != "none")))
		{
			goal = $location[The Limerick Dungeon];
		}

		#(internalQuestStatus("questL10Garbage") < 9) 
		if((goal == $location[The Limerick Dungeon]) && ((item_amount($item[Cyclops Eyedrops]) > 0) || (get_property("lh_orchard") == "start") || (get_property("lh_orchard") == "done") || (get_property("lh_orchard") == "finished") || (get_property("semirareLocation") == goal) || (get_property("lastFilthClearance").to_int() >= my_ascensions()) || (get_property("sidequestOrchardCompleted") != "none") || (get_property("currentHippyStore") != "none")))
		{
...
void initializeDay(int day)
{
	...
	if(get_property("lh_mummeryChoice") != "")
	{
		string[int] mummeryChoice = split_string(get_property("lh_mummeryChoice"), ";");
	...
	if(!get_property("_pottedTeaTreeUsed").to_boolean() && (lh_get_campground() contains $item[Potted Tea Tree]) && !get_property("kingLiberated").to_boolean())
	{
		if(get_property("lh_teaChoice") != "")
		{
			string[int] teaChoice = split_string(get_property("lh_teaChoice"), ";");
	...
	if((item_amount($item[GameInformPowerDailyPro Magazine]) > 0) && (my_daycount() == 1))
	{
		...
		lhAdv(1, $location[Video Game Level 1]);
		set_property("lh_disableAdventureHandling", false);
	...
	if((item_amount($item[GameInformPowerDailyPro Magazine]) > 0) && (my_daycount() == 2) && (lh_my_path() == "Community Service"))
	{
		visit_url("inv_use.php?pwd=&which=3&whichitem=6174", true);
		visit_url("inv_use.php?pwd=&which=3&whichitem=6174&confirm=Yep.", true);
		set_property("lh_disableAdventureHandling", true);
	...
	if(day == 1)
	{
		if(get_property("lh_day_init").to_int() < 1)
		{
			kgbSetup();
			...
			if((lh_get_clan_lounge() contains $item[Clan Floundry]) && (item_amount($item[Fishin\' Pole]) == 0))
			{
			...
			if(get_property("lh_breakstone").to_boolean())
			{
				temp = visit_url("peevpee.php?action=smashstone&pwd&confirm=on", true);
				temp = visit_url("peevpee.php?place=fight");
				set_property("lh_breakstone", false);
			}

			lh_beachCombHead("exp");
		}

		if((get_property("lastCouncilVisit").to_int() < my_level()) && (lh_my_path() != "Community Service"))
		{
			cli_execute("counters");
			council();
		}
	...
	else if(day == 2)
	{
		...
		if(get_property("lh_day_init").to_int() < 2)
		{
			...
			if((item_amount($item[Antique Accordion]) == 0) && (item_amount($item[Aerogel Accordion]) == 0) && isUnclePAvailable() && !($classes[Accordion Thief, Avatar of Boris, Avatar of Jarlsberg, Avatar of Sneaky Pete, Ed, Vampyre] contains my_class()) && (lh_my_path() != "G-Lover"))
			{
			...
			if(lh_have_skill($skill[Summon Smithsness]) && (my_mp() > (3 * mp_cost($skill[Summon Smithsness]))))
			{
			...
			if(!get_property("lh_useCubeling").to_boolean() && (towerKeyCount() == 0) && (fullness_left() >= 4))
			{
				if((item_amount($item[Boris\'s Key]) == 0) && canEat($item[Boris\'s Key Lime Pie]) && !contains_text(get_property("nsTowerDoorKeysUsed"), $item[Boris\'s Key]))
				{
					pullXWhenHaveY($item[Boris\'s Key Lime Pie], 1, 0);
...
#			if((item_amount($item[glass of goat\'s milk]) == 0) || (lh_my_path() == "Picky"))
#			{
#				pullXWhenHaveY($item[milk of magnesium], 1, 0);
#			}
		}
		if(chateaumantegna_havePainting() && (my_class() != $class[Ed]) && (lh_my_path() != "Community Service"))
		{
			if(lh_have_familiar($familiar[Reanimated Reanimator]))
			{
	...
	{
		if(get_property("lh_day_init").to_int() < 3)
		{
			while(acquireHermitItem($item[Ten-leaf Clover]));
	...
	{
		if(get_property("lh_day_init").to_int() < 4)
		{
			while(acquireHermitItem($item[Ten-leaf Clover]));
		}
	}
...
boolean dailyEvents()
{
	...
	if((item_amount($item[Burned Government Manual Fragment]) > 0) && is_unrestricted($item[Burned Government Manual Fragment]) && get_property("lh_alienLanguage").to_boolean())
	{
	...
	if(item_amount($item[Genie Bottle]) > 0 && lh_is_valid($item[pocket wish]))
	{
...
boolean doBedtime()
{
	...
	lh_process_kmail("lh_deleteMail");
	...
	if((my_class() == $class[Seal Clubber]) && guild_store_available() && isHermitAvailable() && (lh_my_path() != "G-Lover"))
	{
		handleFamiliar("stat");
	...
	if(get_property("lh_priorCharpaneMode").to_int() == 1)
	{
	...
	if((my_inebriety() <= inebriety_limit()) && can_drink() && (my_rain() >= 50) && (my_adventures() >= 1))
	{
		...
		if(lh_have_familiar($familiar[Machine Elf]) && (get_property("_machineTunnelsAdv").to_int() < 5) && (inebriety_left() >= 0) && (my_adventures() > 0))
		{
	...
	{
		if(lh_my_path() == "Pocket Familiars" || my_class() == $class[Vampyre])
		{
	...
	if(!get_property("_mayoTankSoaked").to_boolean() && (lh_get_campground() contains $item[Portable Mayo Clinic]) && is_unrestricted($item[Portable Mayo Clinic]))
	{
		string temp = visit_url("shop.php?action=bacta&whichshop=mayoclinic");
	}

	if((lh_my_path() == "Nuclear Autumn") && (get_property("falloutShelterLevel").to_int() >= 3) && !get_property("_falloutShelterSpaUsed").to_boolean())
	{
	...
	if((get_property("telescopeUpgrades").to_int() > 0) && (get_property("lh_sorceress") == ""))
	{
	...
	if(!possessEquipment($item[Vicar\'s Tutu]) && (my_daycount() == 1) && (item_amount($item[lump of Brituminous coal]) > 0))
	{
		...
		{
			lhCraft("smith", 1, $item[lump of Brituminous coal], $item[frilly skirt]);
		}
	}
	...
	if((get_property("_grimBuff") == "false") && lh_have_familiar($familiar[Grim Brother]))
	{
		string temp = visit_url("choice.php?pwd=&whichchoice=835&option=1", true);
	}

	dailyEvents();
	if((get_property("lh_clanstuff").to_int() < my_daycount()) && (get_clan_id() != -1))
	{
		...
		set_property("lh_clanstuff", ""+my_daycount());
	}
	...
	if(get_property("kingLiberated").to_boolean())
	{
		...
		if(lh_have_skill($skill[Summon Clip Art]) && get_property("_clipartSummons").to_int() == 0)
		{
			cli_execute("make unbearable light");
		}
		if(lh_have_skill($skill[Summon Clip Art]) && get_property("_clipartSummons").to_int() == 1)
		{
			cli_execute("make cold-filtered water");
		}
		if(lh_have_skill($skill[Summon Clip Art]) && get_property("_clipartSummons").to_int() == 2)
		{
	...
	if(my_daycount() == 1)
	{
		if((pulls_remaining() > 1) && !possessEquipment($item[antique machete]) && (my_class() != $class[Avatar of Boris]) && (lh_my_path() != "Way of the Surprising Fist") && (lh_my_path() != "Pocket Familiars"))
		{
		...
		if((pulls_remaining() > 1) && (get_property("lh_palindome") != "finished") )
		{
	...
	if(lh_haveWitchess() && (get_property("puzzleChampBonus").to_int() == 20) && !get_property("_witchessBuff").to_boolean())
	{
	...
	if(lh_haveSourceTerminal())
	{
		int enhances = lh_sourceTerminalEnhanceLeft();
		while(enhances > 0)
		{
			lh_sourceTerminalEnhance("items");
			lh_sourceTerminalEnhance("meat");
			enhances -= 2;
		}
	}
	...
	{
		if(!get_property("_kingLiberated").to_boolean() && (get_property("lh_extrudeChoice") != "none"))
		{
			int count = 3 - get_property("_sourceTerminalExtrudes").to_int();

			string[int] extrudeChoice;
			if(get_property("lh_extrudeChoice") != "")
			{
				string[int] extrudeDays = split_string(get_property("lh_extrudeChoice"), ":");
			...
			{
				lh_sourceTerminalExtrude(extrudeChoice[3-count]);
				count -= 1;
			}
		}
		int extrudeLeft = 3 - get_property("_sourceTerminalExtrudes").to_int();
		if((extrudeLeft > 0) && (lh_my_path() != "Pocket Familiars") && (item_amount($item[Source Essence]) >= 10))
		{
	...
	if(smiles > 0)
	{
		if(get_property("lh_smileAt") != "")
		{
			cli_execute("/cast " + smiles + " the smile @ " + get_property("lh_smileAt"));
		}
		else
		{
	...
	if((get_property("lh_dickstab").to_boolean()) && chateaumantegna_available() && (my_daycount() == 1))
	{
	...
	if(!done)
	{
		...
		if(lh_have_familiar($familiar[Stooper]) && (inebriety_left() == 0) && (my_familiar() != $familiar[Stooper]) && (lh_my_path() != "Pocket Familiars"))
		{
			print("You have a Stooper, you can increase liver by 1!", "blue");
			use_familiar($familiar[Stooper]);
		}
		if(lh_have_familiar($familiar[Machine Elf]) && (get_property("_machineTunnelsAdv").to_int() < 5))
		{
			print("You have " + (5 - get_property("_machineTunnelsAdv").to_int()) + " fights in The Deep Machine Tunnels that you should use!", "blue");
		}
		...
		lh_autoDrinkNightcap(true); // simulate;
	...
	else
	{
		if(!get_property("kingLiberated").to_boolean())
		{
			print(get_property("lh_banishes_day" + my_daycount()));
			print(get_property("lh_yellowRay_day" + my_daycount()));
		...
		if(is_unrestricted($item[Potted Tea Tree]) && !get_property("_pottedTeaTreeUsed").to_boolean() && (lh_get_campground() contains $item[Potted Tea Tree]))
		{
...
boolean questOverride()
{
	...
	// At the start of an ascension, lh_get_campground() displays the wrong info.
	// Visiting the campground doesn\'t work.... grrr...
	//	visit_url("campground.php");

#	if(!get_property("lh_haveoven").to_boolean())
#	{
#		if(lh_get_campground() contains $item[Dramatic&trade; range])
#		{
#			set_property("lh_haveoven", true);
#		}
#	}

	if((get_property("lastTempleUnlock").to_int() == my_ascensions()) && (get_property("lh_treecoin") != "finished"))
	{
		print("Found Unlocked Hidden Temple but unaware of tree-holed coin (2)");
		set_property("lh_treecoin", "finished");
	}
	if((get_property("lastTempleUnlock").to_int() == my_ascensions()) && (get_property("lh_spookymap") != "finished"))
	{
		print("Found Unlocked Hidden Temple but unaware of spooky map (2)");
		set_property("lh_spookymap", "finished");
	}
	if((get_property("lastTempleUnlock").to_int() == my_ascensions()) && (get_property("lh_spookyfertilizer") != "finished"))
	{
		print("Found Unlocked Hidden Temple but unaware of spooky fertilizer (2)");
		set_property("lh_spookyfertilizer", "finished");
	}
	if((get_property("lastTempleUnlock").to_int() == my_ascensions()) && (get_property("lh_spookysapling") != "finished"))
	{
		print("Found Unlocked Hidden Temple but unaware of spooky sapling (2)");
		set_property("lh_spookysapling", "finished");
	}

	if((get_property("questL02Larva") == "finished") && (get_property("lh_mosquito") != "finished"))
	{
		print("Found completed Mosquito Larva (2)");
		set_property("lh_mosquito", "finished");
	}
	if((get_property("questL03Rat") == "finished") && (get_property("lh_tavern") != "finished"))
	{
		print("Found completed Tavern (3)");
		set_property("lh_tavern", "finished");
	}
	if((get_property("questL04Bat") == "finished") && (get_property("lh_bat") != "finished"))
	{
		print("Found completed Bat Cave (4)");
		set_property("lh_bat", "finished");
	}
	if((get_property("questL05Goblin") == "finished") && (get_property("lh_goblinking") != "finished"))
	{
		print("Found completed Goblin King (5)");
		set_property("lh_day1_cobb", "finished");
		set_property("lh_goblinking", "finished");
	}
	if((get_property("questL06Friar") == "finished") && (get_property("lh_friars") != "done") && (get_property("lh_friars") != "finished"))
	{
		print("Found completed Friars (6)");
		set_property("lh_friars", "done");
	}
	if((get_property("questL07Cyrptic") == "finished") && (get_property("lh_crypt") != "finished"))
	{
		print("Found completed Cyrpt (7)");
		set_property("lh_crypt", "finished");
	}
	if((get_property("questL08Trapper") == "step2") && (get_property("lh_trapper") != "yeti"))
	{
		print("Found Trapper partially completed (8: Ores/Cheese)");
		set_property("lh_trapper", "yeti");
	}
	if((get_property("questL08Trapper") == "finished") && (get_property("lh_trapper") != "finished"))
	{
		print("Found completed Trapper (8)");
		set_property("lh_trapper", "finished");
	}

	if((get_property("questL09Topping") == "finished") && (get_property("lh_highlandlord") != "finished"))
	{
		print("Found completed Highland Lord (9)");
		set_property("lh_highlandlord", "finished");
		set_property("lh_boopeak", "finished");
		set_property("lh_oilpeak", "finished");
		set_property("lh_twinpeak", "finished");
	}
	if((get_property("questL10Garbage") == "finished") && (get_property("lh_castletop") != "finished"))
	{
		print("Found completed Castle in the Clouds in the Sky with some Pie (10)");
		set_property("lh_castletop", "finished");
		set_property("lh_castleground", "finished");
		set_property("lh_castlebasement", "finished");
		set_property("lh_airship", "finished");
		set_property("lh_bean", true);
	}
	if((internalQuestStatus("questL10Garbage") >= 9) && (get_property("lh_castleground") != "finished"))
	{
		print("Found completed Castle Ground Floor (10)");
		set_property("lh_castleground", "finished");
	}
	if((internalQuestStatus("questL10Garbage") >= 8) && (get_property("lh_castlebasement") != "finished"))
	{
		print("Found completed Castle Basement (10)");
		set_property("lh_castlebasement", "finished");
	}
	if((internalQuestStatus("questL10Garbage") >= 7) && (get_property("lh_airship") != "finished"))
	{
		print("Found completed Airship (10)");
		set_property("lh_airship", "finished");
	}
	if((internalQuestStatus("questL10Garbage") >= 2) && !get_property("lh_bean").to_boolean())
	{
		print("Found completed Planted Beanstalk (10)");
		set_property("lh_bean", true);
	}

#	if((get_property("lastSecondFloorUnlock").to_int() >= my_ascensions()) && (get_property("lh_spookyravennecklace") != "done"))
#	{
#		print("Found completed Spookyraven Necklace Sequence (M20)");
#		set_property("lh_spookyravennecklace", "done");
#	}

	if((internalQuestStatus("questL11Manor") >= 11) && (get_property("lh_ballroom") != "finished"))
	{
		print("Found completed Spookyraven Manor (11)");
		set_property("lh_ballroom", "finished");
		set_property("lh_winebomb", "finished");
		set_property("lh_ballroomflat", "finished");
	}

	if((internalQuestStatus("questL11Manor") >= 3) && (get_property("lh_winebomb") != "finished"))
	{
		print("Found completed Spookyraven Manor Organ Music (11)");
		set_property("lh_winebomb", "finished");
		set_property("lh_masonryWall", true);
		set_property("lh_ballroomflat", "finished");
	}

	if((internalQuestStatus("questL11Manor") >= 1) && (get_property("lh_ballroomflat") != "finished"))
	{
		print("Found completed Spookyraven Manor Organ Music (11)");
		set_property("lh_ballroomflat", "finished");
	}

	if((internalQuestStatus("questL11Worship") >= 3) && (get_property("lh_hiddenunlock") != "finished"))
	{
		print("Found unlocked Hidden City (11)");
		set_property("lh_hiddenunlock", "finished");
	}

	if((internalQuestStatus("questL11Black") >= 2) && (get_property("lh_blackmap") == ""))
	{
		print("Found an unexpected Black Market (11 - via questL11Black)");
		set_property("lh_blackmap", "document");
	}
	if((get_property("blackForestProgress") >= 5) && (get_property("lh_blackmap") == ""))
	{
		print("Found an unexpected Black Market (11 - via blackForestProgress)");
		set_property("lh_blackmap", "document");
	}

	if((get_property("questL11Black") == "finished") && (get_property("lh_blackmap") != "finished"))
	{
		print("Found completed Black Market (11 - via questL11Black)");
		set_property("lh_blackmap", "finished");
	}
	if((internalQuestStatus("questL11MacGuffin") >= 2) && (get_property("lh_blackmap") != "finished"))
	{
		print("Found completed Black Market (11 - via questL11MacGuffin)");
		set_property("lh_blackmap", "finished");
	}

	if((internalQuestStatus("questL11MacGuffin") >= 2) && (get_property("lh_mcmuffin") == ""))
	{
		print("Found started McMuffin quest (11)");
		set_property("lh_mcmuffin", "start");
	}

	if((get_property("questL11Palindome") == "finished") && (get_property("lh_palindome") != "finished"))
	{
		print("Found completed Palindome (11)");
		set_property("lh_palindome", "finished");
	}

	if(get_property("pyramidBombUsed").to_boolean() && (get_property("lh_mcmuffin") == "pyramid"))
	{
		print("Found Ed in the Pyramid (11)");
		set_property("lh_mcmuffin", "ed");
	}

//	if((get_property("questL11Pyramid") == "finished") && (get_property("lh_mcmuffin") != "finished"))
//	{
//		print("Found completed Pyramid (11)");
//		set_property("lh_mcmuffin", "finished");
//	}
	if((get_property("questL11MacGuffin") == "finished") && (get_property("lh_mcmuffin") != "finished"))
	{
	...
	if((get_property("questL11Business") == "finished") && (get_property("lh_hiddenoffice") != "finished"))
	{
		print("Found completed Hidden Office Building (11)");
		set_property("lh_hiddenoffice", "finished");
		if((get_property("lh_hiddenzones") != "finished") && (get_property("lh_hiddenzones").to_int() < 3))
		{
			set_property("lh_hiddenzones", 3);
		}
	}
	if((get_property("questL11Curses") == "finished") && (get_property("lh_hiddenapartment") != "finished"))
	{
		print("Found completed Hidden Apartment Building (11)");
		set_property("lh_hiddenapartment", "finished");
		if((get_property("lh_hiddenzones") != "finished") && (get_property("lh_hiddenzones").to_int() < 2))
		{
			set_property("lh_hiddenzones", 2);
		}
	}
	if((get_property("questL11Spare") == "finished") && (get_property("lh_hiddenbowling") != "finished"))
	{
		print("Found completed Hidden Bowling Alley (11)");
		set_property("lh_hiddenbowling", "finished");
		if((get_property("lh_hiddenzones") != "finished") && (get_property("lh_hiddenzones").to_int() < 5))
		{
			set_property("lh_hiddenzones", 5);
		}
	}
	if((get_property("questL11Doctor") == "finished") && (get_property("lh_hiddenhospital") != "finished"))
	{
		print("Found completed Hidden Hopickle (11)");
		set_property("lh_hiddenhospital", "finished");
		if((get_property("lh_hiddenzones") != "finished") && (get_property("lh_hiddenzones").to_int() < 4))
		{
			set_property("lh_hiddenzones", 4);
		}
	}
	if((get_property("questL11Worship") == "finished") && (get_property("lh_hiddencity") != "finished"))
	{
		print("Found completed Hidden City (11)");
		set_property("lh_hiddencity", "finished");
		set_property("lh_hiddenzones", "finished");
		set_property("lh_hiddenunlock", "finished");
	}
	if((internalQuestStatus("questL11Worship") >= 3) && (get_property("lh_hiddenunlock") != "finished"))
	{
	...
	{
		if(get_property("lh_orchard") != "finished")
		{
			print("Found completed Orchard (12)");
			set_property("lh_orchard", "finished");
		}
	}

	if((get_property("sidequestLighthouseCompleted") != "none") && (get_property("lh_sonofa") != "finished"))
	{
		print("Found completed Lighthouse (12)");
		set_property("lh_sonofa", "finished");
	}
	if((get_property("sidequestJunkyardCompleted") != "none") && (get_property("lh_gremlins") != "finished"))
	{
	...
	{
		if(get_property("lh_gremlins") != "finished")
		{
			print("Found completed Junkyard (12)");
			set_property("lh_gremlins", "finished");
		}

		if(get_property("lh_sonofa") != "finished")
		{
			print("Found completed Lighthouse (12)");
			set_property("lh_sonofa", "finished");
		}
		...
		if((get_property("lh_nuns") != "done") && (get_property("lh_nuns") != "finished"))
		{
			print("Found completed Nuns (12)");
			set_property("lh_nuns", "finished");
		}
	}

	if((get_property("sidequestOrchardCompleted") != "none") && (get_property("lh_orchard") != "finished"))
	{
		print("Found completed Orchard (12)");
		set_property("lh_orchard", "finished");
	}
	...
	if((get_property("sidequestNunsCompleted") != "none") && (get_property("lh_nuns") != "done") && (get_property("lh_nuns") != "finished"))
	{
		print("Found completed Nuns (12)");
		set_property("lh_nuns", "finished");
	}

	if((get_property("sideDefeated") != "neither") && (get_property("lh_war") != "finished"))
	{
		print("Found completed Island War (12)");
		set_property("lh_war", "finished");
		council();
	}

	if((internalQuestStatus("questL12War") >= 1)  && (get_property("lh_prewar") != "started"))
	{
		print("Found Started Island War (12)");
		set_property("lh_prewar", "started");
	}

	if((internalQuestStatus("questL12War") >= 2)  && (get_property("lh_war") != "finished"))
	{
		print("Found completed Island War (12)");
		set_property("lh_war", "finished");
		council();
	...
	if((internalQuestStatus("questL13Final") >= 13) && (get_property("lh_sorceress") != "finished"))
	{
		print("Found completed Prism Recovery (13)");
		set_property("lh_sorceress", "finished");
		if(my_class() == $class[Ed])
		{
			council();
		}
	...
	if(possessEquipment($item[Pirate Fledges]) || (internalQuestStatus("questM12Pirate") >= 6))
	{
		...
		if(get_property("lh_fcle") != "finished")
		{
			print("Found Pirate Fledges and incomplete F\'C\'le, fixing...");
			set_property("lh_fcle", "finished");
		}
	}

	if((get_property("lastQuartetAscension").to_int() >= my_ascensions()) && (get_property("lh_ballroomsong") != "finished"))
	{
		print("Found Ballroom Quarter Song set (X).");
		set_property("lh_ballroomsong", "finished");
	}

	if((get_property("lh_war") != "done") && (get_property("lh_war") != "finished") && ((get_property("hippiesDefeated").to_int() >= 1000) || (get_property("fratboysDefeated").to_int() >= 1000)))
	{
		set_property("lh_nuns", "finished");
		set_property("lh_war", "done");
	}
...
boolean L11_aridDesert()
{
	if(my_level() < 12)
	{
		if(get_property("lh_palindome") != "finished")
		{
			return false;
		}
	}
#	Mafia probably handles this correctly (and probably has done so for a while). (failing again as of r19010)
	...
	if(get_property("lh_mcmuffin") != "start")
	{
		return false;
	}
	if((get_property("lh_hiddenapartment") != "finished") && (get_property("lh_hiddenapartment") != "0"))
	{
		return false;
	}
	...
	if(lh_my_path() == "Way of the Surprising Fist")
	{
		failDesert = false;
	}
	...
	if(failDesert)
	{
		...
		else
		{
			...
			set_property("lh_skipDesert", my_turncount());
		}
		return false;
	}
	...
	if((have_effect($effect[Ultrahydrated]) > 0) || (get_property("desertExploration").to_int() == 0))
	{
		...
		if(lh_have_familiar($familiar[Artistic Goth Kid]))
		{
...
#		if(in_hardcore() && isGuildClass() && (item_amount($item[Worm-Riding Hooks]) > 0) && (get_property("desertExploration").to_int() <= (100 - (5 * progress))) && ((get_property("gnasirProgress").to_int() & 16) != 16))
		...
		if(!get_property("lh_gnasirUnlocked").to_boolean() && ($location[The Arid\, Extra-Dry Desert].turns_spent > 10) && (get_property("desertExploration").to_int() > 10))
		{
			print("Did not appear to notice that Gnasir unlocked, assuming so at this point.", "green");
			set_property("lh_gnasirUnlocked", true);
		}

		if(get_property("lh_gnasirUnlocked").to_boolean() && (item_amount($item[Stone Rose]) > 0) && ((get_property("gnasirProgress").to_int() & 1) != 1))
		{
		...
		if(get_property("lh_gnasirUnlocked").to_boolean() && ((get_property("gnasirProgress").to_int() & 2) != 2))
		{
			boolean canBuyPaint = true;
			if((lh_my_path() == "Way of the Surprising Fist") || (lh_my_path() == "Nuclear Autumn"))
			{
				canBuyPaint = false;
			}
		...
		if(get_property("lh_gnasirUnlocked").to_boolean() && (item_amount($item[Killing Jar]) > 0) && ((get_property("gnasirProgress").to_int() & 4) != 4))
		{
		...
		lhAdv(1, $location[The Arid\, Extra-Dry Desert]);
		handleFamiliar("item");
	...
	else
	{
		...
		if((need > (5 * progress)) && (cloversAvailable() > 2) && !get_property("lovebugsUnlocked").to_boolean())
		{
			...
			cloverUsageInit();
			lhAdvBypass("adventure.php?snarfblat=122", $location[The Oasis]);
			cloverUsageFinish();
		}
		else
		{
			if(!lhAdv(1, $location[The Oasis]))
			{
...
boolean L11_palindome()
{
	...
	if((total == 0) && !possessEquipment($item[Mega Gem]) && lovemeDone && in_hardcore() && (item_amount($item[Wet Stunt Nut Stew]) == 0) && ((internalQuestStatus("questL11Palindome") >= 3) || isGuildClass()) && !get_property("lh_bruteForcePalindome").to_boolean())
	{
		if(item_amount($item[Wet Stunt Nut Stew]) == 0)
		{
			...
			if((item_amount($item[Bird Rib]) == 0) || (item_amount($item[Lion Oil]) == 0))
			{
				if(item_amount($item[white page]) > 0)
				{
					...
					if(lhAdvBypass(0, pages, $location[Whitey\'s Grove], "")) {}
				...
				lhAdv(1, $location[Whitey\'s Grove]);
			}
			else if(item_amount($item[Stunt Nuts]) == 0)
			{
				print("We got no nuts!! :O", "Blue");
				lhEquip($slot[acc3], $item[Talisman o' Namsilat]);
				lhAdv(1, $location[Inside the Palindome]);
			}
			else
			{
	...
	if((((total == 4) && hasILoveMeVolI()) || ((total == 0) && possessEquipment($item[Mega Gem]))) && loveMeDone)
	{
		...
		lhEquip($slot[acc2], $item[Mega Gem]);
		lhEquip($slot[acc3], $item[Talisman o' Namsilat]);
		...
		if(lhAdvBypass(0, pages, $location[Noob Cave], "")) {}
	...
	else
	{
		...
		{
			if(!get_property("lh_bruteForcePalindome").to_boolean())
			{
			...
			else
			{
				...
				print("Set lh_bruteForcePalindome=false to try to get a stunt nut stew", "red");
		...
		lhEquip($slot[acc3], $item[Talisman o' Namsilat]);
		lhAdv(1, $location[Inside the Palindome]);
		if(($location[Inside the Palindome].turns_spent > 30) && (lh_my_path() != "Pocket Familiars") && (lh_my_path() != "G-Lover") && !in_koe())
		{
...
{
	if(!get_property("lh_wandOfNagamar").to_boolean() && (get_property("questL13Final") != "step12"))
	{
		return false;
	}
	...
	{
		return lhAdv($location[The VERY Unquiet Garves]);
	}
	...
	else
	{
		...
		{
			cloverUsageInit();
			lhAdvBypass(322, $location[The Castle in the Clouds in the Sky (Basement)]);
			cloverUsageFinish();
			cli_execute("make " + $item[Wand Of Nagamar]);
			return true;
		}
...
boolean L13_towerNSTransition()
{
	if(get_property("lh_sorceress") == "top")
	{
		set_property("lh_sorceress", "final");
	}
	return false;
}
...
{
	if(get_property("lh_sorceress") != "final")
	{
		return false;
	}
	if(get_property("lh_wandOfNagamar").to_boolean())
	{
	...
	if($strings[Actually Ed the Undying, Avatar of Boris, Avatar of Jarlsberg, Avatar of Sneaky Pete, Bees Hate You, Bugbear Invasion, Community Service, Heavy Rains, The Source, Way of the Surprising Fist, Zombie Slayer] contains lh_my_path())
	{
		if(lh_my_path() == "The Source")
		{
			acquireMP(200, true);
		}
	}
	...
	{
		cli_execute("scripts/postHaps.ash");
	}
	if(my_class() == $class[Turtle Tamer])
	{
		lhEquip($item[Ouija Board\, Ouija Board]);
	}
	...
	lhEquip($slot[Off-Hand], $item[Oscus\'s Garbage Can Lid]);
	...
	else
	{
		...
		if(!lh_have_familiar($familiar[Warbear Drone]))
		{
			handleFamiliar($familiar[Fist Turkey]);
		}
		if(lh_have_familiar($familiar[Machine Elf]))
		{
	...
	if(!useMaximizeToEquip())
	{
		lhEquip($item[Beer Helmet]);
		lhEquip($item[Misty Cloak]);
		lhEquip($slot[acc1], $item[gumshoes]);
		lhEquip($slot[acc3], $item[World\'s Best Adventurer Sash]);
	}
	else
	{
		addToMaximize("10dr,3moxie,0.5da 1000max,-5ml,1.5hp,0item,0meat");
	}
	lhEquip($slot[acc2], $item[Attorney\'s Badge]);


	if(internalQuestStatus("questL13Final") < 13)
	{
		cli_execute("scripts/preHaps.ash");
		set_property("lh_disableAdventureHandling", true);
		lhAdvBypass("place.php?whichplace=nstower&action=ns_10_sorcfight", $location[Noob Cave]);
		...
		if(last_monster() == $monster[Naughty Sorceress])
		{
			...
			if(get_property("lh_stayInRun").to_boolean())
			{
				set_property("lh_disableAdventureHandling", false);
				abort("User wanted to stay in run (lh_stayInRun), we are done.");
			}
			...
			if(!($classes[Seal Clubber, Turtle Tamer, Pastamancer, Sauceror, Disco Bandit, Accordion Thief] contains my_class()))
			{
				...
				if(get_property("lh_sorceress") == "finished" || get_property("questL13Final") == "finished")
				{
...
boolean L13_towerNSTower()
{
	...
	if(contains_text(visit_url("place.php?whichplace=nstower"), "ns_05_monster1"))
	{
		...
		int sources = 0;
		if(lhEquip($item[astral shirt]))
		{
			// nothing, just for else
		}
		else if(lhEquip($item[Unfortunato's foolscap]))
		{
			// nothing, just for else
		}
		...
		else if(have_effect($effect[damage.enh]) == 0)
		{
			...
			if(enhances > 0)
			{
				lh_sourceTerminalEnhance("damage");
			}
		}

		if(my_class() == $class[Turtle Tamer])
		{
			lhEquip($slot[shirt], $item[Shocked Shell]);
		}
		...
		if((lh_have_familiar($familiar[warbear drone])) && !is100FamiliarRun())
		{
			sources = sources + 2;
			handleFamiliar($familiar[Warbear Drone]);
			use_familiar($familiar[Warbear Drone]);
			cli_execute("preHaps"); // TODO: can we remove this?
			...
			if(possessEquipment($item[warbear drone codes]))
			{
				lhEquip($item[warbear drone codes]);
				sources = sources + 2;
			}
		}
		else if((lh_have_familiar($familiar[Sludgepuppy])) && !is100FamiliarRun())
		{
			handleFamiliar($familiar[Sludgepuppy]);
			sources = sources + 3;
		}
		else if((lh_have_familiar($familiar[Imitation Crab])) && !is100FamiliarRun())
		{
			handleFamiliar($familiar[Imitation Crab]);
			sources = sources + 2;
		}
		if(lhEquip($slot[acc1], $item[hippy protest button]))
		{
			sources = sources + 1;
		}
		...
		if(lhEquip($item[smirking shrunken head]))
		{
			sources = sources + 1;
		}
		else if(lhEquip($item[hot plate]))
		{
			sources = sources + 1;
		}
		...
		handleBjornify($familiar[Hobo Monkey]);
		lhEquip($slot[acc2], $item[world\'s best adventurer sash]);
		lhEquip($slot[acc3], $item[Groll Doll]);
		lhEquip($slot[acc3], $item[old school calculator watch]);
		lhEquip($slot[acc3], $item[Bottle Opener Belt Buckle]);
		lhEquip($slot[acc3], $item[acid-squirting flower]);
		...
		if((item_amount($item[Beehive]) > 0) || (sources > sourceNeed))
		{
			...
			lhAdvBypass("place.php?whichplace=nstower&action=ns_05_monster1", $location[Tower Level 1]);
	...
	if(contains_text(visit_url("place.php?whichplace=nstower"), "ns_06_monster2"))
	{
		...
		if(!useMaximizeToEquip())
		{
			lhEquip($item[Silver Cow Creamer]);
			lhEquip($item[Sneaky Pete\'s Leather Jacket]);
		}
		if(is100FamiliarRun())
		{
			if(useMaximizeToEquip())
		...
		{
			lhEquip($item[Meat Tenderizer is Murder]);
		}
		...
		lhAdvBypass("place.php?whichplace=nstower&action=ns_06_monster2", $location[Noob Cave]);
		return true;
	}

	if(contains_text(visit_url("place.php?whichplace=nstower"), "ns_07_monster3"))
	{
		if((item_amount($item[Electric Boning Knife]) > 0) || (lh_my_path() == "Pocket Familiars"))
		{
			set_property("lh_getBoningKnife", false);
		}

		if(!get_property("lh_getBoningKnife").to_boolean() && ((my_class() == $class[Sauceror]) || have_skill($skill[Garbage Nova])))
		{
			...
			if(item_amount($item[Electric Boning Knife]) == 0)
			{
				...
				{
					lhMaximize("myst -equip snow suit", 1500, 0, false);
				}
			}
			...
			lhAdvBypass("place.php?whichplace=nstower&action=ns_07_monster3", $location[Noob Cave]);
		...
		else if((item_amount($item[Electric Boning Knife]) > 0) || (lh_my_path() == "Pocket Familiars"))
		{
			return lhAdvBypass("place.php?whichplace=nstower&action=ns_07_monster3", $location[Noob Cave]);
		}
		else if(canGroundhog($location[The Castle in the Clouds in the Sky (Ground Floor)]))
		{
			...
			lhAdv(1, $location[The Castle in the Clouds in the Sky (Ground Floor)]);
		}
		return true;
	}
	...
	{
		boolean confidence = get_property("lh_confidence").to_boolean();
	...
	if(contains_text(visit_url("place.php?whichplace=nstower"), "ns_09_monster5"))
	{
		...
		lhAdvBypass("place.php?whichplace=nstower&action=ns_09_monster5", $location[Noob Cave]);
		return true;
	}

	if(contains_text(visit_url("place.php?whichplace=nstower"), "ns_10_sorcfight"))
	{
		set_property("lh_sorceress", "top");
		return true;
	}
...
boolean L13_towerNSHedge()
{
	...
	{
		set_property("lh_sorceress", "door");
		return true;
	}
	...
	else if(get_property("lh_hedge") == "fast")
	{
	...
	{
		abort("lh_hedge not set properly (slow/fast), assuming manual handling desired");
	}
	if(have_effect($effect[Beaten Up]) > 0)
	{
...
{
	if(get_property("lh_sorceress") != "start")
	{
		return false;
	}


	if((get_property("lh_trytower") == "pause") || (get_property("lh_trytower") == "stop"))
	{
		ns_crowd1();
		ns_crowd2();
		ns_crowd3();
		...
		ns_hedge2();
		ns_hedge3();
		if(get_property("lh_trytower") == "stop")
		{
		...
		else if(get_property("lh_trytower") == "pause")
		{
			set_property("lh_trytower", "");
	...
	{
		set_property("lh_sorceress", "hedge");
		return true;
	}
	...
	if(contains_text(visit_url("place.php?whichplace=nstower"), "ns_01_contestbooth"))
	{
		...
		{
			if(!get_property("_grimBuff").to_boolean() && lh_have_familiar($familiar[Grim Brother]))
			{
			...
			case 1:
				...
				if(is100FamiliarRun())
				{
					lhMaximize("init, -equip snow suit", 1500, 0, false);
				}
				else
				{
					lhMaximize("init, switch xiblaxian holo-companion, switch oily woim, switch happy medium,switch cute meteor", 1500, 0, false);
					handleFamiliar(my_familiar());
				}
				...
				if(crowd1Insufficient()) lh_beachCombHead("init");

				if(crowd1Insufficient())
				{
					if(get_property("lh_secondPlaceOrBust").to_boolean())
						abort("Not enough initiative for the initiative test, aborting since lh_secondPlaceOrBust=true");
					else
						print("Not enough initiative for the initiative test, but continuing since lh_secondPlaceOrBust=false", "red");
				}
				break;
			}
		...
		if(get_property("nsContestants2").to_int() == -1)
		{
			...
			{
			case $stat[moxie]:
				lhMaximize("moxie -equip snow suit", 1500, 0, false);
				...
				if(crowd2Insufficient()) {
					if(lh_have_skill($skill[Quiet Desperation]))
				...
				if(crowd1Insufficient()) lh_beachCombHead("moxie");
				...
				break;
			case $stat[muscle]:
				lhMaximize("muscle -equip snow suit", 1500, 0, false);
				...
				if(crowd2Insufficient()) buffMaintain($effect[Song of Bravado], 100, 1, 1);
				if(crowd2Insufficient()) buffMaintain($effect[Stevedave\'s Shanty of Superiority], 30, 1, 1);
				if(crowd1Insufficient()) lh_beachCombHead("muscle");
			...
			case $stat[mysticality]:
				...
				if(crowd2Insufficient()) buffMaintain($effect[Stevedave\'s Shanty of Superiority], 30, 1, 1);
				if(crowd1Insufficient()) lh_beachCombHead("mysticality");
			...
			{
				if(get_property("lh_secondPlaceOrBust").to_boolean())
					abort("Not enough " + crowd_stat + " for the stat test, aborting since lh_secondPlaceOrBust=true");
				else
					print("Not enough " + crowd_stat + " for the stat test, but continuing since lh_secondPlaceOrBust=false", "red");
			}
			visit_url("place.php?whichplace=nstower&action=ns_01_contestbooth");
		...
		if(get_property("nsContestants3").to_int() == -1)
		{
			...
			{
				lhMaximize(challenge + " dmg, " + challenge + " spell dmg -equip snow suit", 1500, 0, false);
			}
			...
			score += numeric_modifier(challenge + " spell damage");
			if((score < 80) && get_property("lh_useWishes").to_boolean())
			{
				switch(challenge)
				{
				case $element[cold]:
			...
			{
				if(get_property("lh_secondPlaceOrBust").to_boolean())
					abort("Not enough " + challenge + " for the elemental test, aborting since lh_secondPlaceOrBust=true");
				else
					print("Not enough " + challenge + " for the elemental test, but continuing since lh_secondPlaceOrBust=false", "red");
			}
		...
		if((get_property("nsContestants1").to_int() == 0) && (get_property("nsContestants2").to_int() == 0) && (get_property("nsContestants3").to_int() == 0))
		{
			...
			if((get_property("nsContestants1").to_int() != 0) || (get_property("nsContestants2").to_int() != 0) || (get_property("nsContestants3").to_int() != 0))
			{
				...
				else
				{
					...
					abort("Unknown questL13Final/lh_sorceress state.");
				}
			}
			return true;
		}
	...
	{
		lhAdv(1, $location[Fastest Adventurer Contest]);
		return true;
	}
...
boolean L13_towerNSEntrance()
{
	...
	if(contains_text(visit_url("place.php?whichplace=nstower"), "ns_01_contestbooth"))
	{
		set_property("lh_sorceress", "start");
		set_property("choiceAdventure1003", 0);
	}
	else
	{
		if(my_level() < 13)
		{
			...
			set_property("lh_newbieOverride", true);

			if(snojoFightAvailable() && (lh_my_path() == "Pocket Familiars"))
			{
				lhAdv(1, $location[The X-32-F Combat Training Snowman]);
				return true;
			}
			...
			int delay = get_property("lh_powerLevelTimer").to_int();
			if(delay == 0)
			{
				delay = 10;
			}
			...
			if((get_property("timesRested").to_int() < total_free_rests()) && chateaumantegna_available() && (lh_my_path() != "The Source"))
			{
				doRest();
				cli_execute("scripts/postHaps.ash");
				loopHandlerDelayAll();
			...
			{
				if(get_property("lh_powerLevelAdvCount").to_int() >= 10)
				{
		...
		else
		{
			...
			if(my_level() != get_property("lh_powerLevelLastLevel").to_int())
			{
				print("Hmmm, we need to stop being so feisty about quests...", "red");
				set_property("lh_powerLevelLastLevel", my_level());
				return true;
			}
...
{
	if(get_property("lh_ignoreFlyer").to_boolean())
	{
		return false;
	}
	if(my_level() < 12)
	...
	if(get_property("lh_war") == "finished")
	{
		return false;
	}
	if((item_amount($item[Rock Band Flyers]) == 0) && (item_amount($item[Jam Band Flyers]) == 0))
	...
	if(doHoleInTheSky)
	{
		if(item_amount($item[Steam-Powered Model Rocketship]) == 0)
		{
			...
			lhAdv(1, $location[The Castle in the Clouds in the Sky (Top Floor)]);
			handleFamiliar("item");
		}
		else
		{
...
{
	if(elementalPlanes_access($element[stench]) && lh_have_skill($skill[Summon Smithsness]))
	{
		lhAdv(1, $location[Uncle Gator\'s Country Fun-Time Liquid Waste Sluice]);
	}
	else if(elementalPlanes_access($element[spooky]))
	{
		lhAdv(1, $location[The Deep Dark Jungle]);
	}
	else if(elementalPlanes_access($element[cold]))
	{
		lhAdv(1, $location[VYKEA]);
	}
	...
	{
		lhAdv(1, $location[Sloppy Seconds Diner]);
	}
	...
	{
		if((my_level() >= 11) && (get_property("lh_hiddenzones") == "finished"))
		{
			int flyer = get_property("flyeredML").to_int();
			lhAdv($location[The Hidden Hospital]);
...
{
	if(get_property("lh_powerLevelLastLevel").to_int() != my_level() && get_property("lh_powerLevelAdvCount").to_int() > 0)
	{
		set_property("lh_powerLevelLastLevel", my_level());
		set_property("lh_powerLevelAdvCount", 0);
		return true;
...
boolean LX_melvignShirt()
{
	...
	{
		lhAdv($location[The Thinknerd Warehouse]);
		return true;
	}
...
{
	set_property("lh_powerLevelAdvCount", get_property("lh_powerLevelAdvCount").to_int() + 1);
	set_property("lh_powerLevelLastAttempted", my_turncount());
	...
	if(lh_my_path() == "The Source")
	{
		if(get_property("lh_spookyravensecond") != "")
		{
			if(get_property("barrelShrineUnlocked").to_boolean())
			{
				...
				if((whereTo == $location[The Haunted Ballroom]) && (get_property("lh_ballroomopen") != "open"))
				{
	...
	if(elementalPlanes_access($element[stench]) && lh_have_skill($skill[Summon Smithsness]) && (get_property("lh_beatenUpCount").to_int() == 0))
	{
		lhAdv(1, $location[Uncle Gator\'s Country Fun-Time Liquid Waste Sluice]);
	}
	else if(elementalPlanes_access($element[spooky]))
...
#	}
	...
	else
	{
		...
		{
			lhAdv($location[The Hole In The Sky]);
			return true;
		}
		if((my_level() >= 9) && ((get_property("lh_highlandlord") == "start") || (get_property("lh_highlandlord") == "finished")))
		{
			if((monster_level_adjustment() < 60) && (get_property("oilPeakProgress").to_float() > 0.0))
			{
				lhEquip($slot[Pants], $item[Dress Pants]);
			}
			lhAdv($location[Oil Peak]);
			return true;
		}
		if((my_level() >= 8) && (get_property("lh_trapper") == "finished"))
		{
			...
			if(elemDamage > 20)
			{
				lhAdv($location[The Icy Peak]);
			}
			else
			{
				lhAdv($location[The Extreme Slope]);
			}
			return true;
		}
		return false;
...
{
	if(get_property("lh_hiddencity") == "finished")
	{
		return false;
	}

	if(get_property("lh_hiddenzones") != "finished")
	{
		return false;
	}
	...
	if(get_property("lh_hiddenzones") == "finished")
	{
		if((get_property("_nanorhinoBanishedMonster") == "") && (have_effect($effect[nanobrawny]) == 0) && lh_have_familiar($familiar[Nanorhino]))
		{
		...
		if((get_property("lh_hiddenapartment") == "finished") || (item_amount($item[Moss-Covered Stone Sphere]) > 0))
		{
			set_property("lh_hiddenapartment", "finished");
		...
		if((get_property("lh_hiddenoffice") == "finished") || (item_amount($item[Crackling Stone Sphere]) > 0))
		{
			set_property("lh_hiddenoffice", "finished");
		...
		if((get_property("lh_hiddenbowling") == "finished") || (item_amount($item[Scorched Stone Sphere]) > 0))
		{
			set_property("lh_hiddenbowling", "finished");
	...
	if((item_amount($item[Moss-Covered Stone Sphere]) == 0) && (get_property("lh_hiddenapartment") != "finished"))
	{
	...
	if((get_property("lh_hiddenapartment") != "finished") && (get_counters("Fortune Cookie", 0, 9) != "Fortune Cookie") && (my_adventures() >= (9 - get_property("lh_hiddenapartment").to_int())) && (have_effect($effect[Ancient Fortitude]) == 0))
		{
			print("The idden [sic] apartment!", "blue");

			int current = get_property("lh_hiddenapartment").to_int() + 1;
			set_property("lh_hiddenapartment", current);
			...
			if(!get_property("_claraBellUsed").to_boolean() && (item_amount($item[Clara\'s Bell]) > 0))
			{
				...
				if(lh_my_path() == "Pocket Familiars")
				{
					if(get_property("relocatePygmyLawyer").to_int() != my_ascensions())
					{
						...
						lhAdv(1, $location[The Hidden Apartment Building]);
						return true;
					}
				}
				current = 9;
			...
			if(current <= 8)
			{
				...
				{
					set_property("lh_hiddenapartment", current - 1);
				}
				return true;
			}
			else
		...
		if((get_property("lh_hiddenoffice") != "finished") && (my_adventures() >= 11))
		{
			...
			lhAdv(1, $location[The Hidden Office Building]);
			restoreSetting("autoCraft");
			return true;
		}

		if(get_property("lh_hiddenbowling") != "finished")
		{
...
//			}
			lhAdv(1, $location[The Hidden Bowling Alley]);
...
//			}
		...
		if(get_property("lh_hiddenhospital") != "finished")
		{
			...
			lhEquip($item[bloodied surgical dungarees]);
			lhEquip($item[half-size scalpel]);
			lhEquip($item[surgical apron]);
			lhEquip($slot[acc3], $item[head mirror]);
			lhEquip($slot[acc2], $item[surgical mask]);
			print("Hidden Hospital Progress: " + get_property("hiddenHospitalProgress"), "blue");
			if((my_mp() > 60) || considerGrimstoneGolem(true))
			{
				handleBjornify($familiar[Grimstone Golem]);
			}
			lhAdv(1, $location[The Hidden Hospital]);

			return true;
		}
		if((get_property("lh_hiddenapartment") == "finished") && (get_property("lh_hiddenoffice") == "finished") && (get_property("lh_hiddenbowling") == "finished") && (get_property("lh_hiddenhospital") == "finished"))
		{
			print("Getting the stone triangles", "blue");
			set_property("choiceAdventure791", "1");
			while(item_amount($item[stone triangle]) < 1)
			{
				lhAdv(1, $location[An Overgrown Shrine (Northwest)]);
			}
			while(item_amount($item[stone triangle]) < 2)
			{
				lhAdv(1, $location[An Overgrown Shrine (Northeast)]);
			}
			while(item_amount($item[stone triangle]) < 3)
			{
				lhAdv(1, $location[An Overgrown Shrine (Southwest)]);
			}
			while(item_amount($item[stone triangle]) < 4)
			{
				lhAdv(1, $location[An Overgrown Shrine (Southeast)]);
			}
			...
			try
			{
				...
				if(lhAdvBypass(0, pages, $location[A Massive Ziggurat], "")) {}
				handleFamiliar("item");
			}
			finally
			{
...
boolean L11_hiddenCityZones()
{
	...
	if(get_property("lh_hiddenunlock") != "finished")
	{
		return false;
	}
	if(get_property("lh_hiddenzones") == "finished")
	...
	if(get_property("lh_hiddenzones") == "")
	{
		print("Machete the hidden zones!", "blue");
		set_property("choiceAdventure791", "6");
		set_property("lh_hiddenzones", "0");
	}

	if(get_property("lh_hiddenzones") == "0")
	{
		...
		if(!in_hardcore() || (my_class() == $class[Avatar of Boris]) || (lh_my_path() == "Way of the Surprising Fist") || (lh_my_path() == "Pocket Familiars"))
		{
			needMachete = false;
		}
...
/*
		if(possessEquipment($item[Antique Machete]))
		{
			...
			{
				set_property("lh_hiddenzones", "1");
				return true;
			}
		}

		if(((my_class() == $class[Avatar of Boris]) || (lh_my_path() == "Way of the Surprising Fist") || (lh_my_path() == "Pocket Familiars")) && (get_property("relocatePygmyJanitor").to_int() == my_ascensions()))
		{
			if(!in_hardcore() || (get_property("hiddenTavernUnlock").to_int() == my_ascensions()))
			{
				set_property("lh_hiddenzones", "1");
...
*/
		...
		lhAdv(1, $location[The Hidden Park]);
		return true;
	}

	if(get_property("lh_hiddenzones") == "1")
	{
		...
		lhForceEquip($item[Antique Machete]);
		# Add provision for Golden Monkey, or even more so, "Do we need spleen item"
		if(($familiar[Unconscious Collective].drops_today < 1) && lh_have_familiar($familiar[Unconscious Collective]))
		{
			handleFamiliar($familiar[Unconscious Collective]);
		}
		else if(lh_have_familiar($familiar[Fist Turkey]))
		{
			handleFamiliar($familiar[Fist Turkey]);
		}
	...
	if(get_property("lh_hiddenzones") == "2")
	{
	...
	if(get_property("lh_hiddenzones") == "3")
	{
	...
	if(get_property("lh_hiddenzones") == "4")
	{
	...
	if(get_property("lh_hiddenzones") == "5")
	{
		lhForceEquip($item[Antique Machete]);
		...
		lhAdv(1, $location[A Massive Ziggurat]);
...
{
	if(!get_property("lh_useWishes").to_boolean() && canGenieCombat())
	{
		print("Skipping wishing for Baa'baa'bu'ran because lh_useWishes=false", "red");
	}
	if (get_property("lh_useWishes").to_boolean() && canGenieCombat())
	{
...
boolean L11_unlockHiddenCity()
{
	...
	if(get_property("lh_hiddenunlock") != "nose" && get_property("questL11Worship") != "step2")
	{
		return false;
	}
	if(get_property("lh_mcmuffin") != "start")
	...
	if(lh_my_path() == "G-Lover" || lh_my_path() == "Two Crazy Random Summer")
	{
		if(my_adventures() <= 3)
		{
			return false;
	...
	boolean bypassResult = lhAdvBypass(280);
...
#	}
	...
	if(get_property("lh_hiddenunlock") != "")
	{
		return false;
	}
	...
	{
		lhAdv(1, $location[The Hidden Temple]);
	}
	...
	{
		set_property("lh_hiddenunlock", "nose");
...
boolean LX_spookyBedroomCombat()
{
	...
	lhAdv(1, $location[The Haunted Bedroom]);
...
{
	if((get_property("lh_spookyravensecond") != "") || (get_property("lastSecondFloorUnlock").to_int() < my_ascensions()))
	{
		return false;
	}
	...
	if(my_level() >= 7)
	{
		if((get_property("lh_beatenUpCount").to_int() > 7) && (get_property("lh_crypt") != "finished"))
		{
			return false;
		}
	}
	...
	if((item_amount($item[Lady Spookyraven\'s Powder Puff]) == 1) && (item_amount($item[Lady Spookyraven\'s Dancing Shoes]) == 1) && (item_amount($item[Lady Spookyraven\'s Finest Gown]) == 1))
	{
		...
		visit_url("place.php?whichplace=manor2&action=manor2_ladys");
		set_property("lh_ballroomopen", "open");
		lhAdv(1, $location[The Haunted Ballroom]);
		#
	...
	boolean needCamera = (item_amount($item[disposable instant camera]) == 0) && !get_property("lh_palindome").to_boolean();
	if(my_class() == $class[Avatar of Boris])
	{
	...
	if((lh_my_path() == "Nuclear Autumn") && in_hardcore())
	{
		needSpectacles = false;
	}
	if(needSpectacles)
	...
	if((get_property("lh_ballroomopen") == "open") || (get_property("questM21Dance") == "finished") || (get_property("questM21Dance") == "step3"))
	{
		if(needSpectacles)
		{
		...
		{
			set_property("lh_spookyravensecond", "finished");
		}
		return true;
	}
	else if(get_property("questM21Dance") != "finished")
	{
		...
		if(item_amount($item[Lady Spookyraven\'s Dancing Shoes]) == 0)
		{
			...
			lh_sourceTerminalEducate($skill[Extract], $skill[Portscan]);

			lhAdv(1, $location[The Haunted Gallery]);
			return true;
		}
		if(item_amount($item[Lady Spookyraven\'s Powder Puff]) == 0)
		{
			if((my_daycount() == 1) && (get_property("_hipsterAdv").to_int() < 7) && is_unrestricted($familiar[Artistic Goth Kid]) && lh_have_familiar($familiar[Artistic Goth Kid]))
			{
				handleFamiliar($familiar[Artistic Goth Kid]);
			}
			...
			lhAdv(1, $location[The Haunted Bathroom]);
...
boolean L11_mauriceSpookyraven()
{
	...
	if(get_property("lh_ballroomflat") == "")
	{
		...
		if(!lhAdv(1, $location[The Haunted Ballroom]))
		{
	...
	if(item_amount($item[recipe: mortar-dissolving solution]) == 0)
	{
		...
		if(!useMaximizeToEquip())
		{
			lhEquip($slot[acc3], $item[numberwang]);
		}
	...
	if((get_property("lh_winebomb") == "finished") || get_property("lh_masonryWall").to_boolean() || (internalQuestStatus("questL11Manor") >= 3))
	{
		...
		# The lhAdvBypass case is probably suitable for Ed but we'd need to verify it.
		if(my_class() == $class[Ed])
		{
		...
		{
			lhAdv($location[Summoning Chamber]);
		}
	...
	if(!possessEquipment($item[Lord Spookyraven\'s Spectacles]) || (my_class() == $class[Avatar of Boris]) || (lh_my_path() == "Way of the Surprising Fist") || ((lh_my_path() == "Nuclear Autumn") && !get_property("lh_haveoven").to_boolean()))
	{
		...
		{
			lhAdv($location[The Haunted Kitchen]);
			return true;
		}
		if(item_amount($item[Powdered Castoreum]) == 0)
		{
			lhAdv($location[The Haunted Conservatory]);
			return true;
		}
		if(item_amount($item[Drain Dissolver]) == 0)
		{
			if(get_property("lh_towelChoice") == "")
			{
				set_property("lh_towelChoice", get_property("choiceAdventure882"));
				set_property("choiceAdventure882", 2);
			}
			lhAdv($location[The Haunted Bathroom]);
			if(get_property("lh_towelChoice") != "")
			{
				set_property("choiceAdventure882", get_property("lh_towelChoice"));
				set_property("lh_towelChoice", "");
			}
			return true;
		}
		if(item_amount($item[Triple-Distilled Turpentine]) == 0)
		{
			lhAdv($location[The Haunted Gallery]);
			return true;
		}
		if(item_amount($item[Detartrated Anhydrous Sublicalc]) == 0)
		{
			lhAdv($location[The Haunted Laboratory]);
			return true;
		}
		if(item_amount($item[Triatomaceous Dust]) == 0)
		{
			lhAdv($location[The Haunted Storage Room]);
			return true;
		}
	...
	if((item_amount($item[blasting soda]) == 1) && (item_amount($item[bottle of Chateau de Vinegar]) == 1))
	{
		...
		ovenHandle();
		lhCraft("cook", 1, $item[bottle of Chateau de Vinegar], $item[blasting soda]);
		set_property("lh_winebomb", "partial");
	...
	if((item_amount($item[bottle of Chateau de Vinegar]) == 0) && (get_property("lh_winebomb") == ""))
	{
		...
		lhAdv(1, $location[The Haunted Wine Cellar]);
		return true;
	}
	if((item_amount($item[blasting soda]) == 0) && (get_property("lh_winebomb") == ""))
	{
		...
		lhAdv(1, $location[The Haunted Laundry Room]);
		return true;
	}

	if(get_property("lh_winebomb") == "partial")
	{
		if(item_amount($item[unstable fulminate]) > 0)
		{
			if(weapon_hands(equipped_item($slot[weapon])) > 1)
			{
				lhEquip($slot[weapon], $item[none]);
			}
		}
		print("Now we mix and heat it up.", "blue");

		if((lh_my_path() == "Picky") && (item_amount($item[gumshoes]) > 0))
		{
			lh_change_mcd(0);
			lhEquip($slot[acc2], $item[gumshoes]);
		}

		if(!lhEquip($item[Unstable Fulminate]))
		{
		...
		lhAdv(1, $location[The Haunted Boiler Room]);
...
boolean L13_sorceressDoor()
{
	if(contains_text(visit_url("place.php?whichplace=nstower"), "ns_05_monster1"))
	{
		set_property("lh_sorceress", "tower");
		return false;
	}
	if(get_property("lh_sorceress") != "door")
	{
	...
	if((item_amount($item[white pixel]) >= 30) && (item_amount($item[Digital Key]) == 0))
	{
		...
		set_property("lh_crackpotjar", "finished");
	}
...
{
	if(get_property("lh_mcmuffin") != "pyramid")
	{
		return false;
	}
	...
	if(get_property("lh_tavern") != "finished")
	{
	...
	{
		lhAdv(1, $location[The Upper Chamber]);
		return true;
	}
	...
	{
		if(!contains_text(get_property("lh_banishes"), $monster[Tomb Servant]) && !contains_text(get_property("lh_banishes"), $monster[Tomb Asp]) && (get_property("olfactedMonster") != $monster[Tomb Rat]))
		{
			lhAdv(1, $location[The Upper Chamber]);
			return true;
		}
	...
	lhAdv(1, $location[The Middle Chamber]);
	return true;
}
...
boolean L11_unlockPyramid()
{
	...
	{
		set_property("lh_mcmuffin", "pyramid");
		return true;
	}
	...
	if((item_amount($item[[2325]Staff Of Ed]) > 0) || ((item_amount($item[[2180]Ancient Amulet]) > 0) && (item_amount($item[[2268]Staff Of Fats]) > 0) && (item_amount($item[[2286]Eye Of Ed]) > 0)))
	{
		...
		if(item_amount($item[[2325]Staff Of Ed]) == 0)
		{
			...
			{
				lhCraft("combine", 1, $item[[2180]Ancient Amulet], $item[[2286]Eye Of Ed]);
			}
			if((item_amount($item[Headpiece of the Staff of Ed]) > 0) && (item_amount($item[[2268]Staff Of Fats]) > 0))
			{
				lhCraft("combine", 1, $item[headpiece of the staff of ed], $item[[2268]Staff Of Fats]);
			}
		}
		if(item_amount($item[[2325]Staff Of Ed]) == 0)
		{
		...
		if(get_property("questL11Pyramid") == "unstarted")
		{
			...
			if((lh_my_path() == "One Crazy Random Summer") && (get_property("desertExploration").to_int() == 100))
			{
			...
			if(my_turncount() == get_property("lh_skipDesert").to_int())
			{
...
{
	if(get_property("lh_mcmuffin") != "ed")
	{
		return false;
	}
	if(my_adventures() <= 7)
	...
	if(in_koe())
	{
		...
		lhForceEquip($item[low-pressure oxygen tank]);
	}
	...
	while(item_amount($item[[2334]Holy MacGuffin]) == 0)
	{
		...
		lhAdv(1, $location[The Lower Chambers]);
		...
		if(lh_my_path() == "Pocket Familiars" || in_koe())
		{
			cli_execute("refresh inv");
		}
	}
...
#	if(get_property("lh_hippyInstead").to_boolean() && (get_property("fratboysDefeated").to_int() < 64))
#	{
#		return false;
#	}
	if(!haveWarOutfit())
...
boolean L12_gremlinStart()
{
	...
	if(get_property("lh_prewar") != "started")
	{
		return false;
	}
	if(get_property("lh_gremlins") != "")
	{
		return false;
	}
	...
	if((!possessEquipment($item[Ouija Board\, Ouija Board])) && (item_amount($item[Lump of Brituminous Coal]) > 0))
	{
		...
		lhCraft("smith", 1, $item[lump of Brituminous coal], $item[turtle totem]);
		if(!possessEquipment($item[Turtle Totem]))
		{
			acquireGumItem($item[Turtle Totem]);
		}
...
#		lhCraft("paste", 1, $item[Ben-Gal&trade; Balm], $item[Handful of Smithereens]);
	}
	set_property("lh_gremlins", "start");
	set_property("lh_gremlinBanishes", "");
	return true;
}
...
boolean L12_gremlins()
{
	...
	if(get_property("lh_gremlins") != "start")
	{
		return false;
	}
	if(my_level() < 12)
	...
	if(get_property("lh_hippyInstead").to_boolean() && (get_property("fratboysDefeated").to_int() < 192))
	{
		return false;
	}
	if(lh_my_path() == "Pocket Familiars")
	...
	{
		abort("Do gremlins manually, sorry. Or set lh_gremlins=finished and we will just skip them");
	}
	...
	handleFamiliar(lh_beta() ? "gremlins" : "init");
	songboomSetting("dr");
	if(item_amount($item[molybdenum hammer]) == 0)
	{
		lhAdv(1, $location[Next to that barrel with something burning in it], "ccsJunkyard");
		return true;
	}

	if(item_amount($item[molybdenum screwdriver]) == 0)
	{
		lhAdv(1, $location[Out by that rusted-out car], "ccsJunkyard");
		return true;
	}

	if(item_amount($item[molybdenum crescent wrench]) == 0)
	{
		lhAdv(1, $location[over where the old tires are], "ccsJunkyard");
		return true;
	}

	if(item_amount($item[Molybdenum Pliers]) == 0)
	{
		lhAdv(1, $location[near an abandoned refrigerator], "ccsJunkyard");
		return true;
	}
	handleFamiliar("item");
	warOutfit(true);
...
#		return false;
#	}
	if((get_property("fratboysDefeated").to_int() < 64) && get_property("lh_hippyInstead").to_boolean())
	{
		return false;
	}
	if(item_amount($item[barrel of gunpowder]) >= 5)
	...
	if(lh_my_path() != "Live. Ascend. Repeat.")
	{
	...
	{
		set_property("lh_doCombatCopy", "yes");
	}

	lhAdv(1, $location[Sonofa Beach]);
	set_property("lh_doCombatCopy", "no");
	handleFamiliar("item");
...
boolean L12_sonofaPrefix()
{
	...
	if(item_amount($item[barrel of gunpowder]) >= 4 && !lh_voteMonster())
	{
		return false;
	}
	if(item_amount($item[barrel of gunpowder]) >= 5)
	...
	if(!(lh_get_campground() contains $item[Source Terminal]))
	{
		if(possessEquipment($item[&quot;I voted!&quot; sticker]))
		{
			if(lh_voteMonster() && lh_have_skill($skill[Meteor Lore]) && (get_property("_macrometeoriteUses").to_int() < 10))
			{
				if(item_amount($item[barrel of gunpowder]) < 4)
				{
					set_property("lh_doCombatCopy", "yes");
				}
				set_property("lh_combatDirective", "start;skill macrometeorite");
				lh_voteMonster(false, $location[Sonofa Beach], "");
				set_property("lh_combatDirective", "");
				set_property("lh_doCombatCopy", "no");
				return true;
			}
		}
	...
	if((my_mp() < mp_cost($skill[Digitize])) && (lh_get_campground() contains $item[Source Terminal]) && is_unrestricted($item[Source Terminal]) && (get_property("_sourceTerminalDigitizeMonster") != $monster[Lobsterfrogman]) && (get_property("_sourceTerminalDigitizeUses").to_int() < 3))
	{
		equipBaseline();
		return false;
	}
	...
	if(possessEquipment($item[&quot;I voted!&quot; sticker]) && (my_adventures() > 15))
	{
		if(have_skill($skill[Meteor Lore]) && (get_property("_macrometeoriteUses").to_int() < 10))
		{
			...
			{
				set_property("lh_combatDirective", "start;skill macrometeorite");
				lhEquip($slot[acc3], $item[&quot;I voted!&quot; sticker]);
			}
			else
			{
				return false;
...
boolean L12_filthworms()
{
	...
	if(lh_my_path() == "Two Crazy Random Summer")
	{
		return false;
	}
	if(item_amount($item[Heart of the Filthworm Queen]) > 0)
	{
		set_property("lh_orchard", "done");
	}
	if(get_property("lh_orchard") != "start")
	{
		return false;
	}
	if(get_property("lh_prewar") == "")
	{
		return false;
	}
	...
	{
		handleFamiliar("meat");
		lhAdv(1, $location[The Filthworm Queen\'s Chamber]);
		if(item_amount($item[Heart of the Filthworm Queen]) > 0)
		{
			set_property("lh_orchard", "done");
		}
	...
	if((!possessEquipment($item[A Light That Never Goes Out])) && (item_amount($item[Lump of Brituminous Coal]) > 0))
	{
		...
		lhCraft("smith", 1, $item[Lump of Brituminous Coal], $item[third-hand lantern]);
	}
	...
	{
		if(lh_have_familiar($familiar[XO Skeleton]) && (get_property("_xoHugsUsed").to_int() <= 10) && !is100FamiliarRun($familiar[XO Skeleton]))
		{
			handleFamiliar($familiar[XO Skeleton]);
		}
		lhAdv(1, $location[The Royal Guard Chamber]);
	}
	else if(have_effect($effect[Filthworm Larva Stench]) > 0)
	{
		...
		lhAdv(1, $location[The Feeding Chamber]);
	}
	else
	{
		if(lh_have_familiar($familiar[XO Skeleton]) && (get_property("_xoHugsUsed").to_int() <= 10) && !is100FamiliarRun($familiar[XO Skeleton]))
		...
		lhAdv(1, $location[The Hatching Chamber]);
	}
	handleFamiliar("item");
	return true;
}
...
#	cli_execute("refresh equip");
	...
	else if(get_property("lh_hippyInstead").to_boolean())
	{
...
#	handlePreAdventure(bossFight);
	...
	if(!lhAdvBypass(0, pages, bossFight, ""))
	{
...
boolean LX_malware()
{
	...
	set_property("lh_combatDirective", "start;item daily dungeon malware");
	lhAdv($location[The Daily Dungeon]);
	set_property("lh_combatDirective", "");
...
boolean LX_getDigitalKey()
{
	...
	if(((get_property("lh_war") != "finished") && (get_property("lh_war") != "")) && (item_amount($item[Rock Band Flyers]) == 0) && (item_amount($item[Jam Band Flyers]) == 0) && (get_property("lh_powerLevelAdvCount").to_int() < 9))
	{
		return false;
	}
	...
	if(get_property("lh_crackpotjar") == "")
	{
		if(item_amount($item[Jar Of Psychoses (The Crackpot Mystic)]) == 0)
		{
		...
		if(item_amount($item[Jar Of Psychoses (The Crackpot Mystic)]) == 0)
		{
			set_property("lh_crackpotjar", "fail");
		}
		else
		{
			woods_questStart();
			use(1, $item[Jar Of Psychoses (The Crackpot Mystic)]);
			set_property("lh_crackpotjar", "done");
		}
	}
	...
	#}
	if(get_property("lh_crackpotjar") == "done")
	{
		set_property("choiceAdventure644", 3);
		lhAdv(1, $location[Fear Man\'s Level]);
		if(have_effect($effect[Consumed By Fear]) == 0)
		{
	...
	else if(get_property("lh_crackpotjar") == "fail")
	{
		woods_questStart();
		lhEquip($slot[acc3], $item[Continuum Transfunctioner]);
		if(lh_saberChargesAvailable() > 0)
		{
			lhEquip($item[Fourth of May cosplay saber]);
		}
		lhAdv(1, $location[8-bit Realm]);
	}
	return true;
}
...
boolean L11_getBeehive()
{
	...
	if(!get_property("lh_getBeehive").to_boolean())
	{
		return false;
	}
	...
	lhAdv(1, $location[The Black Forest]);
	if(item_amount($item[beehive]) > 0)
	{
		set_property("lh_getBeehive", false);
	}
...
{
	if((get_property("lh_orchard") == "finished") && (get_property("sidequestOrchardCompleted") == "none"))
	{
		if((get_property("hippiesDefeated").to_int() < 1000) && (get_property("fratboysDefeated").to_int() < 1000))
		{
	...
	if(get_property("lh_orchard") != "")
	{
		return false;
	}
	if((get_property("hippiesDefeated").to_int() < 64) && !get_property("lh_hippyInstead").to_boolean())
	{
		return false;
	}
	...
	set_property("lh_orchard", "start");
	return true;
}

boolean L12_orchardFinalize()
{
	if(get_property("lh_orchard") != "done")
	{
		return false;
	}
	if(!get_property("lh_hippyInstead").to_boolean() && (get_property("hippiesDefeated").to_int() < 64))
	{
		return false;
	}
	set_property("lh_orchard", "finished");
...
boolean L10_plantThatBean()
{
	...
	if(get_property("lh_bean").to_boolean())
	{
		return false;
	}
	...
	if(internalQuestStatus("questL04Bat") >= 0)
	{
		...
		return lhAdv($location[The Beanbat Chamber]);
	}
	return false;
...
boolean L10_holeInTheSkyUnlock()
{
	...
	if(get_property("lh_castleground") != "finished")
	{
		return false;
	}

	if(!get_property("lh_holeinthesky").to_boolean())
	{
		return false;
	}
	if(item_amount($item[Steam-Powered Model Rocketship]) > 0)
	...
	if(lh_have_familiar($familiar[Ms. Puck Man]))
	{
		handleFamiliar($familiar[Ms. Puck Man]);
	}
	else if(lh_have_familiar($familiar[Puck Man]))
	{
...
boolean L10_ground()
{
	...
	if(get_property("lh_castlebasement") != "finished")
	{
		return false;
	}

	if(get_property("lh_castleground") != "")
	{
		return false;
	}
	...
	if((my_class() == $class[Gelatinous Noob]) && lh_have_familiar($familiar[Robortender]))
	{
...
boolean L10_basement()
{
	...
	if(get_property("lh_castlebasement") != "")
	{
		return false;
	}
	if(my_adventures() < 3)
	...
	lhAdv(1, $location[The Castle in the Clouds in the Sky (Basement)]);
	handleFamiliar("item");
	...
	else if(contains_text(get_property("lastEncounter"), "The Fast and the Furry-ous"))
	{
		...
		lhEquip($item[Titanium Assault Umbrella]);
		set_property("choiceAdventure669", "1");
		lhAdv(1, $location[The Castle in the Clouds in the Sky (Basement)]);
	...
	else if(contains_text(get_property("lastEncounter"), "You Don\'t Mess Around with Gym"))
	{
		...
		if(!possessEquipment($item[Amulet Of Extreme Plot Significance]))
		{
			...
			if(!possessEquipment($item[Amulet of Extreme Plot Significance]))
			{
				...
				else
				{
					...
					lhAdv(1, $location[The Penultimate Fantasy Airship]);
				}
				return true;
			}
		}
		...
		if(!lhEquip($slot[acc3], $item[Amulet Of Extreme Plot Significance]))
		{
...
boolean LX_freeCombats()
{
	...
	if((lh_my_path() != "Disguises Delimit") && neverendingPartyCombat())
	{
		return true;
	}

	if(lh_have_familiar($familiar[Machine Elf]) && (get_property("_machineTunnelsAdv").to_int() < 5) && (my_adventures() > 0) && !is100FamiliarRun())
	{
		if(get_property("lh_choice1119") != "")
		{
			set_property("choiceAdventure1119", get_property("lh_choice1119"));
		}
		set_property("lh_choice1119", get_property("choiceAdventure1119"));
		...
		handleFamiliar($familiar[Machine Elf]);
		lhAdv(1, $location[The Deep Machine Tunnels]);
		if(bjorn == $familiar[Machine Elf])
		{
	...
	if(snojoFightAvailable() && (my_adventures() > 0))
	{
		...
		lhAdv(1, $location[The X-32-F Combat Training Snowman]);
		handleFamiliar("item");
		if(get_property("_lh_digitizeDeskCounter").to_int() > 2)
		{
			set_property("_lh_digitizeDeskCounter", get_property("_lh_digitizeDeskCounter").to_int() - 1);
		}
		loopHandlerDelayAll();
		return true;
	}
...
{
	if(lh_have_familiar($familiar[Machine Elf]) && (get_property("_machineTunnelsAdv").to_int() < 5) && (my_adventures() > 10) && !is100FamiliarRun() && ($location[The Deep Machine Tunnels].turns_spent == 5) && (my_daycount() == 2))
	{
		if(get_property("lh_choice1119") != "")
		{
			set_property("choiceAdventure1119", get_property("lh_choice1119"));
...
boolean L0_handleRainDoh()
{
	...
	if(enemy == $monster[Ninja Snowman Assassin])
	{
		...
		if(count == 3)
		{
			set_property("lh_ninjasnowmanassassin", "1");
		}
		return true;
	}
...
{
	if(!get_property("lh_grimstoneOrnateDowsingRod").to_boolean())
	{
		return false;
	}
	...
	{
		lhAdv(1, $location[The Prince\'s Balcony]);
	}
	while(item_amount($item[odd silver coin]) < 2)
	{
		lhAdv(1, $location[The Prince\'s Dance Floor]);
	}
	while(item_amount($item[odd silver coin]) < 3)
	{
		lhAdv(1, $location[The Prince\'s Lounge]);
	}
	while(item_amount($item[odd silver coin]) < 4)
	{
		lhAdv(1, $location[The Prince\'s Kitchen]);
	}
	while(item_amount($item[odd silver coin]) < 5)
	{
		lhAdv(1, $location[The Prince\'s Restroom]);
	}
...
boolean L7_crypt()
{
	if((my_level() < 7) || (get_property("lh_crypt") != ""))
	{
		return false;
	}
	...
	if((get_property("romanticTarget") != $monster[modern zmobie]) && (get_property("lh_waitingArrowAlcove").to_int() < 50))
	{
		set_property("lh_waitingArrowAlcove", 50);
	}
	...
	void useNightmareFuelIfPossible()
	{
		...
		{
			lhChew(1, $item[Nightmare Fuel]);
		}
	}

	if((get_property("cyrptAlcoveEvilness").to_int() > 0) && ((get_property("cyrptAlcoveEvilness").to_int() <= get_property("lh_waitingArrowAlcove").to_int()) || (get_property("cyrptAlcoveEvilness").to_int() <= 25)) && edAlcove && canGroundhog($location[The Defiled Alcove]))
	{
		handleFamiliar("init");

		if((get_property("_badlyRomanticArrows").to_int() == 0) && lh_have_familiar($familiar[Reanimated Reanimator]) && (my_daycount() == 1))
		{
			handleFamiliar($familiar[Reanimated Reanimator]);
		}
		...
		lhEquip($item[Gravy Boat]);

		if(!useMaximizeToEquip() && (get_property("cyrptAlcoveEvilness").to_int() > 26))
		{
			lhEquip($item[The Nuge\'s Favorite Crossbow]);
		}
		bat_formBats();
		...
		lhAdv(1, $location[The Defiled Alcove]);
		handleFamiliar("item");
		return true;
	}
	...
	if((get_property("cyrptNookEvilness").to_int() > 0) && canGroundhog($location[The Defiled Nook]))
	{
		...
		{
			lhEquip($item[broken champagne bottle]);
		}
		...
		lhAdv(1, $location[The Defiled Nook]);
		while((item_amount($item[Evil Eye]) > 0) && lh_is_valid($item[Evil Eye]) && (get_property("cyrptNookEvilness").to_int() > 25))
		{
			use(1, $item[Evil Eye]);
		}
		return true;
	...
	if((get_property("cyrptNicheEvilness").to_int() > 0) && canGroundhog($location[The Defiled Niche]))
	{
		...
		lhAdv(1, $location[The Defiled Niche]);
	...
	if(get_property("cyrptCrannyEvilness").to_int() > 0)
	{
		...
		lhAdv(1, $location[The Defiled Cranny]);
		return true;
	}
	...
	if(get_property("cyrptTotalEvilness").to_int() <= 0)
	{
		...
		boolean tryBoner = lhAdv(1, $location[Haert of the Cyrpt]);
		council();
		cli_execute("refresh quests");
		if(item_amount($item[chest of the bonerdagon]) == 1)
		{
			set_property("lh_crypt", "finished");
			use(1, $item[chest of the bonerdagon]);
			lh_badassBelt(); // mafia doesn't make this any more even if autoCraft = true for some random reason so lets do it manually.
		}
...
boolean LX_hardcoreFoodFarm()
{
	...
	if(my_level() >= 8)
	{
		...
		{
			lh_sourceTerminalEducate($skill[Extract], $skill[Duplicate]);
		}
		lhAdv(1, $location[The Goatlet]);
		lh_sourceTerminalEducate($skill[Extract], $skill[Portscan]);
	}
...
{
	if((my_level() < 6) || (get_property("lh_friars") != "") || (my_adventures() == 0))
	{
		return false;
	}
...
#	}
	...
	{
		lhEquip($slot[Shirt], $item[none]);
	}

	providePlusNonCombat(25);

	if(lh_have_familiar($familiar[Space Jellyfish]) && (get_property("_spaceJellyfishDrops").to_int() < 2))
	{
		handleFamiliar($familiar[Space Jellyfish]);
	}
	...
	if(item_amount($item[box of birthday candles]) == 0)
	{
		...
		lhAdv(1, $location[The Dark Heart of the Woods]);
		return true;
	}
	...
	if(item_amount($item[dodecagram]) == 0)
	{
		...
		lhAdv(1, $location[The Dark Neck of the Woods]);
		return true;
	}
	if(item_amount($item[eldritch butterknife]) == 0)
	{
		print("Getting Eldritch Butterknife", "blue");
		lhAdv(1, $location[The Dark Elbow of the Woods]);
		return true;
	}
	print("Finishing friars", "blue");
	visit_url("friars.php?action=ritual&pwd");
	council();
	set_property("lh_friars", "finished"); # used to be done, but no longer need hot wings
	return true;
}

boolean LX_steelOrgan()
{
	if(!get_property("lh_getSteelOrgan").to_boolean())
	{
		return false;
	}
	if((get_property("lh_friars") != "done") && (get_property("lh_friars") != "finished"))
	{
		return false;
	}
	if(my_adventures() == 0)
	...
	if($classes[Ed, Gelatinous Noob, Vampyre] contains my_class())
	{
		...
		set_property("lh_getSteelOrgan", false);
		return false;
	}
	if((lh_my_path() == "Nuclear Autumn") || (lh_my_path() == "License to Adventure"))
	{
	...
	if(get_property("questM10Azazel") == "started")
	{
		if(((item_amount($item[Observational Glasses]) == 0) || (item_amount($item[Imp Air]) < 5)) && (item_amount($item[Azazel\'s Tutu]) == 0))
		{
			...
			lhAdv(1, $location[The Laugh Floor]);
		}
		else if(((item_amount($item[Azazel\'s Unicorn]) == 0) || (item_amount($item[Bus Pass]) < 5)) && (item_amount($item[Azazel\'s Tutu]) == 0))
		{
			...
			handleFamiliar("item");
			lhAdv(1, $location[Infernal Rackets Backstage]);
		}
	...
	else if(get_property("questM10Azazel") == "finished")
	{
		...
		{
			lhDrink(1, $item[Steel Margarita]);
		}
		if((item_amount($item[Steel-Scented Air Freshener]) > 0) && (spleen_left() >= 5))
		{
			lhChew(1, $item[Steel-Scented Air Freshener]);
		}
	}
	return false;
}
...
{
	if(get_property("lh_friars") != "done")
	{
		return false;
	}
	if((get_property("lh_pirateoutfit") == "almost") || (get_property("lh_pirateoutfit") == "finished") )
	{
		set_property("lh_friars", "finished");
		return false;
	}
	...
	if((item_amount($item[Hot Wing]) >= 3) || (lh_my_path() == "Pocket Familiars"))
	{
		set_property("lh_friars", "finished");
		return true;
	}
...
boolean LX_fancyOilPainting()
{
	...
	if(!get_property("lh_grimstoneFancyOilPainting").to_boolean())
	{
		return false;
	}
	if(get_counters("", 0, 6) != "")
...
boolean L9_leafletQuest()
{
	...
	if(lh_get_campground() contains $item[Frobozz Real-Estate Company Instant House (TM)])
	{
		return false;
	}
...
{
	if(get_property("lh_trapper") != "start")
	{
		return false;
	}
	...
	else if((my_rain() > 50) && (have_effect($effect[Ultrahydrated]) == 0) && (lh_my_path() == "Heavy Rains") && have_skill($skill[Rain Man]))
	{
		print("Trying to summon a mountain man", "blue");
		set_property("lh_mountainmen", "1");
		return rainManSummon("mountain man", false, false);
	}
	else if(lh_my_path() == "Heavy Rains")
	{
	...
	else if (canGenieCombat() && (get_property("lh_useWishes").to_boolean()) && (catBurglarHeistsLeft() >= 2))
	{
	...
	else if((my_level() >= 12) && in_hardcore())
	{
		...
		{
			numCloversKeep = 1;
			if(get_property("lh_powerLevelLastLevel").to_int() == my_level())
			{
				numCloversKeep = 0;
			}
		}
		...
		{
			cloverUsageInit();
			lhAdvBypass(270, $location[Itznotyerzitz Mine]);
			cloverUsageFinish();
			return true;
		}
	}
...
boolean LX_guildUnlock()
{
	...
	if((lh_my_path() == "Nuclear Autumn") || (lh_my_path() == "Pocket Familiars"))
	{
		return false;
	}
...
{
	if((my_level() < 8) || (get_property("lh_trapper") != ""))
	{
		return false;
	}
	council();
...
{
	if(get_property("lh_goblinking") == "finished")
	{
		return false;
	}
	if((my_level() < 8) && (get_property("lh_powerLevelAdvCount").to_int() < 9))
	{
		return false;
	}
	if(my_adventures() <= 2)
	...
	if(!lhOutfit("knob goblin harem girl disguise"))
	{
	...
	{
		lhAdv(1, $location[Cobb\'s Knob Harem]);
	...
	{
		lhEquip($slot[acc2], $item[none]);
	}

	lhAdv(1, $location[Throne Room]);
	cli_execute("refresh quests");
...
boolean L4_batCave()
{
	if(get_property("lh_bat") == "finished")
	{
		return false;
	}
	if(my_level() < 4)
	...
	if(batStatus >= 4)
	{
		if((item_amount($item[Enchanted Bean]) == 0) && !get_property("lh_bean").to_boolean() && (my_class() != $class[Ed]))
		{
			lhAdv(1, $location[The Beanbat Chamber]);
			return true;
		}
		set_property("lh_bat", "finished");
		council();
	...
	if(batStatus >= 3)
	{
		...
		lhAdv(1, $location[The Boss Bat\'s Lair]);
	...
	if(batStatus >= 2)
	{
		...
		lhAdv(1, $location[The Batrat and Ratbat Burrow]);
		return true;
	}
	if(batStatus >= 1)
	{
	...
	if(elemental_resist($element[stench]) < 1)
	{
		if(!useMaximizeToEquip())
		{
			...
			{
				if(get_property("lh_powerLevelAdvCount").to_int() >= 5)
				{
					bat_formBats();
					lhAdv(1, $location[The Bat Hole Entrance]);
					return true;
				}
	...
	{
		cloverUsageInit();
		lhAdvBypass(31, $location[Guano Junction]);
		cloverUsageFinish();
		return true;
	}

	bat_formBats();
	lhAdv(1, $location[Guano Junction]);
	return true;
}
...
boolean LX_craftAcquireItems()
{
	...
	{
		if((have_effect($effect[Adventurer\'s Best Friendship]) > 30) && lh_have_familiar($familiar[Mosquito]))
		{
			set_property("choiceAdventure1106", 3);
		}
		else
	...
	if((item_amount($item[snow berries]) == 3) && (my_daycount() == 1) && get_property("lh_grimstoneFancyOilPainting").to_boolean())
	{
	...
	if(knoll_available() && (item_amount($item[Detuned Radio]) == 0) && (my_meat() > 300) && (lh_my_path() != "G-Lover"))
	{
	...
	if((item_amount($item[Antique Accordion]) == 0) && (item_amount($item[Aerogel Accordion]) == 0) && isUnclePAvailable() && (my_meat() > 12500) && (have_skill($skill[The Ode to Booze])) && (my_class() != $class[Accordion Thief]) && (lh_my_path() != "G-Lover"))
	{
		buyUpTo(1, $item[Antique Accordion]);
	}
	...
	if((get_power(equipped_item($slot[pants])) < 70) && !possessEquipment($item[Demonskin Trousers]) && (my_meat() > 350) && (item_amount($item[Demon Skin]) > 0) && (item_amount($item[Tenderizing Hammer]) > 0) && knoll_available())
	{
		...
		lhCraft("smith", 1, $item[Pants Kit], $item[Demon Skin]);
	}
	...
	{
		buyUpTo(1, $item[Pants Kit]);
		lhCraft("smith", 1, $item[Pants Kit], $item[White Snake Skin]);
	}
	...
	if(item_amount($item[Metal Meteoroid]) > 0 && !in_tcrs())
	{
		...
		if(!possessEquipment(it) && (get_power(equipped_item($slot[Hat])) < 140) && (get_property("lh_beatenUpCount").to_int() >= 5))
		{
			int choice = 1 + to_int(it) - to_int($item[Meteortarboard]);
			string temp = visit_url("inv_use.php?pwd=&which=3&whichitem=9516");
			temp = visit_url("choice.php?pwd=&whichchoice=1264&option=" + choice);
		...
		if(!possessEquipment(it) && !possessEquipment($item[Kol Con 13 Snowglobe]) && (get_property("lh_beatenUpCount").to_int() >= 5))
		{
			int choice = 1 + to_int(it) - to_int($item[Meteortarboard]);
			string temp = visit_url("inv_use.php?pwd=&which=3&whichitem=9516");
			temp = visit_url("choice.php?pwd=&whichchoice=1264&option=" + choice);
...
boolean adventureFailureHandler()
{
	if(my_location().turns_spent > 52)
	{
		...
		int plCount = get_property("lh_powerLevelAdvCount").to_int();
		...
		{
			if(get_property("lh_newbieOverride").to_boolean())
			{
				set_property("lh_newbieOverride", false);
				print("We have spent " + my_location().turns_spent + " turns at '" + my_location() + "' and that is bad... override accepted.", "red");
			}
			else
			{
				print("You can set lh_newbieOverride = true to bypass this once.", "blue");
...
{
	if((have_effect($effect[Beaten Up]) > 0) && (lh_my_path() == "Community Service") && (last_monster() != $monster[X-32-F Combat Training Snowman]))
	{
	...
	if((have_effect($effect[Beaten Up]) > 0) && (lh_my_path() == "The Source") && (last_monster() == $monster[Source Agent]))
	{
		if((my_mp() > 100) && have_skill($skill[Tongue of the Walrus]) && have_skill($skill[Cannelloni Cocoon]))
		{
			use_skill($skill[Tongue of the Walrus]);
	...
	if(have_effect($effect[Beaten Up]) > 0)
	{
		...
		{
			if(get_property("lh_beatenUpCount").to_int() > 10)
			{
...
boolean LX_meatMaid()
{
	if(lh_get_campground() contains $item[Meat Maid])
	{
		return false;
	}
	if(!knoll_available() || (my_daycount() != 1) || (get_property("lh_crypt") != "finished"))
	{
		return false;
	}
	...
	{
		cloverUsageInit();
		lhAdvBypass(58, $location[The VERY Unquiet Garves]);
		cloverUsageFinish();
...
{
	if((item_amount($item[Bitchin\' Meatcar]) > 0) || (lh_my_path() == "Nuclear Autumn"))
	{
		return false;
	}
	...
	else
	{
		...
		{
			if(!lhAdv(1, $location[The Degrassi Knoll Garage]))
			{
...
boolean LX_desertAlternate()
{
	if(lh_my_path() == "Nuclear Autumn")
	{
		...
		if(item_amount($item[Funky Junk Key]) > 0)
		{
			...
			return lhAdv($location[The Old Landfill]);
		}
		else
		{
			return lhAdv($location[The Old Landfill]);
...
{
	if(get_property("lh_phatloot").to_int() >= my_daycount())
	{
		return false;
	}
	...
	if(fantasyRealmAvailable() && get_property("lh_sorceress") != "door")
	{
		return false;
	}

	if((!possessEquipment($item[Ring of Detect Boring Doors]) || (item_amount($item[Eleven-Foot Pole]) == 0) || (item_amount($item[Pick-O-Matic Lockpicks]) == 0)) && lh_have_familiar($familiar[Gelatinous Cubeling]))
	{
		if(!is100FamiliarRun($familiar[Gelatinous Cubeling]) && (lh_my_path() != "Pocket Familiars"))
		{
			return false;
		}
	}
	...
	lhEquip($slot[acc3], $item[Ring Of Detect Boring Doors]);
	...
	lhAdv(1, $location[The Daily Dungeon]);
	...
	{
		set_property("lh_phatloot", "" + my_daycount());
	}
	restoreSetting("choiceAdventure690");
...
{
	if(!get_property("lh_dakotaFanning").to_boolean())
	{
		return false;
	}
	...
	{
		lhAdv(1, $location[The Haunted Conservatory]);
		return true;
	}

	if(item_amount($item[Heavy-Duty Bendy Straw]) == 0)
	{
		if((get_property("lh_friars") != "finished") && (get_property("lh_friars") != "done"))
		{
			lhAdv(1, $location[The Dark Heart of the Woods]);
		}
		else
...
boolean L2_treeCoin()
{
	...
	if(get_property("lh_treecoin") == "finished")
	{
		return false;
	}
	...
	lhAdv(1, $location[The Spooky Forest]);
	return true;
}

boolean L2_spookyMap()
{
	if(get_property("lh_spookymap") == "finished")
	{
		return false;
	}
...
{
	if(get_property("lh_spookyfertilizer") == "finished")
	{
		return false;
	}
...
{
	if(get_property("lh_spookysapling") == "finished")
	{
		return false;
	}
	if(my_meat() < 100)
	...
	if(!lhAdvBypass("adventure.php?snarfblat=15", $location[The Spooky Forest]))
	{
...
boolean L2_mosquito()
{
	if(get_property("lh_mosquito") != "finished" && item_amount($item[mosquito larva]) > 0)
	{
		council();
		set_property("lh_mosquito", "finished");
		string temp = visit_url("tavern.php?place=barkeep");
	}
	if(get_property("lh_mosquito") == "finished")
	{
		return false;
	}
...
boolean LX_handleSpookyravenFirstFloor()
{
	...
	boolean delayKitchen = get_property("lh_delayHauntedKitchen").to_boolean();
	...
	if(delayKitchen)
	{
		...
		if(delayKitchen)
		{
			...
			if(((hot < 9) || (stench < 9)) && lh_have_skill($skill[Spectral Awareness]) && (have_effect($effect[Spectral Awareness]) == 0))
			{
				hot += 2;
				stench += 2;
				hpNeed += hp_cost($skill[Spectral Awareness]);
			}
			if(hot < 9 && lh_canBeachCombHead("hot"))
			{
				hot += 2;
			}
			if(stench < 9 && lh_canBeachCombHead("stench"))
			{
				stench += 2;
			}
			...
			if((my_mp() > mpNeed) && (my_hp() > hpNeed) && (hot >= 9) && (stench >= 9))
			{
				...
				if(elemental_resist($element[hot]) < 9) lh_beachCombHead("hot");
				if(elemental_resist($element[stench]) < 9) lh_beachCombHead("stench");
			}
			...
			if((get_property("lh_powerLevelAdvCount").to_int() > 7) && (get_property("lh_powerLevelLastLevel").to_int() == my_level()))
			{
				delayKitchen = false;
			}
		}
...
#	{
#		if(loopHandler("_lh_digitizeDeskTurn", "_lh_digitizeDeskCounter", "Potentially unable to do anything while waiting on digitized writing desks.", 10))
#		{
#			print("Have a digitized Writing Desk, let's not bother with the Spooky First Floor", "blue");
#		}
#		if((get_property("lh_war") != "finished") && (get_property("lh_powerLevelAdvCount").to_int() < 5) && (get_property("_lh_digitizeDeskCounter").to_int() < 5))
#		{
#			return false;
#		}
#	}
...
#			cli_execute("olfact writing desk");
		...
		lhAdv(1, $location[The Haunted Library]);
	}
	else if(item_amount($item[Spookyraven Billiards Room Key]) == 1)
	{
		...
		if(!possessEquipment($item[Pool Cue]) && !possessEquipment(staffOfFats) && !possessEquipment(staffOfFatsEd) && !possessEquipment(staffOfEd) && !in_tcrs())
		{
			...
			lhAdv(1, $location[The Haunted Billiards Room]);
		...
		lhEquip($slot[weapon], $item[Pool Cue]);
		lhEquip(staffOfFats);
		lhEquip(staffOfFatsEd);
		lhEquip(staffOfEd);
	...
	else
	{
		...
		if(is100FamiliarRun())
		{
			if(lh_have_familiar($familiar[Trick-or-Treating Tot]) && (available_amount($item[Li\'l Candy Corn Costume]) > 0))
			{
				handleFamiliar($familiar[Trick-or-Treating Tot]);
			}
		}
		...
		lhAdv(1, $location[The Haunted Kitchen]);
		handleFamiliar("item");
	}
	return true;
}
...
boolean L5_getEncryptionKey()
{
	...
	if(get_property("lh_day1_cobb") == "finished")
	{
		return false;
	}
	...
	lhAdv(1, $location[The Outskirts of Cobb\'s Knob]);
	return true;
}
...
boolean L12_startWar()
{
	...
	lhOutfit("frat warrior fatigues");
	if((my_mp() > 60) || considerGrimstoneGolem(true))
	{
		handleBjornify($familiar[Grimstone Golem]);
	}
	...
	lhAdv(1, $location[Wartime Hippy Camp]);
...
{
	if(get_property("lh_prehippy") == "done")
	{
		return false;
	}
	if(my_level() < 12)
	...
	if((get_property("lh_orcishfratboyspy") == "done") && !in_hardcore())
	{
	...
	if(!in_hardcore() && (lh_my_path() != "Heavy Rains"))
	{
		pullXWhenHaveY($item[Beer Helmet], 1, 0);
		pullXWhenHaveY($item[Bejeweled Pledge Pin], 1, 0);
		pullXWhenHaveY($item[Distressed Denim Pants], 1, 0);
	...
	if(possessEquipment($item[Beer Helmet]) && possessEquipment($item[Distressed Denim Pants]) && possessEquipment($item[Bejeweled Pledge Pin]))
	{
		...
		set_property("lh_prehippy", "done");
		visit_url("clan_viplounge.php?preaction=goswimming&subaction=submarine");
		return true;
	}

	if(get_property("lh_prehippy") == "firstOutfit")
	{
		lhOutfit("filthy hippy disguise");
		if(my_lightning() >= 5)
		{
			lhAdv(1, $location[Wartime Frat House]);
			return true;
		}
...
boolean L12_preOutfit()
{
	...
	if(get_property("lh_prehippy") != "")
	{
		return false;
	}
	if(have_outfit("Filthy Hippy Disguise"))
	{
		set_property("lh_prehippy", "firstOutfit");
		return true;
	}
	if(possessEquipment($item[Beer Helmet]) && possessEquipment($item[Distressed Denim Pants]) && possessEquipment($item[Bejeweled Pledge Pin]))
	{
...
boolean L9_highLandlord()
{
	...
	if(get_property("lh_highlandlord") == "finished")
	{
		return false;
	}

	if((my_class() == $class[Ed]) && !get_property("lh_chasmBusted").to_boolean())
	{
		return false;
	}

	if(get_property("lh_highlandlord") == "")
	{
		visit_url("place.php?whichplace=highlands&action=highlands_dude");
		set_property("lh_aboocount", "0");
		set_property("choiceAdventure296", "1");
		set_property("lh_highlandlord", "start");
		set_property("lh_grimstoneFancyOilPainting", false);
		return true;
	}
	...
	if((get_property("twinPeakProgress").to_int() == 15) && (get_property("lh_oilpeak") == "finished") && (get_property("lh_boopeak") == "finished"))
	{
...
boolean L9_aBooPeak()
{
	if(get_property("lh_boopeak") == "finished")
	{
		return false;
	}
	...
	if(get_property("lh_aboocount").to_int() < 5)
	{
		print("A-Boo Peak (initial): " + get_property("booPeakProgress"), "blue");
		set_property("lh_aboocount", get_property("lh_aboocount").to_int() + 1);
	...
	boolean clueCheck = ((clueAmt > 0) || (get_property("lh_aboopending").to_int() != 0));
	if(clueCheck && (get_property("booPeakProgress").to_int() > 2))
	{
		...
		lhMaximize("spooky res, cold res, 0.01hp " + lihcface + " -equip snow suit" + parrot, 0, 0, true);
		...
		else if((get_property("lh_blackmap") == "finished") && (item_amount($item[Can of Black Paint]) > 0) && (have_effect($effect[Red Door Syndrome]) == 0))
		{
			coldResist += 2;
			spookyResist += 2;
		}
		...
		loopHandler("_lh_lastABooConsider", "_lh_lastABooCycleFix", "We are in an A-Boo Peak cycle and can't find anything else to do. Aborting. If you have actual other quests left, please report this. Otherwise, complete A-Boo peak manually",15);
		...
		if(doThisBoo)
		{
			...
			{
				lhMaximize("spooky res, cold res " + lihcface + " -equip snow suit" + parrot, 0, 0, false);
			}
			adjustEdHat("ml");
			...
			lh_beachCombHead("cold");
			lh_beachCombHead("spooky");
			...
			if(get_property("lh_aboopending").to_int() == 0)
			{
				if(item_amount(clue) > 0)
				{
					use(1, clue);
				}
				set_property("lh_aboopending", my_turncount());
			}
			handleFamiliar($familiar[Exotic Parrot]);
	...
	else if(get_property("lh_abooclover").to_boolean() && (get_property("booPeakProgress").to_int() >= 40) && booCloversOk)
	{
		cloverUsageInit();
		lhAdvBypass(296, $location[A-Boo Peak]);
...
boolean L9_twinPeak()
{
	...
	if(get_property("lh_oilpeak") != "finished")
	{
		return false;
	}

	buffMaintain($effect[Fishy Whiskers], 0, 1, 1);
	if(get_property("lh_twinpeakprogress").to_int() == 0)
	{
		print("Twin Peak", "blue");
		set_property("choiceAdventure604", "1");
		set_property("choiceAdventure618", "2");
		buffMaintain($effect[Joyful Resolve], 0, 1, 1);
		lhAdv(1, $location[Twin Peak]);
		if(last_monster() != $monster[gourmet gourami])
		{
			...
			set_property("lh_twinpeakprogress", "1");
	...
	if(!attempt && needStench)
	{
		...
		if(elemental_resist($element[stench]) < 4 && !useMaximizeToEquip())
		{
			...
			{
				lhEquip($slot[acc3], $item[Training Legwarmers]);
			}

			familiar resist = $familiar[none];
			if((elemental_resist($element[stench]) < 4) && !is100FamiliarRun())
			{
				if(lh_have_familiar($familiar[Exotic Parrot]))
				{
					resist = $familiar[Exotic Parrot];
				}
				if(lh_have_familiar($familiar[Trick-Or-Treating Tot]))
				{
					if(possessEquipment($item[li'l candy corn costume]) && lh_is_valid($item[li'l candy corn costume]))
					{
				...
				if(resist != $familiar[none])
				{
					...
					{
						lhEquip($slot[familiar], $item[li'l candy corn costume]);
					}
				}
			}
	...
	if(lhAdvBypass(297, $location[Twin Peak]))
	{
		if(lastAdventureSpecialNC())
		{
			lhAdv(1, $location[Twin Peak]);
...
boolean L9_oilPeak()
{
	...
	if((my_class() == $class[Ed]) && get_property("lh_dickstab").to_boolean())
	{
...
boolean LX_loggingHatchet()
{
	...
	lhAdv(1, $location[Camp Logging Camp]);
	return true;
}
...
#	}
	...
	if(get_property("smutOrcNoncombatProgress").to_int() == 15)
	{
		...
		if (my_class() == $class[Vampyre] && have_skill($skill[Sinister Charm]) && !useMaximizeToEquip())
		{
			...
			lhMaximize("myst, 50 sleaze res", 1000, 0, false);
			bat_formMist();
			buffMaintain($effect[Spectral Awareness], 10, 1, 1);
			set_property("choiceAdventure1345", 3);
		}
		...
		lhAdv(1, $location[The Smut Orc Logging Camp]);
		return true;
	}
	else
	{
		if(setFlavour($element[cold]) && lh_have_skill($skill[Stuffed Mortar Shell]))
		{
	...
	{
		cloverUsageInit();
		lhAdvBypass("adventure.php?snarfblat=295", $location[The Smut Orc Logging Camp]);
		cloverUsageFinish();
		visit_url("place.php?whichplace=orc_chasm&action=bridge"+(to_int(get_property("chasmBridgeProgress"))));
	}
	else
...
boolean L11_redZeppelin()
{
	...
	{
		return lhAdv($location[A Mob Of Zeppelin Protesters]);
	}
	...
	{
		lhMaximize("sleaze dmg, sleaze spell dmg", 2500, 0, false);
	}
	lh_beachCombHead("sleaze");
	foreach it in $items[lynyrdskin breeches, lynyrdskin cap, lynyrdskin tunic]
	{
		if(possessEquipment(it) && lh_can_equip(it) &&
	...
	{
		return lhAdvBypass("inv_use.php?pwd=&whichitem=7204&checked=1", $location[A Mob of Zeppelin Protesters]);
	}

	if(cloversAvailable() > 0 && get_property("zeppelinProtestors").to_int() < 75)
	{
		if(cloversAvailable() >= 3 && get_property("lh_useWishes").to_boolean())
		{
		...
		if(best_protestors >= 10)
		{
			...
			cloverUsageInit();
			boolean retval = lhAdv(1, $location[A Mob of Zeppelin Protesters]);
			cloverUsageFinish();
			return retval;
		}
	}
	...
	boolean retval = lhAdv($location[A Mob Of Zeppelin Protesters]);
	if(!lastAdventureSpecialNC())
	{
...
boolean L11_ronCopperhead()
{
	...
	if((internalQuestStatus("questL11Ron") == 2) || (internalQuestStatus("questL11Ron") == 3) || (internalQuestStatus("questL11Ron") == 4))
	{
		...
		bat_formBats();
		boolean retval = lhAdv($location[The Red Zeppelin]);
...
boolean L11_shenCopperhead()
{
	...
	if((internalQuestStatus("questL11Shen") == 0) || (internalQuestStatus("questL11Shen") == 2) || (internalQuestStatus("questL11Shen") == 4) || (internalQuestStatus("questL11Shen") == 6))
	{
		if((item_amount($item[Crappy Waiter Disguise]) > 0) && (have_effect($effect[Crappily Disguised as a Waiter]) == 0) && !in_tcrs())
		{
			...
			switch(get_property("lh_copperhead").to_int())
			{
		...
		boolean retval = lhAdv($location[The Copperhead Club]);
		if(get_property("lastEncounter") == "Behind the 'Stache")
		{
			switch(get_property("choiceAdventure855").to_int())
			{
			case 1:
				set_property("lh_copperhead", get_property("lh_copperhead").to_int() | 1);
				break;
			case 2:
				set_property("lh_copperhead", get_property("lh_copperhead").to_int() | 2);
				break;
			case 3:
				set_property("lh_copperhead", get_property("lh_copperhead").to_int() | 4);
				break;
			}
		}
		return retval;
...
boolean L11_forgedDocuments()
{
	...
	if(get_property("lh_blackmap") != "document")
	{
		return false;
	}
	if(item_amount($item[Forged Identification Documents]) != 0)
...
boolean L11_fistDocuments()
{
	...
	return lhAdvBypass(0, pages, $location[Noob Cave], "");
}
...
#	}
	...
	if($location[The Black Forest].turns_spent > 12)
	{
		...
		{
			abort("We have spent too many turns in The Black Forest and haven't found The Black Market. Something is wrong. (Find Black Forest, set lh_blackmap=document, do not buy Forged Identification Documents");
		}
	}

	print("Must find the Black Market: " + get_property("blackForestProgress"), "blue");
	if(get_property("lh_blackfam").to_boolean())
	{
		council();
		...
		set_property("lh_blackfam", false);
		set_property("choiceAdventure923", "1");
	}
	...
	lhEquip($slot[acc3], $item[Blackberry Galoshes]);
...
boolean L10_holeInTheSky()
{
	...
	if(!get_property("lh_getStarKey").to_boolean())
	{
		return false;
	}
	...
	if(lh_have_familiar($familiar[Space Jellyfish]))
	{
		handleFamiliar($familiar[Space Jellyfish]);
	...
	lhAdv(1, $location[The Hole In The Sky]);
	return true;
}
...
{
	if(get_property("lh_trapper") == "finished")
	{
		return false;
	}
	...
	if(!canGroar && in_hardcore() && ((lh_my_path() == "Avatar of Sneaky Pete") || !lh_have_familiar($familiar[Jumpsuited Hound Dog]) || is100FamiliarRun($familiar[Jumpsuited Hound Dog])))
	{
	...
	if(get_property("lh_powerLevelAdvCount").to_int() > 8)
	{
		if(L8_trapperExtreme())
		{
			return true;
	...
	if((item_amount($item[Groar\'s Fur]) > 0) || (item_amount($item[Winged Yeti Fur]) > 0) || (internalQuestStatus("questL08Trapper") == 5))
	{
		...
		{
			lh_autosell(5, $item[Dense Meat Stack]);
		}
		set_property("lh_trapper", "finished");
		council();
		return true;
	...
	if(canGroar)
	{
		...
		if((elemental_resist($element[cold]) < 5) && (my_level() == get_property("lh_powerLevelLastLevel").to_int()))
		{
			lhMaximize("cold res 5 max,-tie,-equip snow suit", 0, 0, true);
			int coldResist = simValue("Cold Resistance");
			if(coldResist >= 5)
			{
				if(useMaximizeToEquip())
				...
				{
					lhMaximize("cold res,-tie,-equip snow suit,-weapon", 0, 0, false);
				}
			}
		}
		...
		if(elemental_resist($element[cold]) >= 5)
		{
			...
			if((item_amount($item[Groar\'s Fur]) == 0) && (item_amount($item[Winged Yeti Fur]) == 0))
			{
				...
				lhAdv(1, $location[Mist-shrouded Peak]);
			}
			else
			{
				visit_url("place.php?whichplace=mclargehuge&action=trappercabin");
...
boolean L8_trapperExtreme()
{
	...
	{
		if(lhOutfit("eXtreme Cold-Weather Gear"))
		{
			set_property("choiceAdventure575", "3");
			lhAdv(1, $location[The eXtreme Slope]);
			return true;
		}
		else
		{
...
{
	if(get_property("lh_trapper") != "yeti")
	{
		return false;
	}
	...
	if(get_property("_sourceTerminalDigitizeMonster") == $monster[Ninja Snowman Assassin])
	{
		if(loopHandler("_lh_digitizeAssassinTurn", "_lh_digitizeAssassinCounter", "Potentially unable to do anything while waiting on digitized Ninja Snowman Assassin.", 10))
		{
	...
	if(in_hardcore())
	{
		...
		if(!lhAdv(1, $location[Lair of the Ninja Snowmen]))
		{
...
boolean lh_tavern()
{
	...
	foreach loc in locations
	{
		...
		if(possessEquipment($item[Kremlin's Greatest Briefcase]))
		{
			...
			{
				lhEquip($slot[acc3], $item[Kremlin\'s Greatest Briefcase]);
			}
		}
		...
		{
			if(lh_have_skill($skill[Intimidating Mien]))
			{
		...
		if(char_at(tavern, loc) == "0")
		{
			...
			if(lhAdvBypass("cellar.php?action=explore&whichspot=" + actual, $location[Noob Cave]))
			{
				return true;
			}
...
{
	if((get_property("lh_ballroomopen") != "open") || (get_property("lh_ballroomsong") == "finished"))
	{
		return false;
	}
...
boolean autosellCrap()
{
	...
	{
		lh_autosell(1, $item[dense meat stack]);
	}
	foreach it in $items[Blue Money Bag, Red Money Bag, White Money Bag]
	{
		if(item_amount(it) > 0)
		{
			lh_autosell(item_amount(it), it);
		}
	}
	...
	foreach it in $items[Anticheese, Awful Poetry Journal, Beach Glass Bead, Beer Bomb, Chaos Butterfly, Clay Peace-Sign Bead, Decorative Fountain, Dense Meat Stack, Empty Cloaca-Cola Bottle, Enchanted Barbell, Fancy Bath Salts, Frigid Ninja Stars, Feng Shui For Big Dumb Idiots, Giant Moxie Weed, Half of a Gold Tooth, Headless Sparrow, Imp Ale, Keel-Haulin\' Knife, Kokomo Resort Pass, Leftovers Of Indeterminate Origin, Mad Train Wine, Mangled Squirrel, Margarita, Meat Paste, Mineapple, Moxie Weed, Patchouli Incense Stick, Phat Turquoise Bead, Photoprotoneutron Torpedo, Plot Hole, Procrastination Potion, Rat Carcass, Smelted Roe, Spicy Jumping Bean Burrito, Spicy Bean Burrito, Strongness Elixir, Sunken Chest, Tambourine Bells, Tequila Sunrise, Uncle Jick\'s Brownie Mix, Windchimes]
	{
		if(item_amount(it) > 0)
		{
			lh_autosell(min(5,item_amount(it)), it);
		}
	}
	if(item_amount($item[hot wing]) > 3)
	{
		lh_autosell(item_amount($item[hot wing]) - 3, $item[hot wing]);
	}
	return true;
}
...
void print_header()
{
	if(my_thunder() > get_property("lh_lastthunder").to_int())
	{
		set_property("lh_lastthunderturn", "" + my_turncount());
		set_property("lh_lastthunder", "" + my_thunder());
	}
	if(in_hardcore())
	{
	...
	if(((item_amount($item[Rock Band Flyers]) == 1) || (item_amount($item[Jam Band Flyers]) == 1)) && (get_property("flyeredML").to_int() < 10000) && !get_property("lh_ignoreFlyer").to_boolean())
	{
...
boolean doTasks()
{
	...
	if(get_property("lh_doCombatCopy") == "yes")
	...
	int delay = get_property("lh_delayTimer").to_int();
	if(delay != 0)
	{
	...
	int paranoia = get_property("lh_paranoia").to_int();
	if(paranoia != -1)
	{
		int paranoia_counter = get_property("lh_paranoia_counter").to_int();
		...
		{
			set_property("lh_paranoia_counter", paranoia_counter + 1);
		}
	}
	if(get_property("lh_helpMeMafiaIsSuperBrokenAaah").to_boolean())
	{
		cli_execute("refresh inv");
	}
	bat_formNone();
	...
	if(get_property("lh_beatenUpCount").to_int() > 5)
	{
		songboomSetting("dr");
	}
	else if ((get_property("lh_prewar") == "started") && (get_property("lh_war") != "finished"))
	{
		// Once we've started the war, we want to be able to micromanage songs
		// for Gremlins and Nuns. Don't break this for them.
	}
	else if((my_class() != $class[Ed]) && (get_property("lh_crypt") != "finished") && (get_property("_boomBoxFights").to_int() == 10) && (get_property("_boomBoxSongsLeft").to_int() > 3))
	{
	...
	else
	{
		...
		{
			if((lh_my_path() == "G-Lover") && (my_meat() > 10000))
			{
				songboomSetting("dr");
			}
			else
	...
	if(get_property("lh_useCubeling").to_boolean())
	{
		...
		if(get_property("lh_cubeItems").to_boolean() && (my_familiar() != $familiar[Gelatinous Cubeling]) && lh_have_familiar($familiar[Gelatinous Cubeling]))
		{
			handleFamiliar($familiar[Gelatinous Cubeling]);
		}
	}

	if((my_daycount() == 1) && ($familiar[Fist Turkey].drops_today < 5) && lh_have_familiar($familiar[Fist Turkey]))
	{
		handleFamiliar($familiar[Fist Turkey]);
	}
...
#	if(LX_dictionary())					return true;
	...
	if((my_level() >= 12) && (item_amount($item[Rock Band Flyers]) == 0) && (item_amount($item[Jam Band Flyers]) == 0) && (get_property("flyeredML").to_int() < 10000) && ((get_property("lh_hiddenapartment") == "0") || (get_property("lh_hiddenapartment") == "finished")) && ((have_effect($effect[Ultrahydrated]) == 0) || (get_property("desertExploration").to_int() >= 100)))
	{
	...
	if(knoll_available() && get_property("lh_spoonconfirmed").to_int() == my_ascensions())
	{
	...
	if(!get_property("lh_slowSteelOrgan").to_boolean() && get_property("lh_getSteelOrgan").to_boolean())
	{
	...
	if((get_property("lh_nuns") == "done") && (item_amount($item[Half A Purse]) == 1))
	{
	...
	if((my_level() >= 12) && ((get_property("hippiesDefeated").to_int() >= 192) || get_property("lh_hippyInstead").to_boolean()) && (get_property("lh_nuns") == ""))
	{
		if(doThemtharHills())
		{
			return true;
	...
	{
		int[item] camp = lh_get_campground();
	...
	if(in_koe())
	{
		if(get_property("lh_war") != "finished" && (get_property("hippiedDefeated").to_int() < 333) && (get_property("fratboysDefeated").to_int() < 333) && can_equip($item[Distressed denim pants]) && can_equip($item[beer helmet]) && can_equip($item[bejeweled pledge pin]))
		{
			handleFamiliar("item");
			if(haveWarOutfit())
			{
			...
			item warKillDoubler = get_property("lh_hippyInstead").to_boolean() ? $item[Jacob's rung] : $item[Haunted paddle-ball];
			pullXWhenHaveY(warKillDoubler, 1, 0);
			if(possessEquipment(warKillDoubler))
			{
				lhEquip($slot[weapon], warKillDoubler);
			}
			...
			if(!lhAdvBypass("adventure.php?snarfblat=533", $location[The Exploaded Battlefield]))
			{
	...
	{
		if((get_property("hippiesDefeated").to_int() < 64) && (get_property("fratboysDefeated").to_int() < 64) && (my_level() >= 12) && (get_property("lh_prewar") == "started") && (get_property("lh_war") != "finished"))
		{
		...
		if((get_property("hippiesDefeated").to_int() < 192) && (get_property("fratboysDefeated").to_int() < 192) && (my_level() >= 12) && (get_property("lh_prewar") != ""))
		{
		...
		if(((get_property("lh_nuns") == "finished") || (get_property("lh_nuns") == "done")) && ((get_property("hippiesDefeated").to_int() < 1000) && (get_property("fratboysDefeated").to_int() < 1000)) && (my_level() >= 12))
		{
...
#	if(my_class() == $class[Astral Spirit])
	...
	print("This is version: " + svn_info("lh_ascend").last_changed_rev + " Mafia: " + get_revision());
	print("This is day " + my_daycount() + ".");
	print("Turns played: " + my_turncount() + " current adventures: " + my_adventures());
	print("Current Ascension: " + lh_my_path());
	...
	backupSetting("kingLiberatedScript", "scripts/lordHaps.ash");
	backupSetting("afterAdventureScript", "scripts/postHaps.ash");
	backupSetting("betweenAdventureScript", "scripts/preHaps.ash");
	backupSetting("betweenBattleScript", "scripts/preHaps.ash");
	...
	{
		backupSetting("counterScript", "scripts/lh_ascend/lhCounter.ash");
	}
	...
	if(contains_text(charpane, "<hr width=50%><table"))
	{
		...
		set_property("lh_priorCharpaneMode", 1);
	...
	while((my_adventures() > 1) && (my_inebriety() <= inebriety_limit()) && !(my_inebriety() == inebriety_limit() && my_familiar() == $familiar[Stooper]) && !get_property("kingLiberated").to_boolean() && doTasks())
	{
		...
		if((my_adventures() == 1) && !get_property("lh_limitConsume").to_boolean())
		{
			keepOnTruckin();
		}
	}
...
{
	print_html("Thank you for using lh_ascend!");
	print_html("If you need to configure or interrupt the script, choose <b>hapsascend</b> from the drop-down \"run script\" menu in your browser.");
	//print_html("If you want to contribute, please open an issue <a href=\"https://github.com/soolar/lh_ascend/issues\">on Github</a>");
...
//Although this is sad times, I am continuing to play I dont need a constant reminder so disabling this
// will still leave this here as a memory for the abused though
void sad_times()
{
	//print_html('Soolar (the maintainer of lh_ascend) and Jeparo (the most active contributor) have decided to cease development of lh_ascend in response to Jick\'s behavior that has recently <a href="https://www.reddit.com/r/kol/comments/d0cq9s/allegations_of_misconduct_by_asymmetric_members/">come to light</a>. The KoL community is amazing, and we love it, but we no longer want to contribute to the game in the way that we have until now. We wish you all the best in the future. Please see the readme on the <a href="https://github.com/soolar/lh_ascend">github</a> page for more information.');
}
