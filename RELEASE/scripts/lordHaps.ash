...
{
	restoreAllSettings();
	if(get_property("kingLiberated").to_boolean() && (get_property("lh_snapshot") == ""))
	{
		print("Yay! The King is saved. I suppose you should do stuff.");
		if(!get_property("lh_kingLiberation").to_boolean())
		{
			set_property("lh_snapshot", "aborted");
			return;
		}
...
//		visit_url("lair2.php?preaction=key&whichkey=436");
		...
		if(get_property("lh_borrowedTimeOnLiberation").to_boolean() && (get_property("_borrowedTimeUsed") == "false"))
		{
	...
	if((get_property("kingLiberated") == "true") && !get_property("lh_aftercore").to_boolean())
	{
...
//		buy_item($item[hand in glove], 1, 5000);
	...
	if(get_property("lh_clearCombatScripts").to_boolean())
	{
...
boolean pullPVPJunk()
{
	if(!get_property("lh_pullPVPJunk").to_boolean())
	{
		return false;
	}
